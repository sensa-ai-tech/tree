...
> ⚠️ The v1 seed had a recurring **fabricated-"consensus" hallucination**: authoritative-sounding `ACVIM/ACVD/VSSO Consensus – Topic` titles carrying a wrong DOI (resolves to an unrelated real paper) or — in `open-access-resources.ts` — a wrong PMC URL (e.g. an MUO "ACVIM consensus" entry whose link opened a human-obstetrics sepsis paper). When adding a guideline ref, confirm the consensus actually exists; `open-access-resources.ts` URLs still need a liveness/title audit.
