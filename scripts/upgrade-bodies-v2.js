/**
 * upgrade-bodies-v2.js
 * Adds 7-aspect section headers to body fields in contents.ts files.
 * Fixed version: pre-scans entire file for node_id→body mapping.
 */
const fs = require('fs');
const path = require('path');

const SPECIALTIES = ['im', 'derm', 'surg', 'neuro', 'onco', 'ecc', 'cpath'];
const SEED_DIR = path.join(__dirname, '..', 'src', 'data', 'seed');

function isUpgraded(bodyContent) {
  return bodyContent.includes('\u4e00\u3001') &&
    (bodyContent.includes('\u4eba\u91ab\u501f\u9452') || bodyContent.includes('\u4eba\u91ab\u501f\u9451'));
}

/**
 * Pre-scan entire file to build nodeId→position mapping,
 * then match each body to its closest preceding node_id.
 */
function buildNodeIdMap(content) {
  const nodeIdRegex = /node_id:\s*'([^']+)'/g;
  const entries = [];
  let m;
  while ((m = nodeIdRegex.exec(content)) !== null) {
    entries.push({ nodeId: m[1], pos: m.index });
  }
  return entries;
}

function getNodeTypeFromId(nodeId) {
  if (nodeId.includes('-L0-')) return 'L0';
  if (nodeId.includes('-L1-')) return 'L1';
  if (nodeId.includes('-L2-')) return 'L2';
  if (nodeId.includes('-L3-')) return 'L3';
  if (nodeId.includes('-L4-')) return 'L4';
  if (nodeId.includes('-L5-')) return 'L5';
  return 'unknown';
}

function findNodeIdForBody(nodeIdEntries, bodyPos) {
  // Find the closest node_id BEFORE this body position
  let best = null;
  for (const entry of nodeIdEntries) {
    if (entry.pos < bodyPos) {
      best = entry;
    } else {
      break; // entries are sorted by position
    }
  }
  return best ? best.nodeId : 'unknown';
}
...
function buildL3Body(titleLine, sections) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (buckets[i].length > 0) {
      for (const sec of buckets[i]) {
        if (sec.heading) result += sec.heading.replace(/^## /, '### ') + '\n';
...
function buildNonL3Body(titleLine, sections, nodeType) {
  let result = titleLine + '\n\n';
  ...
  if (nodeType === 'L0' || nodeType === 'L1' || nodeType === 'L2') {
    ...
    // Fallback: treat unknown as L0/L1/L2 style (4 sections)
    headers = [
      '## \u4e00\u3001\u6838\u5fc3\u6982\u5ff5\u8207\u6a5f\u8f49 (Core Concept & Mechanism)',
      '## \u4e8c\u3001\u81e8\u5e8a\u610f\u7fa9 (Clinical Significance)',
      '## \u4e09\u3001\u4eba\u91ab\u501f\u9452 (Translational Insights)',
...
function processFile(specialty) {
  ...
  const nodeIdEntries = buildNodeIdMap(content);
  let processed = 0;
  let skipped = 0;
  ...
  while ((match = bodyRegex.exec(content)) !== null) {
    ...
    const nodeId = findNodeIdForBody(nodeIdEntries, bodyStart);
    const nodeType = getNodeTypeFromId(nodeId);
    const title = bodyContent.split('\n').find(l => l.startsWith('# ')) || bodyContent.split('\n')[0];

    console.log(`  Processing: ${nodeId} (${nodeType}) "${title.substring(0, 50)}"`);

    const { titleLine, sections } = extractSections(bodyContent);

    let upgraded;
    if (nodeType === 'L3') {
      upgraded = buildL3Body(titleLine, sections);
    } else {
      upgraded = buildNonL3Body(titleLine, sections, nodeType);
    }
...
console.log('=== Upgrading body fields to 7-aspect format (v2) ===\n');
...
for (const specialty of SPECIALTIES) {
  ...
  console.log(`  → Upgraded: ${result.processed}, Already done: ${result.skipped}\n`);
