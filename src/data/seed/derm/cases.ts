...
export const DERM_CASES: CaseChallenge[] = [
    ...
    steps: [
      {
        ...
        feedback_correct: '很好！搔癢嚴重度評分（PVAS）是評估治療反應的基線。飲食細節對後續排除飲食試驗的設計不能省（需選擇從未接觸的蛋白質來源）。跳蚤預防的完整性影響 FAD 的排除。',
      ...
      },
      {
        ...
        feedback_incorrect: 'CAD 的典型皮膚分佈（面部、腋下、腹股溝、指間）與 FAD（腰背、尾根）有明顯差異。辨識分佈型態是鑑別過敏性皮膚病的第一步。外耳炎的耳道分泌物特徵也提供感染類型的線索。',
        key_learning_points: [
      ...
      },
      {
        ...
        feedback_incorrect: '繼發感染的控制是過敏性皮膚病管理中不能省的步驟。感染會明顯加劇瘙癢，若不先控制感染就評估底層過敏，可能高估疾病嚴重度。',
        key_learning_points: [
  ...
  // --- CASE-DERM-002 ---
  {
    ...
    steps: [
      { step_number: 1, step_type: 'history_collection', title: '病史採集', prompt: '你需要後續詢問哪些問題以區分 CAD 與 FAD？選擇 3 項。', available_actions: ['確認搔癢分佈的精確部位與季節性變化', '確認 fipronil 的使用規律性', '詢問家中環境是否有跳蚤或蟲卵', '詢問飼主是否被跳蚤叮咬', '詢問是否常去公園', '詢問食慾或體重變化'], correct_actions: ['確認搔癢分佈的精確部位與季節性變化', '確認 fipronil 的使用規律性', '詢問家中環境是否有跳蚤或蟲卵'], feedback_correct: '正確！搔癢分佈型態是區分 CAD（腋下/腹股溝/指間）與 FAD（腰背/尾根）的第一線指標。Fipronil 效力不如 isoxazoline 穩定。環境跳蚤調查有助判斷暴露風險。', feedback_incorrect: '鑑別 CAD 與 FAD 核心：(1) 病灶分佈；(2) 跳蚤預防完整性；(3) 環境跳蚤暴露證據。', key_learning_points: ['FAD 典型分佈：腰背、尾根、後肢內側', 'CAD 典型分佈：腋下、腹股溝、指間、耳翼', 'Fipronil 效力可能不足以排除 FAD，isoxazoline 類更可靠'] },
      ...
      { step_number: 5, step_type: 'treatment_plan', title: '長期管理', prompt: '排除飲食 8 週無後續改善，飲食挑戰無惡化。最終 FAD+CAD。長期方案？', available_actions: ['全年 isoxazoline+oclacitinib/lokivetmab+定期監測', '僅跳蚤預防', '長期 prednisolone', '停跳蚤預防僅用 oclacitinib'], correct_actions: ['全年 isoxazoline+oclacitinib/lokivetmab+定期監測'], feedback_correct: '正確！FAD 需全年跳蚤預防，CAD 用 oclacitinib 或 lokivetmab。', feedback_incorrect: '停跳蚤預防會 FAD 復發。Prednisolone 長期副作用多。', key_learning_points: ['台灣氣候下 FAD 犬需全年預防', 'Oclacitinib 適合 CAD 長期控制', 'Lokivetmab 適合口服困難犬隻'] },
    ],
  ...
  // --- CASE-DERM-008 ---
  {
    id: 'CASE-DERM-008', title: '新藥物後皮膚病變，藥物不良反應 vs 自體免疫', specialty: 'DERM', cross_specialties: ['IM', 'CPATH'], required_nodes: ['DERM-L3-011', 'DERM-L3-010', 'DERM-L2-003', 'DERM-L4-003', 'DERM-L5-003'], difficulty: 4, species: '犬',
    scenario: { signalment: '6 歲已絕育雄性米格魯（Beagle），體重 12 kg', chief_complaint: '口鼻部、耳翼、腹部出現紅斑靶心樣病灶與水泡，快速惡化 5 天', brief_history: '10 天前因尿道感染開始服用 trimethoprim-sulfonamide（TMS）。服藥第 5 天開始出現口鼻部紅斑，隨後擴展至耳翼、腋下與腹部。病灶從紅斑進展為靶心樣同心圓病變，部分形成水泡與表皮剝離。疼痛明顯，搔癢輕微。食慾下降。體溫 39.5°C。過去無類似皮膚問題。' },
    steps: [
      { step_number: 1, step_type: 'history_collection', title: '病史採集', prompt: '選擇最重要的 3 項問題以釐清藥物不良反應的可能性。', available_actions: ['確認所有近期使用藥物（含劑量、開始日期）', '確認近期是否有疫苗接種', '詢問病灶出現與用藥的時間關係', '詢問過去是否對任何藥物過敏', '詢問飲食變更', '詢問近期是否有外傷'], correct_actions: ['確認所有近期使用藥物（含劑量、開始日期）', '詢問病灶出現與用藥的時間關係', '確認近期是否有疫苗接種'], feedback_correct: '正確！完整用藥史是藥物不良反應診斷的基石。時間關係是因果推論的關鍵，藥物反應通常在用藥後 1-3 週出現。疫苗也可誘發多形性紅斑或天疱瘡。', feedback_incorrect: '藥物不良反應最重要的線索是用藥時間線。多形性紅斑（EM）與 TEN 最常發生在用藥後 7-21 天。', key_learning_points: ['藥物不良反應的時間窗：通常用藥後 7-21 天', 'TMS 是犬藥物反應最常見的誘因藥物', '疫苗也可誘發免疫介導性皮膚反應', '完整用藥史是鑑別的第一步'] },
      { step_number: 2, step_type: 'physical_examination', title: '理學檢查', prompt: 'TMS 開始第 5 天出現紅斑，目前第 10 天。無近期疫苗。選擇最關鍵的理學檢查發現。', available_actions: ['口鼻部與耳翼紅斑伴靶心樣同心圓病變（target lesions）', '腹部多處水泡，部分破裂形成潰瘍', '口腔黏膜潰瘍與出血', '足墊表皮輕度剝離', '體溫 39.5°C，下頷淋巴結腫大', '腰背部皮膚正常'], correct_actions: ['口鼻部與耳翼紅斑伴靶心樣同心圓病變（target lesions）', '腹部多處水泡，部分破裂形成潰瘍', '口腔黏膜潰瘍與出血'], feedback_correct: '正確！靶心樣同心圓病變（target lesions）是多形性紅斑（EM）的標誌性皮膚表現。水泡與表皮剝離提示較嚴重的 EM major 或可能進展至 TEN。口腔黏膜受累支持 EM major。合併黏膜受累使預後較差。', feedback_incorrect: 'Target lesions 是 EM 最具特徵性的表現。黏膜受累將 EM minor 升級為 EM major。', key_learning_points: ['Target lesions（靶心樣同心圓病變）是 EM 的標誌性表現', '黏膜受累=EM major，預後較差', '足墊剝離提示可能進展至 TEN', '需評估體表受累面積（BSA）以區分 EM vs SJS vs TEN'] },
      { step_number: 3, step_type: 'differential_diagnosis', title: '鑑別診斷', prompt: '根據目前發現，列出最重要的鑑別診斷。選擇 3 項。', available_actions: ['藥物誘發多形性紅斑（drug-induced EM）', '天疱瘡（pemphigus foliaceus）', '全身性紅斑狼瘡（SLE）', '深層膿皮症', '跳蚤過敏性皮膚炎', '接觸性皮膚炎'], correct_actions: ['藥物誘發多形性紅斑（drug-induced EM）', '天疱瘡（pemphigus foliaceus）', '全身性紅斑狼瘡（SLE）'], feedback_correct: '正確！鑑別三大免疫介導性/藥物相關皮膚病：(1) 藥物誘發 EM 最符合，靶心樣病變+黏膜潰瘍+用藥時間線。(2) PF 的水泡結痂可類似，但 PF 鮮少有靶心樣病變且不侵犯口腔黏膜。(3) SLE 可有多系統受累與皮膚黏膜病變。深層膿皮症可模仿但時間線不符。', feedback_incorrect: '免疫介導性皮膚病的鑑別需考慮：病灶型態（target lesions vs 鬆弛水泡 vs 蝴蝶斑）、黏膜受累、用藥史、多系統症狀。', key_learning_points: ['EM 特徵：target lesions + 用藥時間線', 'PF 特徵：表淺水泡結痂好發鼻鏡/足墊，不侵犯口腔', 'SLE 特徵：多系統受累 + 陽性 ANA', '深層膿皮症可模仿自體免疫皮膚病'] },
      { step_number: 4, step_type: 'diagnostic_plan', title: '診斷計畫', prompt: '安排哪些檢查？選擇 3 項。', available_actions: ['皮膚切片送組織病理（取靶心樣病灶邊緣）', '皮膚細胞學（水泡底部印片）', 'CBC 生化 尿檢（評估全身狀態與 TMS 相關血液異常）', '血清 IgE 檢測', 'ANA 檢測', '細菌培養'], correct_actions: ['皮膚切片送組織病理（取靶心樣病灶邊緣）', '皮膚細胞學（水泡底部印片）', 'CBC 生化 尿檢（評估全身狀態與 TMS 相關血液異常）'], feedback_correct: '完美！組織病理是 EM 確診金標準，界面皮膚炎（interface dermatitis）伴個別角質細胞壞死。細胞學排除感染性原因（acantholytic cells→PF）。TMS 可引起血液學異常（貧血、血小板減少、嗜中性球減少），需監測。', feedback_incorrect: 'EM 確診需組織病理。ANA 在此不是優先，SLE 不是最主要鑑別。TMS 可影響骨髓需監測。', key_learning_points: ['組織病理是 EM 確診金標準', 'EM 組織學：界面皮膚炎 + 個別角質細胞壞死', '細胞學排除 PF（acantholytic cells）', 'TMS 可引起藥物性血液異常需監測'] },
      { step_number: 5, step_type: 'result_interpretation', title: '結果判讀', prompt: '結果：\n- 組織病理：界面皮膚炎，基底層與棘層個別角質細胞壞死（衛星細胞壞死），淋巴球浸潤\n- 細胞學：未見 acantholytic cells，少量嗜中性球\n- CBC：輕度嗜中性球減少（3,200/μL）\n\n確定診斷為何？', available_actions: ['藥物誘發多形性紅斑（EM major）確診', '天疱瘡確診', '全身性紅斑狼瘡確診', '結果不確定需更多檢查'], correct_actions: ['藥物誘發多形性紅斑（EM major）確診'], feedback_correct: '正確！組織病理符合 EM 典型，界面皮膚炎+衛星細胞壞死。臨床 target lesions+黏膜潰瘍=EM major。TMS 時間線+嗜中性球減少（TMS 副作用）後續支持藥物誘發。無 acantholytic cells 排除 PF。', feedback_incorrect: 'EM 的確診結合：組織病理（界面皮膚炎+角質細胞壞死）+ 臨床表現（target lesions）+ 用藥時間線。', key_learning_points: ['衛星細胞壞死（satellite cell necrosis）= EM 核心病理', '界面皮膚炎 + target lesions + 用藥史 = 藥物誘發 EM', 'EM major = EM + 黏膜受累', 'TMS 相關嗜中性球減少需監測'] },
      { step_number: 6, step_type: 'treatment_plan', title: '治療計畫', prompt: '治療方案？選擇最適當的組合。', available_actions: ['立即停用 TMS + 支持性治療 + 免疫抑制劑量 prednisolone + 止痛', '停 TMS 改用其他磺胺類', '繼續 TMS 加用 prednisolone', '僅外用類固醇'], correct_actions: ['立即停用 TMS + 支持性治療 + 免疫抑制劑量 prednisolone + 止痛'], feedback_correct: '正確！治療三大核心：(1) 立即停用可疑藥物，最關鍵的第一步。(2) 免疫抑制劑量 prednisolone（2-4 mg/kg/day）控制免疫反應。(3) 支持性治療包括靜脈輸液、止痛（tramadol/gabapentin）、口腔護理、防止繼發感染。泌尿道感染改用 amoxicillin-clavulanate 等非磺胺類抗生素。', feedback_incorrect: '停用可疑藥物是治療成功的先決條件。繼續使用或換同類藥物會惡化病情。', key_learning_points: ['立即停藥是 drug eruption 治療最關鍵的一步', 'Prednisolone 免疫抑制劑量控制免疫反應', '支持性治療：輸液、止痛、口腔護理', '不可更換為同類藥物（交叉反應）'] },
      { step_number: 7, step_type: 'case_summary', title: '病例總結與預後', prompt: '停 TMS 後 3 天病灶開始改善，2 週後大部分癒合。關於此病例的長期建議？', available_actions: ['記錄藥物過敏史，終身避免磺胺類+嘧啶類藥物，告知飼主交叉反應風險', '3 個月後可再嘗試低劑量 TMS', '不需特別記錄', '改用 doxycycline 預防復發'], correct_actions: ['記錄藥物過敏史，終身避免磺胺類+嘧啶類藥物，告知飼主交叉反應風險'], feedback_correct: '正確！終身禁止使用 TMS 及其他磺胺類/嘧啶類藥物。病歷與飼主均需明確記錄。告知飼主：再次暴露可能引發更嚴重且更快速的反應（記憶性免疫反應）。EM major 預後通常良好（停藥後 2-4 週癒合），但 TEN 死亡率可達 30-50%。', feedback_incorrect: '藥物不良反應一旦確認，終身禁止使用同類藥物。再次暴露可能致命。', key_learning_points: ['終身避免誘發藥物及同類藥物', '病歷與飼主雙重記錄藥物過敏', '再次暴露可能更快更嚴重', 'EM major 預後良好（停藥後 2-4 週癒合）', 'TEN 預後差（死亡率 30-50%）'] },
    ],
    expert_pathway: { reasoning: '米格魯+TMS 用藥第 5 天出現皮膚病變+target lesions+黏膜潰瘍+組織病理界面皮膚炎+衛星細胞壞死=藥物誘發 EM major。停藥+免疫抑制+支持性治療。', key_decisions: ['辨識 target lesions 指向 EM', '詳細用藥史建立時間線', '組織病理確診 EM', '鑑別 PF 與 SLE', '立即停用 TMS', '選擇適當免疫抑制方案'], common_pitfalls: ['未詳問用藥史直接按自體免疫治療', '未停藥即開始免疫抑制', '更換為同類磺胺抗生素', '未監測 TMS 相關血液異常', '臨床改善後未記錄藥物過敏'] },
    related_nodes_for_review: ['DERM-L3-011', 'DERM-L3-010', 'DERM-L2-003', 'DERM-L4-003', 'DERM-L5-003'],
    tags: ['drug_eruption', 'erythema_multiforme', 'TMS', 'sulfonamide', 'autoimmune_DDx', 'Beagle', 'case_challenge'],
  },

  // --- CASE-DERM-009: Recurrent Pyoderma ---
  {
    id: 'CASE-DERM-009', title: '鬥牛犬反覆膿皮症，尋找底層病因', specialty: 'DERM', cross_specialties: ['IM', 'CPATH'], required_nodes: ['DERM-L3-005', 'DERM-L3-001', 'DERM-L4-003', 'DERM-L5-002'], difficulty: 3, species: '犬',
    scenario: { signalment: '5 歲已絕育雄性英國鬥牛犬，體重 25 kg', chief_complaint: '今年第三次膿皮症發作，抗生素治療後復發', brief_history: '每次膿皮症以 cephalexin 治療約 3 週後改善，但停藥 2-4 週即復發。病灶以腋下、腹部、指間為主。有慢性指間濕疹史。皮膚皺褶區常有異味。使用 fluralaner 每 3 個月投予。飲食為雞肉口味犬糧。' },
    steps: [
      { step_number: 1, step_type: 'history_collection', title: '病史採集', prompt: '這隻鬥牛犬已第三次膿皮症發作。選擇最重要的 3 項問題。', available_actions: ['確認每次抗生素治療的完整療程（劑量與天數）', '詢問是否有搔癢，膿皮症之前就癢還是感染後才癢', '詢問是否有多飲多尿、腹部膨大等 Cushing 症狀', '詢問飼料品牌', '詢問最近洗澡頻率', '詢問是否有打噴嚏'], correct_actions: ['確認每次抗生素治療的完整療程（劑量與天數）', '詢問是否有搔癢，膿皮症之前就癢還是感染後才癢', '詢問是否有多飲多尿、腹部膨大等 Cushing 症狀'], feedback_correct: '正確！反覆膿皮症最重要的問題：(1) 抗生素療程是否足夠，表淺膿皮症需治療至臨床痊癒後 7-14 天，過早停藥是「假性復發」最常見原因。(2) 搔癢先於感染 = 底層過敏（CAD/FAD），這是膿皮症復發最常見的底層原因。(3) Cushing 症候群是免疫抑制性內分泌病，使皮膚易感染且難癒合。', feedback_incorrect: '反覆膿皮症的病史核心：(1) 治療是否充分（劑量、療程），(2) 底層過敏的線索（搔癢時序），(3) 免疫抑制原因排除（Cushing/甲低）。', key_learning_points: ['反覆膿皮症第一步：確認抗生素療程是否足夠', '表淺膿皮症需治療至臨床痊癒後「額外」7-14 天', '底層病因排序：過敏 > Cushing > 甲低 > 蠕形蟎 > 原發性角化異常', '搔癢先於膿皮 = 底層過敏可能性高'] },
      { step_number: 2, step_type: 'physical_examination', title: '理學檢查', prompt: '病史補充：Cephalexin 30 mg/kg BID 每次用 3 週（未延長至症狀消失後 14 天）。搔癢在膿皮症發作前就存在（PVAS 5/10）。無 PU/PD。選擇最關鍵的理學檢查發現。', available_actions: ['腋下與腹部多處環狀脫毛伴中央結痂（epidermal collarettes）', '指間紅腫濕潤，趾間有紅棕色着色（唾液染色）', '皮膚皺褶區（面部、尾根）有濕疹與異味', '體型肥胖 BCS 7/9，腹部正常', '體溫 38.6°C', '心肺聽診正常'], correct_actions: ['腋下與腹部多處環狀脫毛伴中央結痂（epidermal collarettes）', '指間紅腫濕潤，趾間有紅棕色着色（唾液染色）', '皮膚皺褶區（面部、尾根）有濕疹與異味'], feedback_correct: '正確！Epidermal collarettes（環狀脫毛伴中央結痂或鱗屑）是表淺膿皮症的特徵性病灶。指間唾液染色（紅棕色）= 慢性舔舐 = 搔癢的客觀證據（底層過敏）。皮膚皺褶區濕疹是鬥牛犬的品種易感問題，皺褶提供溫暖潮濕環境促進細菌與酵母菌過度增生。', feedback_incorrect: '表淺膿皮症的經典病灶：epidermal collarettes、丘疹、膿疱。指間唾液染色是慢性舔舐的客觀證據。皺褶皮膚炎在短頭犬品種極為常見。', key_learning_points: ['Epidermal collarettes 是表淺膿皮症最具特徵性的病灶', '指間唾液染色（紅棕色）= 慢性舔舐的客觀證據', '英國鬥牛犬的三重皮膚風險：過敏易感 + 皺褶皮膚炎 + 蠕形蟎'] },
      { step_number: 3, step_type: 'diagnostic_plan', title: '診斷計畫', prompt: '如何系統性地找出底層原因？選擇最適當的 3 項。', available_actions: ['皮膚細胞學（多部位印片：腋下 + 指間 + 皺褶區）', '深層皮膚刮搔（多部位，排除蠕形蟎）', '細菌培養與藥敏試驗（第三次復發，篩檢 MRSP）', '血清 IgE 過敏原檢測', '皮膚切片', 'CBC'], correct_actions: ['皮膚細胞學（多部位印片：腋下 + 指間 + 皺褶區）', '深層皮膚刮搔（多部位，排除蠕形蟎）', '細菌培養與藥敏試驗（第三次復發，篩檢 MRSP）'], feedback_correct: '完美！(1) 細胞學評估繼發感染類型與嚴重度，球菌、桿菌、馬拉色菌各需不同治療。(2) 深層刮搔，鬥牛犬是蠕形蟎高風險品種，蠕形蟎本身可導致膿皮症。(3) 細菌培養+藥敏，反覆使用 cephalexin 增加 MRSP（Methicillin-Resistant Staphylococcus pseudintermedius）風險。第三次復發是進行培養的適應症。', feedback_incorrect: '反覆膿皮症的診斷三步：感染評估（細胞學）→ 蟎蟲排除（刮搔）→ 抗藥性篩檢（培養）。IgE 是確診 CAD 後用於 ASIT 選擇，不是診斷工具。', key_learning_points: ['反覆膿皮症（≥ 3 次/年）需做細菌培養篩檢 MRSP', 'MRSP 在獸醫皮膚科日益嚴重，反覆抗生素使用是主要風險因子', '鬥牛犬品種需特別排除蠕形蟎（即使懷疑過敏）', '細胞學中桿菌的出現提示深層膿皮症或特殊感染'] },
      { step_number: 4, step_type: 'result_interpretation', title: '結果判讀', prompt: '結果：\n- 細胞學：腋下球菌 > 10/OIF，指間球菌 + 馬拉色菌 3-5/OIF，皺褶區大量馬拉色菌\n- 深層刮搔：陰性（多部位）\n- 培養：Staphylococcus pseudintermedius，對 cephalexin 敏感，amoxicillin-clavulanate 敏感，oxacillin 敏感（mecA 陰性 = 非 MRSP）\n\n如何解讀並規劃下一步？', available_actions: ['非 MRSP 膿皮症 + 馬拉色菌共感染。療程不足是復發主因。需延長 cephalexin 至臨床痊癒後 14 天 + 抗馬拉色菌洗劑 + 完成治療後啟動底層過敏調查', '培養結果正常不需改變治療', '所有感染治療後就不會再復發', '應換用 fluoroquinolone 預防抗藥性'], correct_actions: ['非 MRSP 膿皮症 + 馬拉色菌共感染。療程不足是復發主因。需延長 cephalexin 至臨床痊癒後 14 天 + 抗馬拉色菌洗劑 + 完成治療後啟動底層過敏調查'], feedback_correct: '正確！好消息是非 MRSP，cephalexin 仍為有效選擇。但之前 3 週療程不足是「假性復發」的主因，表淺膿皮症需至少治療至臨床痊癒後額外 7-14 天（通常總療程 4-6 週）。馬拉色菌共感染需同步處理（chlorhexidine + miconazole 洗劑每週 2 次）。最重要的是：感染控制後必須調查底層原因，此犬搔癢先於膿皮 + CAD 好發品種 + 好發部位 = 高度懷疑 CAD。', feedback_incorrect: '療程不足是膿皮復發最常見的可修正原因。換用 fluoroquinolone 不恰當，當第一線仍有效時不應使用。感染只是冰山一角，不找底層原因就會不斷復發。', key_learning_points: ['表淺膿皮症療程：臨床痊癒後額外 7-14 天（非固定 3 週）', '非 MRSP 時應優先使用第一線抗生素（cephalexin）', 'Fluoroquinolone 為最後線用藥，不應作為常規膿皮選擇', '感染控制後必須調查底層原因，過敏是最常見的底層病因'] },
      { step_number: 5, step_type: 'treatment_plan', title: '完整治療策略', prompt: '延長療程 6 週後膿皮完全緩解。停藥 2 週後搔癢 PVAS 仍 4/10（感染已控制但搔癢持續）。如何進行底層病因調查與長期管理？', available_actions: ['感染清除後仍搔癢 = 底層過敏確認。啟動排除飲食試驗（水解蛋白飲食 8 週）排除 CAFR。若飲食試驗後仍搔癢 → 符合 Favrot 標準 → CAD 推定診斷 → oclacitinib 或 lokivetmab 長期控制。同時：皺褶區持續使用抗菌洗劑預防', '膿皮已好不需後續調查', '直接開始 ASIT', '長期使用 prednisolone 預防復發'], correct_actions: ['感染清除後仍搔癢 = 底層過敏確認。啟動排除飲食試驗（水解蛋白飲食 8 週）排除 CAFR。若飲食試驗後仍搔癢 → 符合 Favrot 標準 → CAD 推定診斷 → oclacitinib 或 lokivetmab 長期控制。同時：皺褶區持續使用抗菌洗劑預防'], feedback_correct: '完美！感染控制後殘餘搔癢 = 底層過敏確認。診斷步驟：(1) 排除飲食試驗（水解蛋白或新穎蛋白，8 週嚴格執行）排除 CAFR。(2) 飲食試驗無改善 + 符合 Favrot 標準（好發部位、品種、年齡、非食物相關）→ 推定 CAD。(3) 長期控制選擇：oclacitinib（JAK inhibitor，16 mg BID × 14 天後 SID）或 lokivetmab（anti-IL-31 mAb，每月 SC）。長期 prednisolone 不推薦（代謝副作用嚴重）。皺褶護理需終身維持。', feedback_incorrect: '膿皮復發的根本原因未找到就會持續復發。ASIT 需先確診 CAD 並做 IgE 篩檢後才能開始。Prednisolone 長期使用在犬有嚴重副作用（醫源性 Cushing）。', key_learning_points: ['感染控制後殘餘搔癢 = 底層過敏的確認', '過敏診斷順序：跳蚤排除 → 食物排除 → CAD 推定', 'Oclacitinib vs lokivetmab：前者口服便利、後者適合口服困難犬隻', '鬥牛犬的皺褶管理需終身維持，是膿皮預防的重要環節', 'MRSP 預防的抗生素管理原則（stewardship）'] },
    ],
    expert_pathway: { reasoning: '5 歲英國鬥牛犬 + 反覆膿皮 3 次/年 + 搔癢先於膿皮 + CAD 好發品種/部位。療程不足為近因，底層 CAD 為遠因。延長抗生素 + 抗馬拉色菌 → 膿皮清除 → 殘餘搔癢 → 排除飲食 → CAD 推定 → oclacitinib 長期控制。', key_decisions: ['辨識療程不足是近因', '培養排除 MRSP', '感染控制後評估殘餘搔癢', '系統性排除底層過敏原因', '選擇安全的長期控制藥物'], common_pitfalls: ['反覆使用短療程抗生素不查底層原因', '每次復發都換不同抗生素（增加 MRSP 風險）', '忽略馬拉色菌共感染', '使用 fluoroquinolone 作為第一線', '長期 prednisolone 導致醫源性 Cushing'] },
    related_nodes_for_review: ['DERM-L3-005', 'DERM-L3-001', 'DERM-L4-003', 'DERM-L5-002', 'DERM-L5-001'],
    tags: ['pyoderma', '膿皮症', 'MRSP', 'English_Bulldog', 'recurrent', 'antibiotic_stewardship', 'case_challenge'],
  },

  // --- CASE-DERM-010: Feline EGC ---
  {
    id: 'CASE-DERM-010', title: '貓上唇潰瘍合併腹部斑塊，嗜酸性肉芽腫複合症', specialty: 'DERM', cross_specialties: ['CPATH', 'IM'], required_nodes: ['DERM-L3-015', 'DERM-L2-001', 'DERM-L4-003', 'DERM-L5-001'], difficulty: 3, species: '貓',
    scenario: { signalment: '4 歲已絕育雌性家貓（DSH），體重 4.2 kg', chief_complaint: '上唇持續不癒合的潰瘍 + 腹部紅色隆起斑塊', brief_history: '約 2 個月前注意到上唇有一處不痛的潰瘍，逐漸擴大。1 個月前腹部出現紅色隆起濕潤斑塊，會舔但不太像很癢。食慾正常。室內飼養，偶爾到陽台。使用 selamectin 每月。飲食為乾糧（鮪魚口味）+ 偶爾罐頭。無其他同居動物。' },
    steps: [
      { step_number: 1, step_type: 'history_collection', title: '病史採集', prompt: '選擇最重要的 3 項問題以釐清此貓的皮膚病變。', available_actions: ['確認上唇潰瘍的位置，是否位於上唇正中或兩側', '詢問是否有搔癢或過度理毛行為', '詢問飲食內容細節（蛋白來源、零食、是否有餵人食）', '詢問最近是否搬家', '詢問貓砂品牌', '詢問是否有打噴嚏或流鼻涕'], correct_actions: ['確認上唇潰瘍的位置，是否位於上唇正中或兩側', '詢問是否有搔癢或過度理毛行為', '詢問飲食內容細節（蛋白來源、零食、是否有餵人食）'], feedback_correct: '正確！上唇潰瘍位置是鑑別的關鍵，EGC 的 indolent ulcer 典型位於上唇邊緣（唇黏膜皮膚交界處），而鱗狀細胞癌好發鼻鏡/耳尖。過度理毛提示底層搔癢/過敏。飲食細節對後續排除飲食設計必要，80% 以上 EGC 有底層過敏基礎，食物過敏尤為常見。', feedback_incorrect: 'EGC 的病史重點：病灶位置（indolent ulcer 的典型部位）、搔癢評估（底層過敏線索）、飲食（食物過敏排除）。', key_learning_points: ['EGC 三種表現：indolent ulcer（上唇）、eosinophilic plaque（腹部/大腿內側）、eosinophilic granuloma（口腔/下唇/後肢）', '80%+ EGC 有底層過敏基礎，最常見為食物過敏與環境過敏', 'Indolent ulcer 特徵：上唇邊緣、不痛、邊緣隆起、紅銅色'] },
      { step_number: 2, step_type: 'physical_examination', title: '理學檢查', prompt: '上唇潰瘍位於右側上唇邊緣。貓會過度理毛腹部。飲食為鮪魚乾糧 + 雞肉罐頭。選擇最關鍵的理學檢查發現。', available_actions: ['右側上唇邊緣一處約 1.5 cm 邊緣隆起、紅銅色、中央凹陷的潰瘍（典型 indolent ulcer）', '腹部一處約 3x4 cm 紅色、濕潤、隆起的斑塊，表面有滲出液', '腹部腹股溝有輕度對稱性脫毛', '周邊淋巴結正常', '口腔黏膜正常，無潰瘍', '體溫 38.5°C，體態正常'], correct_actions: ['右側上唇邊緣一處約 1.5 cm 邊緣隆起、紅銅色、中央凹陷的潰瘍（典型 indolent ulcer）', '腹部一處約 3x4 cm 紅色、濕潤、隆起的斑塊，表面有滲出液', '腹部腹股溝有輕度對稱性脫毛'], feedback_correct: '正確！(1) 上唇邊緣的紅銅色凹陷潰瘍是 indolent ulcer 的經典描述。(2) 腹部紅色濕潤隆起斑塊是 eosinophilic plaque 的典型表現。(3) 對稱性腹部脫毛提示過度理毛，支持底層過敏。一隻貓同時有 indolent ulcer + eosinophilic plaque = EGC complex 多型態表現，高度提示底層過敏。', feedback_incorrect: 'EGC 可在同一隻貓表現出多種型態，indolent ulcer + plaque + granuloma 的任意組合。多型態表現強烈支持底層過敏的系統性觸發。', key_learning_points: ['Indolent ulcer：上唇邊緣、紅銅色、邊緣隆起、中央凹陷、通常不痛', 'Eosinophilic plaque：腹部/大腿內側、紅色濕潤隆起、搔癢劇烈', '同一貓多型態 EGC = 系統性過敏觸發', '腹部對稱性脫毛在貓中 = 過度理毛 = 搔癢'] },
      { step_number: 3, step_type: 'diagnostic_plan', title: '診斷計畫', prompt: '臨床高度懷疑 EGC complex。你的診斷策略？選擇 3 項。', available_actions: ['皮膚細胞學（腹部 plaque 表面印片 + 上唇潰瘍邊緣印片）', '皮膚切片（上唇潰瘍邊緣）送組織病理，排除鱗狀細胞癌', 'CBC（評估外周嗜酸性球增多）', '血清 IgE 檢測', '糞便寄生蟲檢查', 'FIV/FeLV 篩檢'], correct_actions: ['皮膚細胞學（腹部 plaque 表面印片 + 上唇潰瘍邊緣印片）', '皮膚切片（上唇潰瘍邊緣）送組織病理，排除鱗狀細胞癌', 'CBC（評估外周嗜酸性球增多）'], feedback_correct: '完美！(1) 細胞學是快速評估工具，EGC 的 plaque 通常可見大量嗜酸性球（> 50% 嗜酸性球為典型）。(2) 組織病理是排除腫瘤的必要手段，上唇潰瘍的鑑別必須包括鱗狀細胞癌（SCC），尤其在單側、非典型部位或對治療反應差的情況。(3) CBC 外周嗜酸性球增多支持 EGC 但非必然（約 20-50% EGC 貓外周嗜酸性球正常）。', feedback_incorrect: '上唇潰瘍必須排除 SCC，切片是唯一可靠方法。細胞學快速確認嗜酸性球浸潤。IgE 不用於 EGC 診斷。FIV/FeLV 雖重要但不解釋 EGC 發生。', key_learning_points: ['上唇潰瘍的鑑別診斷：indolent ulcer vs SCC vs herpesvirus vs trauma', '組織病理排除腫瘤是上唇潰瘍的必要步驟', 'EGC 細胞學：大量嗜酸性球浸潤為特徵', '外周嗜酸性球增多僅見於 20-50% EGC 貓（正常不排除 EGC）'] },
      { step_number: 4, step_type: 'result_interpretation', title: '結果判讀', prompt: '結果：\n- 細胞學（plaque）：大量嗜酸性球（> 60%），少量嗜中性球，無微生物\n- 細胞學（唇潰瘍）：混合炎症細胞伴嗜酸性球\n- 組織病理（唇潰瘍）：潰瘍性病灶伴大量嗜酸性球與組織球浸潤，無腫瘤細胞\n- CBC：嗜酸性球 2,800/µL（↑，正常 < 1,500）\n\n確定診斷？', available_actions: ['EGC complex 確診（indolent ulcer + eosinophilic plaque）。組織病理排除 SCC。外周嗜酸性球增多支持。下一步需調查底層過敏原因', '可能是寄生蟲感染', '結果不確定需更多檢查', '直接終身使用 prednisolone'], correct_actions: ['EGC complex 確診（indolent ulcer + eosinophilic plaque）。組織病理排除 SCC。外周嗜酸性球增多支持。下一步需調查底層過敏原因'], feedback_correct: '正確！臨床表現 + 細胞學嗜酸性球浸潤 + 組織病理確認 = EGC 確診。SCC 已排除。外周嗜酸性球增多後續支持。關鍵的下一步：調查底層過敏，80%+ EGC 有過敏基礎，若不處理底層原因，症狀會在類固醇減量後復發。貓的 EGC 中食物過敏佔比高於犬（可達 50%+）。', feedback_incorrect: 'EGC 診斷已確立。寄生蟲需排除但 selamectin 使用中已涵蓋大部分。關鍵是 EGC 不是終點診斷，必須找底層過敏原因才能長期控制。', key_learning_points: ['EGC 確診 = 臨床型態 + 細胞學/組織病理', 'EGC 不是終點診斷，是底層過敏的皮膚表現', '貓 EGC 中食物過敏佔比高（可達 50%+）', '不處理底層原因 → 類固醇減量後必復發'] },
      { step_number: 5, step_type: 'treatment_plan', title: '治療策略', prompt: 'EGC 確診，需同時控制症狀並調查底層原因。你的治療方案？', available_actions: ['Prednisolone 2 mg/kg/day PO × 2 週後漸減（快速控制 EGC 病灶）。同時啟動排除飲食試驗（水解蛋白飲食 8 週嚴格執行）。若飲食試驗改善 → CAFR 確診；若無改善 → 考慮環境過敏 → ASIT', '僅 prednisolone 長期維持', '僅外用藥膏', '立即開始 ASIT 不做排除飲食'], correct_actions: ['Prednisolone 2 mg/kg/day PO × 2 週後漸減（快速控制 EGC 病灶）。同時啟動排除飲食試驗（水解蛋白飲食 8 週嚴格執行）。若飲食試驗改善 → CAFR 確診；若無改善 → 考慮環境過敏 → ASIT'], feedback_correct: '完美！治療策略：(1) 短期 prednisolone 控制急性 EGC 病灶，大多數 indolent ulcer 與 plaque 對類固醇反應良好。(2) 同步啟動排除飲食，貓的 EGC 食物過敏佔比高，水解蛋白飲食需 8 週嚴格執行（零零食、零人食、限零嘴）。(3) 飲食挑戰確認 CAFR 後可終身水解/新穎蛋白飲食，不再需要類固醇。(4) 若非食物相關 → 可考慮血清 IgE → ASIT。長期 prednisolone 在貓可導致糖尿病，貓對類固醇的糖尿病風險高於犬。', feedback_incorrect: '長期 prednisolone 在貓有高糖尿病風險。外用藥對 indolent ulcer 效果有限。ASIT 前應先排除食物過敏（在貓中佔比高）。', key_learning_points: ['貓 EGC 治療：短期類固醇 + 同步排除飲食試驗', '貓長期類固醇的最大風險 = 糖尿病（高於犬）', '排除飲食在貓需更嚴格，零零食零人食（貓飲食來源較犬單一但零食常被忽略）', 'CAFR 確診後飲食管理可能為終身唯一治療', 'Cyclosporine（7 mg/kg SID）可作為類固醇替代選擇'] },
    ],
    expert_pathway: { reasoning: '4 歲 DSH + 上唇 indolent ulcer + 腹部 eosinophilic plaque + 過度理毛 = EGC complex。細胞學嗜酸性球浸潤 + 組織病理排除 SCC → EGC 確診。底層過敏調查：排除飲食 8 週。Prednisolone 短期控制急性病灶。', key_decisions: ['辨識 EGC 三種型態中的兩種', '組織病理排除上唇 SCC', '認識 80%+ 有底層過敏', '啟動排除飲食（貓食物過敏佔比高）', '避免長期類固醇（貓糖尿病風險）'], common_pitfalls: ['未切片排除 SCC 而直接治療', '僅用類固醇不查底層原因', '排除飲食不夠嚴格（零食破壞試驗）', '長期 prednisolone 導致貓糖尿病', '忽略 EGC 的多型態表現'] },
    related_nodes_for_review: ['DERM-L3-015', 'DERM-L2-001', 'DERM-L4-003', 'DERM-L5-001', 'DERM-L3-001'],
    tags: ['EGC', 'indolent_ulcer', 'eosinophilic_plaque', 'feline', 'food_allergy', 'case_challenge'],
  },

  // --- CASE-DERM-011: Sebaceous Adenitis ---
  {
    id: 'CASE-DERM-011', title: '標準貴賓犬進行性脫毛與毛囊角化，脂腺炎', specialty: 'DERM', cross_specialties: ['CPATH'], required_nodes: ['DERM-L3-016', 'DERM-L2-002', 'DERM-L4-003', 'DERM-L5-003'], difficulty: 4, species: '犬',
    scenario: { signalment: '3 歲已絕育雄性標準貴賓犬，體重 28 kg', chief_complaint: '漸進性脫毛、毛幹粗糙斷裂、毛囊處可見黃色角化物附著', brief_history: '約 6 個月前飼主注意到背部中線區域毛髮開始變薄、粗糙。近 3 個月擴展至頸背、耳翼、尾部。毛幹易斷裂，根部可見黃色鱗屑樣物質緊密附著（follicular casting）。被毛呈「moth-eaten（蟲蛀）」樣不規則脫毛。輕度搔癢。無全身性症狀。已在他院治療：先以黴菌培養陰性，後以甲狀腺功能正常排除甲低。' },
    steps: [
      { step_number: 1, step_type: 'history_collection', title: '病史採集', prompt: '選擇最重要的 3 項問題以釐清此貴賓犬的脫毛原因。', available_actions: ['確認脫毛的分佈型態，是否以背部中線與軀幹為主', '詢問毛髮質地變化，是否從捲曲變粗糙直化', '詢問是否有其他品種犬（如秋田犬、Samoyed）的家族/繁殖線相關資訊', '詢問飲食品牌', '詢問洗澡產品', '詢問是否有環境變化'], correct_actions: ['確認脫毛的分佈型態，是否以背部中線與軀幹為主', '詢問毛髮質地變化，是否從捲曲變粗糙直化', '詢問是否有其他品種犬（如秋田犬、Samoyed）的家族/繁殖線相關資訊'], feedback_correct: '正確！(1) SA 的分佈以背部中線（dorsal midline）、頸背、耳翼、尾部為特徵，與甲低（對稱性軀幹脫毛）和 Cushing（腹側為主）不同。(2) 貴賓犬 SA 的毛質變化是關鍵線索，正常捲曲被毛變直、粗糙、易斷是脂腺破壞後毛囊營養不良的表現。(3) SA 有遺傳傾向，標準貴賓犬、秋田犬、Samoyed 為最高風險品種。詢問繁殖線有助判斷。', feedback_incorrect: 'SA 的病史線索：背部中線為主的分佈、毛質改變（捲→直→粗→斷）、品種易感性。黴菌和甲低已排除是正確的第一步。', key_learning_points: ['SA 最高風險品種：標準貴賓犬、秋田犬、Samoyed、Vizsla', 'SA 分佈特徵：背部中線 > 頸背 > 耳翼 > 尾部', '貴賓犬 SA 的毛質改變：捲曲 → 直化 → 粗糙 → 斷裂', 'SA 有遺傳傾向，繁殖者篩檢很重要'] },
      { step_number: 2, step_type: 'physical_examination', title: '理學檢查', prompt: '脫毛以背部中線為主，毛從捲曲變粗糙直化。同繁殖場另一隻貴賓也有類似問題。選擇最關鍵的理學檢查發現。', available_actions: ['背部中線至頸背部廣泛「moth-eaten」型不規則脫毛，殘餘毛幹粗糙黯淡', '毛囊處有緊密附著的黃色管狀角化物（follicular casting / follicular keratosis）', '耳翼邊緣毛髮稀疏伴鱗屑，耳道無異常分泌物', '腹部、四肢皮膚正常', '無搔癢反應，腋下與腹股溝無紅斑', '體態正常 BCS 5/9，精神活力正常'], correct_actions: ['背部中線至頸背部廣泛「moth-eaten」型不規則脫毛，殘餘毛幹粗糙黯淡', '毛囊處有緊密附著的黃色管狀角化物（follicular casting / follicular keratosis）', '耳翼邊緣毛髮稀疏伴鱗屑，耳道無異常分泌物'], feedback_correct: '正確！(1) Moth-eaten（蟲蛀樣）不規則脫毛是 SA 的特徵性被毛型態，不同於甲低的對稱性完全脫毛。(2) Follicular casting（毛囊角化鑄型）是 SA 最具特異性的臨床徵象，脂腺破壞後毛囊口角化異常，形成黃色管狀物緊密包裹毛幹根部。(3) 耳翼受累伴鱗屑但耳道正常（排除外耳炎）。腹部/腋下正常排除 CAD。全身狀態正常排除內分泌。', feedback_incorrect: 'Follicular casting 是 SA 最具診斷價值的臨床徵象，黃色管狀角化物緊密附著毛幹根部。Moth-eaten 脫毛型態也是 SA 的特徵。', key_learning_points: ['Follicular casting（毛囊角化鑄型）= SA 最具特異性的臨床徵象', 'Moth-eaten 脫毛 = 不規則、斑塊狀、非對稱的脫毛型態', 'SA 的鑑別要點：背部中線為主 + follicular casting + 無搔癢 + 無全身症狀', '耳翼鱗屑在 SA 常見但耳道本身正常'] },
      { step_number: 3, step_type: 'diagnostic_plan', title: '診斷計畫', prompt: '高度懷疑 SA。確診需要什麼檢查？選擇 3 項。', available_actions: ['皮膚切片（punch biopsy，取背部中線脫毛區與正常皮膚交界處）', '毛髮拔取檢查（trichography），評估毛幹結構與毛囊角化', '深層皮膚刮搔排除蠕形蟎', '血清 IgE 檢測', '甲狀腺功能再次檢測', '黴菌培養'], correct_actions: ['皮膚切片（punch biopsy，取背部中線脫毛區與正常皮膚交界處）', '毛髮拔取檢查（trichography），評估毛幹結構與毛囊角化', '深層皮膚刮搔排除蠕形蟎'], feedback_correct: '完美！(1) 皮膚切片是 SA 確診的唯一方法，特徵性組織病理：肉芽腫性炎症靶向破壞脂腺（granulomatous inflammation targeting sebaceous glands），脂腺完全消失被淋巴球與巨噬細胞取代。取切片位置：脫毛區與正常皮膚交界（可見脂腺從正常到破壞的過渡）。(2) Trichography 可見：毛幹粗糙、follicular cast 附著、不規則毛髮週期。(3) 蠕形蟎需排除，可導致類似的毛囊性脫毛。', feedback_incorrect: '皮膚切片是 SA 唯一確診方法，臨床表現雖高度提示但需組織病理確認。蠕形蟎需排除因為也可導致毛囊性脫毛伴鱗屑。甲低已排除不需重複。', key_learning_points: ['SA 確診 = 皮膚切片組織病理（唯一方法）', 'SA 組織病理特徵：肉芽腫性炎症靶向破壞脂腺', '切片位置選擇：脫毛區與正常皮膚交界最有診斷價值', 'Trichography 支持診斷但無法確診'] },
      { step_number: 4, step_type: 'result_interpretation', title: '結果判讀', prompt: '結果：\n- 組織病理：毛囊周圍肉芽腫性至淋巴球性炎症浸潤，脂腺結構完全消失並被炎症細胞取代。毛囊角化過度（follicular orthokeratosis）。殘餘毛幹結構萎縮。\n- Trichography：毛幹粗糙不規則、follicular cast 附著、多處斷裂\n- 深層刮搔：陰性\n\n確定診斷？', available_actions: ['脂腺炎（Sebaceous Adenitis）確診。肉芽腫性炎症靶向破壞脂腺是 SA 的病理學金標準。毛囊角化過度為脂腺功能喪失的結果。需開始長期管理', 'Demodex 相關的毛囊炎', '甲狀腺功能低下的皮膚表現', '僅為原發性角化異常'], correct_actions: ['脂腺炎（Sebaceous Adenitis）確診。肉芽腫性炎症靶向破壞脂腺是 SA 的病理學金標準。毛囊角化過度為脂腺功能喪失的結果。需開始長期管理'], feedback_correct: '正確！組織病理確認 SA：(1) 肉芽腫性炎症靶向脂腺（最具特異性的病理發現），脂腺被淋巴球、巨噬細胞、漿細胞取代直至完全消失。(2) 繼發性毛囊角化過度，脂腺分泌的皮脂是毛囊正常角化的必要成分，脂腺消失 → 毛囊口異常角化 → follicular casting。(3) 毛幹萎縮反映營養不良。蠕形蟎陰性排除。SA 的病因被認為是針對脂腺的免疫介導性攻擊。', feedback_incorrect: '肉芽腫性炎症「靶向」脂腺是 SA 的核心病理，這是排除其他脫毛原因的確診依據。Demodex 陰性排除。甲低已先前排除。原發性角化異常不會有脂腺破壞。', key_learning_points: ['SA 病理金標準：肉芽腫性至淋巴球性炎症靶向破壞脂腺', 'SA 的病因：免疫介導性攻擊脂腺（確切機制未明）', '脂腺消失 → 皮脂缺乏 → 毛囊角化過度 → follicular casting → 脫毛', 'SA 分為肉芽腫型（貴賓犬）與非肉芽腫型（短毛犬種）'] },
      { step_number: 5, step_type: 'treatment_plan', title: '長期管理策略', prompt: 'SA 確診。標準貴賓犬的 SA 是慢性疾病，你的長期管理方案是什麼？', available_actions: ['多模式治療：(1) Cyclosporine 5 mg/kg SID（免疫調節核心藥物）。(2) 油浴療法（baby oil / 礦物油全身浸泡 1-2 小時後洗劑清洗），每週 1 次軟化 follicular casting。(3) 含 salicylic acid / sulfur 的角質溶解洗劑每週 1-2 次。(4) 口服 omega-3/6 脂肪酸補充。(5) 4-6 週後評估反應，6 個月追蹤皮膚切片', '僅使用角質溶解洗劑', '高劑量 prednisolone 長期使用', '不需治療等待自癒'], correct_actions: ['多模式治療：(1) Cyclosporine 5 mg/kg SID（免疫調節核心藥物）。(2) 油浴療法（baby oil / 礦物油全身浸泡 1-2 小時後洗劑清洗），每週 1 次軟化 follicular casting。(3) 含 salicylic acid / sulfur 的角質溶解洗劑每週 1-2 次。(4) 口服 omega-3/6 脂肪酸補充。(5) 4-6 週後評估反應，6 個月追蹤皮膚切片'], feedback_correct: '完美！SA 需多模式長期管理：(1) Cyclosporine 是 SA 最有效的全身性免疫調節藥物，可減緩/逆轉脂腺炎症破壞。約 60-70% 犬有明顯改善。反應後可漸減至隔日或最低有效劑量。(2) 油浴是最具特色的局部治療，油浸泡軟化角化物、潤滑毛幹、暫時替代缺失的皮脂。(3) 角質溶解洗劑清除 follicular casting。(4) Omega 脂肪酸輔助皮膚屏障修復。(5) 追蹤切片評估脂腺恢復程度。SA 不會自癒且通常漸進，但適當治療可維持良好毛髮品質與生活品質。', feedback_incorrect: 'SA 不會自癒，是慢性進行性疾病。僅洗劑不足以控制免疫介導性的脂腺破壞。Prednisolone 在 SA 的效果不如 cyclosporine，且長期副作用更多。Cyclosporine + 油浴是目前最佳實證組合。', key_learning_points: ['Cyclosporine 5 mg/kg SID 是 SA 最有效的全身性治療', '油浴療法是 SA 最具特色的局部治療，軟化角化 + 替代皮脂', '治療反應需 4-8 週評估，完全反應可能需 3-6 個月', 'SA 是慢性疾病，大多數犬需長期維持治療', '繁殖者篩檢：SA 確診犬不應用於繁殖（遺傳性）'] },
    ],
    expert_pathway: { reasoning: '3 歲標準貴賓犬 + 背部中線為主的 moth-eaten 脫毛 + follicular casting + 毛質改變 → 高度懷疑 SA。皮膚切片確認肉芽腫性炎症靶向破壞脂腺。Cyclosporine + 油浴 + 角質溶解洗劑 + omega 脂肪酸的多模式長期管理。', key_decisions: ['辨識 follicular casting 為 SA 的特徵性線索', '甲低排除後考慮 SA', '選擇正確的切片位置（交界處）', 'Cyclosporine 優於 prednisolone 作為 SA 首選', '油浴療法的正確執行（浸泡 1-2 小時）'], common_pitfalls: ['將 SA 誤診為甲低（兩者均有脫毛但分佈和毛質改變不同）', '未做皮膚切片而經驗性使用類固醇', '切片位置選擇不當（完全脫毛區脂腺可能已完全消失，失去過渡期特徵）', '僅用局部治療不使用全身免疫調節', '未告知飼主 SA 為遺傳性且需長期管理'] },
    related_nodes_for_review: ['DERM-L3-016', 'DERM-L2-002', 'DERM-L4-003', 'DERM-L5-003', 'DERM-L1-001'],
    tags: ['sebaceous_adenitis', '脂腺炎', 'Standard_Poodle', 'follicular_casting', 'cyclosporine', 'oil_bath', 'case_challenge'],
  },
