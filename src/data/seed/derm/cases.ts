...
export const DERM_CASES: CaseChallenge[] = [
  ...
  // --- CASE-DERM-010: Feline EGC ---
  {
    ...
    steps: [
      ...
      { step_number: 4, step_type: 'result_interpretation', title: '結果判讀', prompt: '結果：\n- 細胞學（plaque）：大量嗜酸性球（> 60%），少量嗜中性球，無微生物\n- 細胞學（唇潰瘍）：混合炎症細胞伴嗜酸性球\n- 組織病理（唇潰瘍）：潰瘍性病灶伴大量嗜酸性球與組織球浸潤，無腫瘤細胞\n- CBC：嗜酸性球 2,800/µL（↑，正常 < 1,500）\n\n確定診斷？', available_actions: ['EGC complex 確診（indolent ulcer + eosinophilic plaque）。組織病理排除 SCC。外周嗜酸性球增多支持。下一步需調查底層過敏原因', '可能是寄生蟲感染', '結果不確定需更多檢查', '直接終身使用 prednisolone'], correct_actions: ['EGC complex 確診（indolent ulcer + eosinophilic plaque）。組織病理排除 SCC。外周嗜酸性球增多支持。下一步需調查底層過敏原因'], feedback_correct: '正確！臨床表現 + 細胞學嗜酸性球浸潤 + 組織病理確認 = EGC 確診。SCC 已排除。外周嗜酸性球增多進一步支持。關鍵的下一步：調查底層過敏——80%+ EGC 有過敏基礎，若不處理底層原因，症狀會在類固醇減量後復發。貓的 EGC 中食物過敏佔比高於犬（可達 50%+）。', feedback_incorrect: 'EGC 診斷已確立。寄生蟲需排除但 selamectin 使用中已涵蓋大部分。關鍵是 EGC 不是終點診斷——必須找底層過敏原因才能長期控制。', key_learning_points: ['EGC 確診 = 臨床型態 + 細胞學/組織病理', 'EGC 不是終點診斷——是底層過敏的皮膚表現', '貓 EGC 中食物過敏佔比高（可達 50%+）', '不處理底層原因 → 類固醇減量後必復發'] },
