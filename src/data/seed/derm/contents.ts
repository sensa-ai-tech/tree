...
### 常見併發症
- **繼發細菌性膿皮症**：最常見（約 60-70% CAD 犬），搔抓破壞皮膚屏障 → Staphylococcus pseudintermedius 感染 → 表皮圓環、丘疹、膿疱
- **繼發 Malassezia 皮膚炎**：約 30-50%，尤其好發於皮膚皺褶處（腋窩、腹股溝、趾間）
- **慢性外耳炎**：約 50-80% CAD 犬伴隨外耳炎，可能為唯一臨床表現
- **苔蘚化與色素沉著**：慢性搔抓導致表皮增生、苔蘚化（lichenification）與色素過度沉著
...
### 藥物特異性監測
- **Oclacitinib**：前 6 個月每月 CBC（偵測嗜中性球減少）；長期使用監測皮膚腫塊（乳突瘤/肥大細胞瘤風險略增）
- **Cyclosporine**：牙齦增生（約 10%）、消化道症狀（嘔吐約 25%，通常自限）、罕見弓蟲復發風險（戶外貓禁用）
- **Lokivetmab**：安全性最佳，極少副作用；監測療效持續時間是否縮短（可能產生抗藥物抗體）

## 五、預後與預後因子 (Prognosis)

CAD 為**終生性疾病**，無法治癒但可良好控制：
- **預後良好因子**：早期診斷（< 3 歲）、飼主高度配合、對 ASIT 反應良好（約 60-70% 有效）、可辨識並避免主要過敏原
- **預後不良因子**：發病年齡 < 6 月齡（常更嚴重）、多重過敏原致敏、反覆深層膿皮症、飼主順從性差
- **ASIT（過敏原免疫治療）**：唯一可能改變疾病自然史的治療，約 60-70% 犬有顯著改善，但需持續 6-12 個月才能評估療效
- **生活品質**：多數 CAD 犬在適當多模式管理下可維持良好生活品質，飼主需理解「控制而非治癒」的長期管理概念
...
const contentAFR: NodeContent = {
  ...
  node_id: 'DERM-L3-002',
  version: 1,
  summary: '食物不良反應（Adverse Food Reaction, AFR）涵蓋食物過敏（免疫介導）與食物不耐受（非免疫介導），是犬貓第三常見的過敏性皮膚病。臨床表現與犬異位性皮膚炎高度重疊，唯一確診方法為嚴格的排除飲食試驗（8 週新蛋白質或水解蛋白飲食）並進行食物激發試驗。常見過敏原為牛肉、雞肉、乳製品、小麥。部分病例同時伴有腸胃道症狀。台灣亞熱帶環境下食物過敏與環境過敏常並存，本土流行病學顯示雞肉與牛肉為最常見食物過敏原。',
  learning_objectives: [
...
### 常見併發症
- **消化道症狀**：約 10-30% AFR 犬合併慢性嘔吐/腹瀉/排氣增加（食物過敏可同時影響皮膚與消化道）
- **繼發感染**：因搔抓導致繼發性膿皮症或 Malassezia（同 CAD）
- **營養不均衡**：長期使用限制性排除飲食（尤其自製飲食）可能導致營養素缺乏
- **飼主疲勞**：排除飲食試驗需 6-8 週嚴格執行，飼主順從性是最大挑戰

### 監控計畫
| 時間點 | 監測項目 | 目標 |
|--------|---------|------|
| 排除飲食第 2 週 | 搔癢評估 + 消化道症狀 | 初步反應（部分犬 2 週即見改善） |
| 排除飲食第 6-8 週 | PVAS + 臨床評估 | 判定排除飲食成功與否 |
| 食物激發試驗 | 每次加回一種蛋白質，觀察 1-2 週 | 辨識特定致敏食物 |
| 穩定期每 6 個月 | 體重、BCS、營養評估 | 確認限制飲食營養充足 |

### 注意事項
- 排除飲食期間**禁止**零食、口味藥物、牛皮骨等非處方食物
- 水解蛋白飼料與新穎蛋白飼料效果相當（ACVD 建議均可作為首選）

## 五、預後與預後因子 (Prognosis)

AFR 的預後**整體良好**：
- **可治癒性**：與 CAD 不同，AFR 在辨識出致敏食物後可透過飲食管理達到完全控制，無需長期用藥
- **預後良好因子**：單一蛋白過敏、飼主高度配合、純犬糧餵食（易控制成分）
- **預後不良因子**：多重食物過敏、合併 CAD（約 30% CAD 犬同時有 AFR）、自由放養犬（飲食難以控制）
- **長期展望**：嚴格飲食控制下大部分犬可完全無症狀；部分犬可能隨年齡發展新的食物過敏
...
### 常見併發症
- **繼發熱點（hot spot/急性濕疹）**：嚴重搔抓 → 急性濕性皮膚炎，好發腰薦部與大腿後側
- **繼發膿皮症**：搔抓破壞表皮屏障 → Staphylococcus 感染
- **貓嗜酸性肉芽腫複合病**：FAD 是貓 EGC（嗜酸性潰瘍、嗜酸性斑塊、線性肉芽腫）最常見的觸發因素
- **心因性脫毛（貓）**：慢性搔癢 → 過度舔毛 → 對稱性脫毛，需排除 FAD
...
### 預防關鍵
- **全年不間斷**的跳蚤預防（台灣亞熱帶氣候全年有跳蚤活動）
- **同住動物全部治療**：包括貓、犬及其他哺乳動物
- **環境處理**：跳蚤卵/幼蟲存在於環境中 → 吸塵器 + 環境噴霧（含 IGR 成分）

## 五、預後與預後因子 (Prognosis)

FAD 的預後**極佳**（所有過敏性皮膚病中最佳）：
- **可預防性**：只要能維持嚴格跳蚤預防 → 幾乎完全可控制，無需長期免疫抑制藥物
- **預後良好因子**：室內飼養、飼主順從性佳、使用 isoxazoline 類新型跳蚤預防藥（效力持續 1-3 個月）
- **預後不良因子**：多犬/貓家庭（交叉感染）、戶外自由活動犬、飼主不願持續投藥、同時合併 CAD
- **台灣特殊考量**：亞熱帶氣候跳蚤全年活躍，冬季不可停藥；都市中流浪貓犬為持續感染源
...
const contentDemodicosis: NodeContent = {
  ...
  node_id: 'DERM-L3-004',
  version: 1,
  summary: '蠕形蟎症（Demodicosis）是由蠕形蟎（Demodex spp.）過度增殖引起的皮膚病，犬以 Demodex canis 為主。分為局部型與廣泛型。幼犬型（< 18 個月）多有自限性傾向，成犬型（> 4 歲新發）需排查潛在免疫抑制因素。Isoxazoline 類藥物已取代傳統 ivermectin/amitraz 成為首選治療。診斷以深層皮膚刮搔為金標準。台灣高溫多濕環境下蠕形蟎症盛行率較高，本土常見短鼻品種（西施犬、巴哥犬）為高風險族群。',
  learning_objectives: [
...
### 常見併發症
- **深層膿皮症（最嚴重）**：全身性蠕形蟎症 + 繼發 Staphylococcus → 癤腫（furunculosis）、蜂窩組織炎、敗血症風險
- **毛囊損傷性瘢痕脫毛**：嚴重病例毛囊被破壞 → 即使蟎蟲清除，毛髮可能無法完全再生
- **免疫抑制根本原因**：成犬突發蠕形蟎症為「紅旗信號」→ 需排除 Cushing、甲狀腺低下、腫瘤、免疫抑制藥物
...
### 治療反應評估
- **蟎蟲計數趨勢**：活成蟲減少 + 蟲卵消失 = 良好反應
- **治療失敗**：3 個月後仍可見活成蟲 → 排除免疫根本原因、確認用藥順從性

## 五、預後與預後因子 (Prognosis)

- **幼犬局部型**：預後**極佳** — 約 90% 自行緩解（6-18 月齡免疫成熟後），通常無需治療或僅局部治療
- **幼犬全身型**：預後**良好** — isoxazoline 類藥物（fluralaner、afoxolaner）治癒率 > 95%，較舊藥 ivermectin 更安全有效
- **成犬發病型**：預後**謹慎** — 取決於能否辨識並控制潛在免疫抑制病因
  - 找到並控制根本原因 → 預後良好
  - 原因不明或無法矯正（如晚期腫瘤）→ 預後較差，可能需要終生管理
...
### 常見併發症
- **抗藥菌感染（MRSP）**：Methicillin-resistant Staphylococcus pseudintermedius 在全球盛行率升高（部分地區達 20-40%），反覆或慢性膿皮症風險更高
- **深層膿皮症**：表層膿皮症未控制 → 進展至真皮層（癤腫 furunculosis）→ 疼痛、瘢痕、局部蜂窩組織炎
- **反覆復發**：膿皮症復發幾乎必然存在潛在原因（過敏、內分泌、免疫缺陷），單純抗生素治療不夠
...
### 抗生素使用原則
- **表層膿皮症**：優先外用治療（chlorhexidine 3-4% 洗劑/噴劑 + mupirocin/fusidic acid 局部），中度以上才系統性抗生素
- **深層膿皮症**：必須系統性抗生素 ≥ 4-6 週，加上外用輔助
- **培養送檢時機**：深層膿皮症、反覆膿皮症（≥ 3 次/年）、先前抗生素治療無效
- **MRSP 處理**：依藥敏結果選藥（常用 chloramphenicol、doxycycline、rifampicin 合併治療）

## 五、預後與預後因子 (Prognosis)

- **單次表層膿皮症**：預後**極佳** — 適當抗生素 3-4 週通常可完全治癒
- **反覆膿皮症**：預後取決於**根本原因**是否被辨識並控制
  - 根本原因可控（如 CAD + 適當管理）→ 預後良好，復發頻率顯著降低
  - 根本原因難控（如嚴重多重過敏、免疫缺陷）→ 可能需要間歇性或低劑量維持抗生素
- **深層膿皮症**：預後**謹慎** — 療程長（4-8 週）、疼痛明顯、可能留下瘢痕；德國牧羊犬深層膿皮症為品種特異性問題，常需反覆治療
- **MRSP 感染**：預後仍可良好，但治療選擇受限、療程較長、須更嚴格執行感染控制（避免傳播給其他動物或免疫低下人）
...
### 常見併發症
- **脂漏性皮膚炎惡化**：Malassezia 利用皮膚表面脂質增殖 → 刺激更多皮脂分泌 → 惡性循環
- **慢性苔蘚化**：長期 Malassezia + 搔癢 → 表皮增厚、色素沉著（尤其腹側、腋窩）
- **繼發細菌感染**：Malassezia + Staphylococcus 常同時存在，混合感染需同時治療
- **外耳炎成分**：Malassezia 是犬外耳炎最常見的微生物之一（約 50% 以上外耳炎有 Malassezia）

### 監控計畫
| 時間點 | 監測項目 | 目標 |
|--------|---------|------|
| 治療第 2 週 | 搔癢評估 + 氣味改善 | 初步療效（Malassezia 減少 → 氣味快速改善） |
| 治療第 4 週 | 皮膚細胞學（透明膠帶/直接壓片） | 確認 Malassezia 數量顯著減少（< 2 個/油鏡視野） |
| 穩定後每 2-3 月 | 皮膚細胞學 + 臨床評估 | 早期偵測復發 |
| 根本原因控制中 | 依原發病監控計畫 | CAD、甲狀腺低下等需同步管理 |

### 治療反應指標
- **氣味消退**：最早期且最明顯的改善指標（通常 1-2 週內）
- **細胞學計數**：治療前 vs 後的 Malassezia 數量/油鏡視野比較

## 五、預後與預後因子 (Prognosis)

- **根本原因可控**：預後**良好** — Malassezia 本身容易用外用/口服抗黴菌藥控制
- **復發性極高**：Malassezia 是犬皮膚正常共生菌 → 若根本原因（CAD、脂漏症、皮膚皺褶）未解決，停藥後幾乎必然復發
- **品種好發**：西高地白梗、巴吉度獵犬、可卡犬、鬥牛犬 — 這些品種皮脂腺活躍或皮膚皺褶多，需終生間歇性抗黴菌維持治療
- **長期管理策略**：每週 1-2 次含 chlorhexidine + miconazole/ketoconazole 的藥浴可有效預防復發，成本低且副作用少
...
const contentOtitis: NodeContent = {
  ...
  node_id: 'DERM-L3-007',
  version: 1,
  summary: '外耳炎是犬貓最常見的耳科疾病，影響約 10-20% 的犬和 2-6% 的貓。理解外耳炎的關鍵在於「PSPP」分類系統——原發因素（Primary）、繼發因素（Secondary）、易感因素（Predisposing）與持續因素（Perpetuating）。過敏性皮膚病是犬外耳炎最常見的原發因素（占 43-75%）。耳道細胞學是不可或缺的診斷工具，用於鑑定感染微生物（球菌、桿菌、馬拉色菌）並指導治療。慢性外耳炎可進展為中耳炎，需要更積極的管理。台灣亞熱帶高溫多濕氣候顯著增加外耳炎盛行率，尤其垂耳犬種在本土環境下復發率更高。',
  learning_objectives: [
...
### 常見併發症
- **中耳炎延伸**：反覆外耳炎 → 鼓膜破裂 → 中耳炎（約 50% 慢性外耳炎合併中耳炎）→ 前庭症狀、Horner syndrome、顏面神經麻痺
- **耳道增生狹窄**：慢性發炎 → 耳道上皮增生、纖維化、骨化 → 不可逆狹窄 → 最終需 TECA-LBO（全耳道切除 + 外側鼓泡切開）
- **耳血腫**：劇烈搖頭/搔耳 → 耳殼內血管破裂 → 耳翼腫脹 → 需手術引流
- **抗藥菌感染**：反覆使用耳用抗生素 → Pseudomonas aeruginosa 或 MRSP 耐藥菌
...
### 治療反應評估
- 分泌物量與性質變化（膿性→漿液性→乾燥 = 改善）
- 耳道腫脹消退程度（可見鼓膜 = 良好指標）
- 細胞學：細菌/Malassezia 計數歸零

## 五、預後與預後因子 (Prognosis)

- **急性外耳炎（首次發作）**：預後**極佳** — 適當局部治療 2-4 週通常可完全治癒
- **反覆性外耳炎**：預後取決於**根本原因控制**
  - 過敏性（CAD/AFR）為原因 → 控制過敏 = 控制外耳炎
  - 結構性（垂耳犬、耳道狹窄犬如沙皮犬）→ 可能需要終生間歇性耳道清潔與維持
- **慢性增生性外耳炎**：預後**謹慎至差** — 耳道不可逆狹窄時內科治療效果有限，可能需 TECA-LBO（手術治癒率高但不可逆）
- **品種易感性**：可卡犬、拉布拉多、巴吉度獵犬（垂耳+耳道狹窄+皮脂分泌旺盛）為最高風險品種
...
### 常見併發症
- **人畜共通感染（Zoonosis）**：Microsporum canis 可感染人類（尤其兒童、免疫低下者），接觸感染犬貓後出現環形紅斑（ringworm）。**飼主衛教為治療中不可省略的環節**
- **環境汙染**：癬菌孢子可在環境中存活 12-18 個月，地毯、沙發、梳子為主要汙染源
- **Kerion（肉芽腫型）**：宿主過度免疫反應 → 局部腫脹、化膿、疼痛結節，易誤診為細菌性膿皮症或腫瘤
- **貓多貓家庭傳播**：M. canis 在貓群中高度傳染，一隻確診 → 全群視為暴露，需全群治療
...
| 此後每 2 週 | 黴菌培養 | 追蹤至**連續 2-3 次培養陰性**才停藥 |
| 停藥後 1 個月 | 黴菌培養 | 確認無復發 |
| 全程 | 同住人員皮膚狀態 | 監測人畜共通感染 |

### 停藥標準（關鍵）
- **不能僅靠臨床外觀判斷治癒**：毛髮再生不等於黴菌清除
- **金標準**：連續 2-3 次黴菌培養陰性（每次間隔 1-2 週）
- 過早停藥是復發與持續環境汙染的主要原因

## 五、預後與預後因子 (Prognosis)

- **免疫功能正常犬貓**：預後**良好** — 即使不治療，多數免疫健全動物在 1-4 個月內可自行清除；但治療可加速痊癒並減少傳播
- **多貓家庭/繁殖場**：預後**謹慎** — 環境除汙困難，根除可能需數月系統性治療 + 環境處理 + 全群篩檢
- **免疫低下動物**：預後**較差** — FIV/FeLV 陽性貓、幼齡（< 4 月齡）或使用免疫抑制藥物者可能出現廣泛性頑固感染
- **波斯貓特殊考量**：波斯貓/長毛貓對 M. canis 感受性更高，可能為無症狀帶菌者（asymptomatic carrier），成為持續感染源
- **人畜共通風險**：所有確診病例應告知飼主傳染風險，尤其家中有幼兒或免疫低下成員時
...
### 常見併發症
- **人畜共通感染**：Sarcoptes scabiei var. canis 可暫時性感染人類（「假性疥瘡」），出現搔癢性丘疹（主要在與犬接觸的手臂/軀幹），但蟎蟲無法在人類完成生活史，犬隻治癒後人類症狀自行消退
- **繼發膿皮症**：劇烈搔抓 → 表皮破損 → Staphylococcus 感染
- **耳廓邊緣結痂性皮膚炎**：疥蟎症的特徵性病灶，pinnal-pedal reflex（耳廓-後腿反射）陽性率約 75-90%
- **淋巴結腫大**：反應性淋巴結腫大常見，為免疫反應而非轉移

### 監控計畫
| 時間點 | 監測項目 | 目標 |
|--------|---------|------|
| 治療後 2 週 | 搔癢程度評估 | 搔癢應在 2-4 週內顯著緩解 |
| 治療後 4 週 | 臨床評估 + 皮膚刮搔（必要時） | 確認蟎蟲清除 |
| 治療後 6-8 週 | 追蹤訪視 | 確認痊癒、無復發 |

### 注意事項
- 搔癢在蟎蟲死亡後仍可持續 2-4 週（殘留過敏反應），不代表治療失敗
- 同居犬必須同時治療，即使無症狀（可能為潛伏期或低蟲量感染）

## 五、預後與預後因子 (Prognosis)

疥蟎症的預後**極佳**，是所有搔癢性皮膚病中最容易治癒的之一：
- **Isoxazoline 單劑治癒率 > 95%**：fluralaner 單次口服通常即可清除所有蟎蟲，是目前最便捷有效的治療
- **預後良好因子**：單犬家庭、飼主配合度高、無免疫抑制、早期診斷
- **預後不良因子**：多犬家庭未全群治療（持續交叉感染）、犬舍/收容所環境（高密度飼養）
- **治癒後不產生持久免疫**：痊癒犬可再次感染，與流浪犬或疥蟎犬接觸仍有風險
- **診斷延遲的代價**：疥蟎症常被誤診為過敏（搔癢 + 皮膚刮搔假陰性率高達 50%），延遲診斷數月期間的不必要免疫抑制治療反而可能加重蟎蟲增殖
...
const contentPemphigus: NodeContent = {
  ...
  node_id: 'DERM-L3-010',
  version: 1,
  summary: '天疱瘡（Pemphigus）是一組自體免疫性水泡性皮膚病，以落葉型天疱瘡（Pemphigus Foliaceus, PF）在犬貓中最為常見。其病理機轉為自體抗體（主要針對 desmoglein-1）攻擊表皮細胞間的橋粒（desmosome），導致棘層松解（acantholysis）與表皮內膿皰形成。臨床特徵為面部（鼻樑、耳翼）對稱性膿皰、結痂與脫屑。皮膚細胞學見棘層松解細胞（acantholytic cells）為關鍵線索，確診需皮膚切片病理學。治療以免疫抑制為核心，長期管理挑戰性高。台灣秋田犬飼養量大使犬 PF 病例相對可見，而貓 PF 須注意 methimazole 觸發型（台灣甲亢貓使用 methimazole 比例高）。',
  learning_objectives: [
    '描述落葉型天疱瘡的棘層松解病理機轉',
    '辨識 PF 的典型臨床分佈與病灶特徵',
    '正確判讀皮膚細胞學中的棘層松解細胞',
...
### 免疫抑制治療併發症（最主要風險來源）
- **繼發感染**：長期免疫抑制 → 細菌性膿皮症（Staphylococcus）、蠕形蟎症復發、真菌感染；是天疱瘡治療中最常見的致死原因
- **醫源性 Cushing**：長期高劑量 Prednisolone → 多飲多尿、腹部膨脹、皮膚萎縮、肌肉萎縮、肝腫大
- **消化道潰瘍**：類固醇 + NSAID 禁止併用；高劑量類固醇本身即有 GI 出血風險
- **骨髓抑制（Azathioprine）**：約 5-15% 犬出現嗜中性球減少或血小板減少，需定期 CBC 監測
- **肝毒性**：Azathioprine 偶發肝損傷；Cyclosporine 可致肝酶升高
...
### 治療反應評估
- **完全緩解**：無新病灶、舊病灶癒合 → 開始緩慢減藥
- **部分緩解**：病灶減少但未完全消退 → 維持當前劑量或加入二線藥物
- **治療失敗**：4-6 週誘導期無改善 → 重新確認診斷、考慮更換免疫抑制方案

## 五、預後與預後因子 (Prognosis)

- **天疱瘡落葉型（PF）**：預後**謹慎至良好** — 約 50% 犬可達長期緩解（低劑量維持治療或停藥）；約 20-30% 需終生免疫抑制；約 10-15% 因治療副作用（繼發感染）而死亡或安樂死
- **天疱瘡尋常型（PV）**：預後**差** — 口腔+皮膚嚴重潰瘍，對治療反應較 PF 差，常需積極多藥免疫抑制
- **藥物誘發型**：預後**良好** — 停用致敏藥物（如 methimazole）後多數可自行緩解或僅需短期免疫抑制
- **預後良好因子**：早期診斷（< 3 個月病程）、對初始 prednisolone 反應良好、無嚴重繼發感染
- **預後不良因子**：診斷延遲（長期誤治為膿皮症）、多處黏膜受累（PV）、合併糖尿病（類固醇加重）、年老犬（免疫抑制耐受性差）
- **飼主溝通重點**：天疱瘡為需要終生管理的慢性病，治療目標是「最低藥物劑量下的疾病控制」而非「治癒」
...
| 犬 PF 停藥標準與復發預測因子 | 何時可嘗試停藥尚無共識，anti-Dsg1 抗體定量可能有助預測 | Level IV |`,
  ...
  disease_data: {
    signalment: '犬：中年犬最常見（中位年齡 4-6 歲），品種偏好：秋田犬、鬆獅犬、臘腸犬、紐芬蘭犬、英國鬥牛犬。貓：無明顯品種偏好，中老年貓。無性別差異。台灣秋田犬飼養量大，PF 病例相對可見；貓 PF 則需注意 methimazole 觸發型——台灣甲亢貓使用 methimazole 比例高。',
    etiology: '自體免疫疾病：免疫耐受失敗 → 產生抗 desmoglein-1（PF）或抗 desmoglein-3（PV）的 IgG 自體抗體。觸發因素可能包括：藥物（methimazole、cephalexin）、紫外線暴露、慢性皮膚病。多數為特發性。台灣日照充足（UV 暴露強），可能為部分鼻部 PF 的加重因素。',
...
皮膚科疾病佔小動物門診量的 20-30%，是一般臨床最常遇到的問題之一。系統性掌握皮膚科知識對於：
- **鑑別診斷效率**：皮膚病的臨床表現高度重疊（搔癢、脫毛、丘疹），需要有結構化的診斷思路才能避免經驗性用藥
- **減少不必要的轉診**：基層獸醫掌握基礎皮膚科技能（刮搔、細胞學、Wood's lamp）後，可處理 70% 以上的常見皮膚問題
- **長期管理能力**：異位性皮膚炎等慢性皮膚病需要終生管理，飼主衛教與治療順從性是預後的關鍵
- **跨專科整合**：許多全身性疾病（甲狀腺低下、Cushing、食物不良反應）會以皮膚症狀為首發表現
...
理解皮膚解剖是所有皮膚科診斷與治療的基石：
- **皮膚厚度差異影響用藥**：犬表皮僅 3-5 層細胞（人類 15-20 層），經皮吸收藥物的滲透率更高，外用類固醇更易導致皮膚萎縮
- **被毛密度決定病灶分佈**：犬貓的複合毛囊結構（一個毛孔含多根毛）使得毛囊炎（folliculitis）表現與人類不同——犬的膿皮症以「表皮圓環」（epidermal collarette）為特徵性病灶
- **皮脂腺功能影響診斷**：犬的皮脂腺分佈廣泛，脂漏症（seborrhea）是許多原發病（如甲狀腺低下、Malassezia）的共同表現
- **指間結構特殊性**：犬的趾間皮膚（interdigital skin）解剖獨特，易形成指間囊腫（interdigital cyst/furuncle），與深層毛囊破裂相關
- **耳道解剖**：犬貓的 L 型外耳道（垂直段 + 水平段）使得外耳炎治療需特別注意藥物投遞到水平段
...
皮膚免疫學直接解釋現代皮膚科藥物的作用機制與治療策略選擇：
- **IL-31 與搔癢的關聯**：IL-31 是犬異位性皮膚炎（CAD）最主要的致癢細胞因子，Lokivetmab（anti-IL-31 單株抗體）的成功正是基於此發現。理解 Th2 免疫偏移→IL-31 釋放→神經元活化的路徑，才能理解為何 Lokivetmab 能快速止癢但不影響感染免疫
- **JAK-STAT 訊號通路**：Oclacitinib 透過抑制 JAK1 同時阻斷 IL-31（止癢）與 IL-4/IL-13（減少 Th2 發炎），解釋其「多靶點」療效；但也因抑制 JAK 涉及的其他細胞激素信號，帶來感染風險增加的副作用
- **Type I 過敏 vs Type IV 過敏**：犬 AD 急性期以 Type I（IgE/肥大細胞/組胺）為主，慢性期轉變為 Type IV（T 細胞介導），這解釋了為何抗組織胺對犬 AD 效果有限（約 15-30%），而免疫調節劑（cyclosporine）對慢性病例更有效
- **Langerhans 細胞的門戶角色**：表皮 Langerhans 細胞是皮膚首道免疫哨兵，其活化程度影響過敏原致敏效率，也是過敏原免疫療法（ASIT）的主要標靶
...
- **為何不同過敏病表現不同**：犬異位性皮膚炎（CAD）以 Type I + Th2 偏移為主（全身性搔癢）、食物不良反應（AFR）涉及 Type I 與 Type IV 混合（搔癢 + 消化道症狀）、跳蚤過敏（FAD）為 Type I + Type IV（局部搔癢，以腰薦部為主）
- **經皮致敏理論的臨床啟示**：Filaggrin 缺陷→皮膚屏障功能不全→環境過敏原更易穿透→致敏。這直接支持「屏障修復」作為 CAD 多模式治療的基礎策略（保濕、必需脂肪酸、ceramide 噴劑）
- **治療標靶的選擇邏輯**：
  - 急性搔癢 → 抗 IL-31（Lokivetmab）或 JAK inhibitor（Oclacitinib）
  - 慢性發炎 → 免疫調節（Cyclosporine）或環境過敏原免疫療法（ASIT）
  - 屏障修復 → 外用保濕劑 + 必需脂肪酸
  - 感染控制 → 抗菌/抗黴（針對繼發 Staphylococcus/Malassezia）
- **過敏閾值概念**：犬可同時對多種過敏原敏感（環境+食物+跳蚤），當過敏原負荷總和超過個體閾值才出現臨床症狀。這解釋了為何「多模式管理」（同時控制跳蚤、食物、環境）比單一介入更有效
...
角化異常機轉的理解對於鑑別原發性與繼發性脂漏症至關重要：
- **原發性 vs 繼發性脂漏症**：原發性角化缺陷（如美國可卡犬的原發性脂漏症）需要終生外用角質溶解/角質調節治療；繼發性脂漏（如甲狀腺低下、Cushing）需找出並治療根本原因，外用治療僅為輔助
- **外用治療的選擇依據**：
  - 油性脂漏（seborrhea oleosa）→ 含硫（sulfur）、焦油（tar）、selenium sulfide 的洗劑，去脂去屑
  - 乾性脂漏（seborrhea sicca）→ 含保濕成分（oatmeal、phytosphingosine）的洗劑，避免過度去脂
  - 角化過度（hyperkeratosis）→ 含 salicylic acid（角質溶解劑）的洗劑
- **品種好發模式**：
  - 西高地白梗：Malassezia 相關脂漏（角化異常提供酵母菌生長的脂質基質）
  - 英國鬥牛犬/沙皮犬：皺褶處角化異常 + 繼發細菌/酵母菌感染
  - 西伯利亞哈士奇：鋅反應性皮膚病（zinc-responsive dermatosis）→ 表皮角質化異常特殊型
- **內分泌關聯**：甲狀腺低下導致表皮更新速度減慢、皮脂腺萎縮→乾性脂漏+非搔癢性對稱性脫毛，是最常見的「脂漏症外觀需排除內分泌病」的情境
...
理解自體免疫機轉直接影響診斷策略與免疫抑制藥物的選擇：
- **診斷必需組織病理確認**：自體免疫性皮膚病（天疱瘡、類天疱瘡、DLE）無法僅憑臨床外觀確診，必須取皮膚切片做組織病理學 ± 免疫組織化學/免疫螢光染色。選擇取樣位置很關鍵——應取早期完整水疱（vesicle）或膿疱（pustule）邊緣，而非潰瘍病灶
- **免疫抑制治療的階梯策略**：
  - 一線：Prednisolone（免疫抑制劑量 2-4 mg/kg/day）→ 4-6 週控制後逐漸減量至最低維持劑量
  - 二線（steroid-sparing agent）：Azathioprine（犬）或 Chlorambucil（貓，因 azathioprine 在貓致命性骨髓抑制風險高）
  - 三線：Mycophenolate mofetil、Cyclosporine（部分病例）
- **藥物觸發需警覺**：Methimazole 是貓天疱瘡的已知觸發因素——治療甲狀腺亢進的貓出現面部/耳廓膿疱時，應高度懷疑藥物誘發天疱瘡（drug-triggered pemphigus foliaceus），需停藥並切片確認
- **預後差異大**：天疱瘡落葉型（PF）預後尚可（約 50% 可達長期緩解），但天疱瘡尋常型（PV）預後差（口腔+皮膚嚴重潰瘍），常需積極多藥免疫抑制
- **避免過度免疫抑制**：長期高劑量免疫抑制最常見的致死併發症是繼發感染（尤其是蠕形蟎症復發、深層膿皮症）；定期監測 CBC + 尿培養是必要的
