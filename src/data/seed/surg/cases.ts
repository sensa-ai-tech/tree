import type { CaseChallenge } from '@/types/case';

export const SURG_CASES: CaseChallenge[] = [
  {
    id: 'CASE-SURG-001',
    title: '大丹犬急性腹部膨脹 — 胃擴張扭轉',
    specialty: 'SURG',
    cross_specialties: ['IM', 'ECC'],
    required_nodes: ['SURG-L3-001', 'SURG-L4-001', 'SURG-L5-001'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '6 歲已絕育雄性大丹犬，體重 58 kg',
      chief_complaint: '晚餐後約 2 小時突發不安、反覆作嘔但無法嘔出、腹部急速膨脹',
      brief_history: '飼主報告晚餐為一日一次大量餵食乾糧，進食後約 30 分鐘在院子裡跑跳。約 1.5 小時後開始出現焦躁不安、多次嘗試嘔吐但僅有少量泡沫、腹部明顯膨脹。無先前腹部手術史，過去健康無特殊。定期驅蟲與疫苗。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集與初步評估',
        prompt: '這隻大丹犬被緊急送入急診。你需要進一步確認哪些資訊？請選擇最重要的 3 項。',
        available_actions: [
          '確認嘔吐是否為非生產性（retching without vomiting）',
          '詢問腹部膨脹的發展速度（急性 vs 漸進性）',
          '詢問過去是否有類似發作或 GDV 病史',
          '詢問飼料品牌與成分',
          '詢問最近是否有疫苗接種',
          '詢問飲水量是否改變',
        ],
        correct_actions: [
          '確認嘔吐是否為非生產性（retching without vomiting）',
          '詢問腹部膨脹的發展速度（急性 vs 漸進性）',
          '詢問過去是否有類似發作或 GDV 病史',
        ],
        feedback_correct: '非生產性嘔吐（retching without vomiting）是 GDV 最典型的主訴。急性腹部膨脹在進食後 2-4 小時發生是典型的 GDV 時間框架。GDV 病史對治療決策重要——曾發生 GDV 未做 gastropexy 的犬復發率極高。',
        feedback_incorrect: 'GDV 急診中最關鍵的病史資訊是嘔吐型態（非生產性為特徵）、腹部膨脹速度（急性提示 GDV）與過去是否有類似發作。飼料品牌和疫苗史在此刻不是優先資訊。',
        key_learning_points: [
          '非生產性嘔吐（retching without vomiting）是 GDV 的最典型臨床表現',
          'GDV 通常在進食後 2-4 小時內發生',
          '大丹犬是 GDV 最高風險品種，一生發生率約 42%',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '補充病史：嘔吐確為非生產性，腹部在 1 小時內急速膨脹，無先前類似發作。進行理學檢查，請選出最重要的 3 項發現。',
        available_actions: [
          '腹部明顯膨脹且緊繃，左側叩診呈鼓音（tympanic）',
          '心搏過速（HR 180 bpm），股動脈脈搏微弱',
          '黏膜蒼白偏灰，CRT > 3 秒',
          '體溫 38.0°C',
          '腸音減少',
          '雙側後肢反射正常',
        ],
        correct_actions: [
          '腹部明顯膨脹且緊繃，左側叩診呈鼓音（tympanic）',
          '心搏過速（HR 180 bpm），股動脈脈搏微弱',
          '黏膜蒼白偏灰，CRT > 3 秒',
        ],
        feedback_correct: '腹部鼓音提示胃內大量氣體積聚。心搏過速合併弱脈與蒼白黏膜為分配性/阻塞性休克的典型表現——擴張的胃壓迫後腔靜脈導致靜脈回流減少，心輸出量下降。這三項發現高度支持 GDV 合併心血管衰竭。',
        feedback_incorrect: '在 GDV 急診中，最關鍵的理學檢查發現是腹部膨脹（鼓音）加上休克徵候（心搏過速、弱脈、蒼白黏膜、CRT 延長）。這些代表胃擴張導致的血流動力學障礙，需要立即處置。',
        key_learning_points: [
          '腹部叩診鼓音提示胃內大量氣體，為 GDV 的重要理學檢查發現',
          '心搏過速 + 弱脈 + CRT 延長 = 血流動力學不穩定（休克）',
          'GDV 造成休克的機制：擴張胃壓迫後腔靜脈 → 靜脈回流減少 → 心輸出量下降',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '急診診斷與穩定化',
        prompt: '初步高度懷疑 GDV。你的下一步行動是什麼？請選擇最適當的 3 項。',
        available_actions: [
          '立即建立兩條大口徑靜脈導管，開始 shock dose 晶體液輸液',
          '右側臥位腹部 X 光確認 GDV',
          '抽血檢驗：lactate、電解質、CBC、凝血功能',
          '立即送手術室麻醉開腹',
          '安排腹部超音波詳細評估',
          '先觀察 30 分鐘看是否自行改善',
        ],
        correct_actions: [
          '立即建立兩條大口徑靜脈導管，開始 shock dose 晶體液輸液',
          '右側臥位腹部 X 光確認 GDV',
          '抽血檢驗：lactate、電解質、CBC、凝血功能',
        ],
        feedback_correct: '積極輸液復甦是最優先的行動——在不穩定休克狀態下直接麻醉風險極高。右側臥 X 光可快速確認 GDV（「雙氣泡」或 Popeye arm sign）。Lactate 是最重要的預後指標，電解質（尤其低血鉀）影響麻醉安全性。',
        feedback_incorrect: 'GDV 急診的核心原則是「先穩定再手術」。直接送手術室或觀察等待都不恰當。需要同時進行輸液復甦、影像確診與血液檢查。腹部超音波在此情境不如 X 光快速且特異。',
        key_learning_points: [
          'GDV 急診原則：先穩定（輸液、減壓）再手術',
          '右側臥位 X 光是確診 GDV 的標準影像學方法',
          'Lactate 是 GDV 最重要的預後生物標記物',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '檢查結果判讀',
        prompt: '檢查結果如下：\n\n- X 光：右側臥位可見典型「雙氣泡」徵象，胃極度擴張，幽門軟組織帶將胃分隔為兩個充氣腔室\n- Lactate：4.8 mmol/L\n- 電解質：K+ 3.0 mEq/L（偏低），Na+ 正常\n- CBC：PCV 55%（偏高，脫水），WBC 正常\n- 凝血：PT/aPTT 正常\n\n嘗試口胃管通過但遇到阻力無法通過。\n\n如何解讀這些結果？',
        available_actions: [
          '確認 GDV 診斷。Lactate 4.8 mmol/L 尚在可接受範圍，預後相對較佳。胃管無法通過確認有扭轉。需矯正低血鉀後進行手術。',
          '可能只是單純胃擴張（GD），因為 lactate 不高。先嘗試右側腹壁 trocar 減壓觀察。',
          'Lactate 正常，不需要緊急手術。可以先保守治療。',
          'X 光無法區分 GDV 與 GD，需要 CT 確認。',
        ],
        correct_actions: [
          '確認 GDV 診斷。Lactate 4.8 mmol/L 尚在可接受範圍，預後相對較佳。胃管無法通過確認有扭轉。需矯正低血鉀後進行手術。',
        ],
        feedback_correct: '完美！X 光「雙氣泡」+ 胃管無法通過 = 確認 GDV。Lactate 4.8 mmol/L < 6.0，胃壁壞死風險相對較低，預後較佳。低血鉀（3.0）需在麻醉前矯正，因為低鉀增加心律不整風險。PCV 偏高反映脫水（脾臟淤血也可能貢獻）。',
        feedback_incorrect: '「雙氣泡」徵象是 GDV 的特徵性 X 光表現，加上胃管無法通過，已可確診。Lactate 4.8 雖然 < 6.0（較佳預後），但 4.8 仍然異常，絕不代表「正常」或可以保守治療。GDV 必須手術處理。',
        key_learning_points: [
          '「雙氣泡」/Popeye arm sign 是 GDV 的特徵性 X 光表現',
          '胃管無法通過是 GDV 與單純 GD 的重要鑑別點',
          'Lactate < 6.0 mmol/L 預後相對較佳，但仍需手術',
          '麻醉前必須矯正低血鉀以降低心律不整風險',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術計畫',
        prompt: '輸液復甦後血壓趨於穩定（MAP 65 mmHg），已補充 KCl。準備進入手術室。手術步驟應如何安排？',
        available_actions: [
          '正中開腹 → 胃 derotation → 評估胃壁與脾臟活性 → 壞死組織切除（如有）→ 右側 incisional gastropexy',
          '正中開腹 → 直接進行 gastropexy → 再進行 derotation',
          '左肋弓旁切口 → 胃切開引流 → gastropexy',
          '正中開腹 → derotation → 不做 gastropexy（術後觀察即可）',
        ],
        correct_actions: [
          '正中開腹 → 胃 derotation → 評估胃壁與脾臟活性 → 壞死組織切除（如有）→ 右側 incisional gastropexy',
        ],
        feedback_correct: '正確的手術順序：1) 正中開腹提供最佳視野；2) 先 derotation 恢復胃的正常位置（幽門推回右側）；3) 評估胃壁（特別是大彎側）與脾臟的血流與活性——灰黑色、無出血或蠕動的區域提示壞死；4) 壞死區域切除（partial gastrectomy / splenectomy）；5) 最後進行 incisional gastropexy 防止復發。',
        feedback_incorrect: '必須先 derotation 才能 gastropexy（因為胃在錯誤位置無法固定）。不做 gastropexy 的復發率高達 54-80%。正中開腹是標準入路，提供雙側腹腔完整視野。胃壁與脾臟活性評估是決定是否需要切除的關鍵步驟。',
        key_learning_points: [
          'GDV 手術標準順序：derotation → 活性評估 → 必要時切除 → gastropexy',
          '胃壁壞死評估：顏色（灰黑色 = 壞死）、漿膜面出血、切開時是否出血',
          '右側 incisional gastropexy 是預防復發的標準方式',
          '脾臟必須同時評估——脾臟扭轉或嚴重淤血需切除',
        ],
      },
      {
        step_number: 6,
        step_type: 'treatment_plan',
        title: '術中發現與處置',
        prompt: '術中發現：胃順時針扭轉約 270 度，成功 derotation。胃大彎側有一約 5x3 cm 區域呈暗紫色但切開漿膜後仍有滲血。脾臟輕度淤血但無扭轉，恢復正常位置後顏色改善。你的決定是？',
        available_actions: [
          '該暗紫區域有滲血表示尚有血流，可保留觀察。脾臟改善不需切除。完成 incisional gastropexy。',
          '暗紫區域必須立即切除（partial gastrectomy），脾臟也應預防性切除。',
          '暗紫區域保留，但脾臟應切除以預防未來腫瘤。',
          '暗紫區域注射 fluorescein 評估血流後再決定。不做 gastropexy 觀察復原。',
        ],
        correct_actions: [
          '該暗紫區域有滲血表示尚有血流，可保留觀察。脾臟改善不需切除。完成 incisional gastropexy。',
        ],
        feedback_correct: '術中胃壁活性評估的關鍵指標是漿膜面切開後是否出血。暗紫色但仍有滲血表示組織尚有血流灌注，可保留。完全灰黑色且無出血才需切除。脾臟淤血在 derotation 後改善，無需切除。Gastropexy 必須完成。',
        feedback_incorrect: '術中判斷胃壁活性的最實用方法是漿膜切開看是否出血。有滲血 = 有血流 = 可保留。不需要對有血流的組織進行 partial gastrectomy。脾臟在 derotation 後恢復正常代表淤血是可逆的。但 gastropexy 絕對不可省略。',
        key_learning_points: [
          '胃壁活性評估：漿膜切開後有滲血 = 可保留，完全無出血/灰黑色 = 壞死需切除',
          '脾臟淤血如果在 derotation 後恢復正常顏色，通常不需切除',
          '不確定的胃壁區域可術中 invagination 或術後密切監測',
          'Gastropexy 永遠是 GDV 手術的最後必要步驟',
        ],
      },
      {
        step_number: 7,
        step_type: 'case_summary',
        title: '術後管理與飼主衛教',
        prompt: '手術順利完成：derotation + incisional gastropexy，未需切除胃壁或脾臟。請選擇最適當的術後管理計畫。',
        available_actions: [
          'ICU 住院至少 48-72 小時。持續 ECG 監測（VPCs 好發 12-72 hr）。每 6-12 小時追蹤 lactate 與電解質。NPO 12-24 小時後開始少量流質。教導飼主出院後改為少量多餐。',
          '術後觀察 24 小時即可出院。每日餵食一次大量乾糧沒有問題。',
          'ICU 住院 1 週。預防性使用 Lidocaine CRI。限制飲水 48 小時。',
          '術後當天即可出院，口服止痛藥與抗生素。一週後回診拆線。',
        ],
        correct_actions: [
          'ICU 住院至少 48-72 小時。持續 ECG 監測（VPCs 好發 12-72 hr）。每 6-12 小時追蹤 lactate 與電解質。NPO 12-24 小時後開始少量流質。教導飼主出院後改為少量多餐。',
        ],
        feedback_correct: 'GDV 術後最大風險是心室心律不整（VPCs），好發於 12-72 小時，因此需 ICU 至少 48-72 小時 ECG 監測。Lactate 追蹤可確認組織灌流改善。電解質（尤其鉀離子）需密切監測。恢復餵食應從少量流質開始。長期改為少量多餐（2-3 次/日）可降低復發風險。',
        feedback_incorrect: 'GDV 是高風險外科急診，術後不可過早出院。心室心律不整在 12-72 小時內發生率 40-70%，可能致命，必須 ECG 監測。預防性 Lidocaine CRI 不是常規需要（僅在嚴重 VPCs 時使用）。飲食管理（少量多餐）是預防復發的重要生活調整。',
        key_learning_points: [
          'GDV 術後 VPCs 發生率 40-70%，好發於 12-72 小時，需 ICU ECG 監測',
          '嚴重 VPCs（R-on-T 現象或血流動力學不穩定）需 Lidocaine CRI 50-80 mcg/kg/min',
          'Lactate 在 12 小時內應逐步下降，持續升高為預後不良指標',
          '飲食管理：少量多餐（2-3 次/日）、避免進食後劇烈運動、升高食盆',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '大丹犬（最高風險品種）+ 餐後急性非生產性嘔吐與腹部膨脹 = 高度懷疑 GDV。急診處理遵循「先穩定再手術」原則：積極輸液 → X 光確診 → lactate 預後評估 → derotation + gastropexy → ICU 術後監測。',
      key_decisions: [
        '即使高度懷疑 GDV 也要先穩定再手術——休克狀態下麻醉風險極高',
        '術前 lactate 判讀影響與飼主的溝通（> 6.0 需告知較高死亡率）',
        '術中胃壁活性評估決定是否需要 partial gastrectomy',
        '術後 48-72 小時持續 ECG 監測是降低 VPCs 相關死亡的關鍵',
      ],
      common_pitfalls: [
        '在未穩定血流動力學的情況下急於手術',
        '僅做胃減壓未做 gastropexy，導致高復發率',
        '忽略術後心律不整監測，VPCs 可在看似穩定的犬突然發生',
        '過早恢復正常餵食或出院',
        '未評估脾臟——脾臟撕裂出血可能在術後才表現',
      ],
    },
    related_nodes_for_review: [
      'SURG-L3-001',
      'SURG-L2-001',
      'SURG-L4-001',
      'SURG-L4-002',
      'SURG-L5-001',
      'SURG-L5-003',
    ],
    tags: ['GDV', '急診', '大丹犬', 'gastropexy', 'lactate', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-002',
    title: '犬 IVDD 手術決策 (Canine IVDD Surgical Decision-Making)',
    specialty: 'SURG',
    cross_specialties: ['NEURO'],
    required_nodes: ['NEURO-L3-002', 'SURG-L3-005', 'NEURO-L5-002'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '5 歲已絕育雄性迷你臘腸犬（Miniature Dachshund），體重 6.5 kg',
      chief_complaint: '今晨突然後肢完全癱瘓，無法自行站立或排尿',
      brief_history: '飼主報告兩天前犬從沙發跳下後出現背痛和後肢無力（仍可行走但搖晃）。昨日症狀加劇，行走明顯困難。今晨醒來發現犬完全無法移動後肢，嘗試排尿但僅有少量溢出。精神沉鬱，拒絕進食。過去一年曾有兩次類似但較輕微的「背痛」發作，經止痛藥和休息後改善。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '面對急性癱瘓的臘腸犬，你需要進一步確認哪些關鍵病史？請選擇最重要的 3 項。',
        available_actions: [
          '確認癱瘓發展的時間進程（急性惡化 vs 漸進性）',
          '確認排尿功能（自主排尿 vs 溢尿 vs 完全無法排尿）',
          '詢問過去「背痛」發作的細節與治療反應',
          '詢問飲食品牌',
          '詢問最近是否接觸過壁蝨',
          '詢問運動習慣與生活環境（階梯、跳躍）',
        ],
        correct_actions: [
          '確認癱瘓發展的時間進程（急性惡化 vs 漸進性）',
          '確認排尿功能（自主排尿 vs 溢尿 vs 完全無法排尿）',
          '詢問過去「背痛」發作的細節與治療反應',
        ],
        feedback_correct: '很好！癱瘓時間進程對預後至關重要——急性完全癱瘓（< 24-48 小時）且深層痛覺消失需緊急手術評估。排尿功能是上位運動神經元（UMN）vs 下位運動神經元（LMN）膀胱的重要鑑別。過去反覆背痛高度提示慢性椎間盤退化（Hansen Type I）。',
        feedback_incorrect: '急性脊髓壓迫的病史重點：(1) 時間進程決定緊急度；(2) 排尿功能反映脊髓損傷程度；(3) 過去類似發作提示椎間盤慢性退化。臘腸犬為 IVDD 最好發品種。',
        key_learning_points: [
          '臘腸犬是 Hansen Type I IVDD 最好發品種（發生率 19-24%）',
          '排尿功能是評估脊髓損傷程度的重要指標',
          '反覆「背痛」發作提示多處椎間盤退化',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '神經學檢查與分級',
        prompt: '病史補充：癱瘓在 12 小時內從可行走惡化至完全癱瘓。目前無法自主排尿（溢尿狀態）。進行完整神經學檢查。請選擇最關鍵的 3 項發現。',
        available_actions: [
          '後肢完全癱瘓（paraplegia），無自主運動',
          '後肢脊髓反射亢進（膝蓋反射 +3，退縮反射亢進）——上位運動神經元徵象',
          '深層痛覺（deep pain perception）：用止血鉗夾趾骨——犬轉頭嚎叫（深痛覺存在）',
          '前肢神經學檢查完全正常',
          '背部觸診 T12-L2 區域明顯疼痛反應',
          '肛門反射存在',
        ],
        correct_actions: [
          '後肢完全癱瘓（paraplegia），無自主運動',
          '後肢脊髓反射亢進（膝蓋反射 +3，退縮反射亢進）——上位運動神經元徵象',
          '深層痛覺（deep pain perception）：用止血鉗夾趾骨——犬轉頭嚎叫（深痛覺存在）',
        ],
        feedback_correct: '正確！後肢癱瘓 + UMN 徵象（反射亢進）= T3-L3 脊髓節段病變。深層痛覺是最關鍵的預後指標——存在深痛覺（Grade IV）比消失深痛覺（Grade V）預後顯著較佳。背痛定位在 T12-L2 進一步確認胸腰椎 IVDD。此犬為 Modified Frankel Grade IV（癱瘓、有深痛覺）。',
        feedback_incorrect: '脊髓損傷的神經學檢查三大重點：(1) 運動功能（行走能力 → 自主運動 → 癱瘓）；(2) 反射型態（UMN vs LMN 定位脊髓節段）；(3) 深層痛覺（最關鍵的預後指標）。Dachshund 胸腰椎 IVDD 最常見在 T12-L2 區域。',
        key_learning_points: [
          'IVDD 神經學分級（Modified Frankel）：Grade I 僅背痛 → Grade V 深痛覺消失',
          'UMN 徵象（反射亢進）= T3-L3 脊髓節段病變',
          '深層痛覺是 IVDD 最重要的預後指標',
          '深痛覺存在（Grade IV）手術後恢復率 > 90%',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '影像學診斷',
        prompt: '神經學檢查定位為 T3-L3 脊髓病變，Grade IV（癱瘓、深痛覺存在）。你會安排哪些影像學檢查？選擇最適當的 2 項。',
        available_actions: [
          'MRI 胸腰椎（首選：評估脊髓壓迫位置、程度與脊髓訊號變化）',
          'CT 脊椎（替代方案：快速、可見鈣化椎間盤，但對脊髓評估不如 MRI）',
          '脊椎 X 光（篩檢椎間盤鈣化與椎間隙變窄）',
          '脊髓攝影（myelography）',
          '腹部超音波',
          '全身 CT 掃描',
        ],
        correct_actions: [
          'MRI 胸腰椎（首選：評估脊髓壓迫位置、程度與脊髓訊號變化）',
          'CT 脊椎（替代方案：快速、可見鈣化椎間盤，但對脊髓評估不如 MRI）',
        ],
        feedback_correct: '完美！MRI 是 IVDD 術前影像的金標準——可精確定位壓迫節段、區分急性 vs 慢性、評估脊髓訊號變化（水腫 vs 壞死軟化）作為預後參考。CT 在 Hansen Type I（鈣化椎間盤突出）也很有價值，速度快且對鈣化物質的定位優異。實務上若無 MRI 可用，CT 是可接受的替代。脊椎 X 光僅能看到鈣化與椎間隙變窄，無法確認壓迫位置，不足以做手術規劃。',
        feedback_incorrect: 'IVDD 術前影像首選 MRI（脊髓評估最佳）或 CT（速度快、鈣化定位佳）。脊椎 X 光無法確認壓迫的確切位置和脊髓狀態，不適合作為手術規劃的唯一依據。Myelography 為侵入性，已被 MRI/CT 取代。',
        key_learning_points: [
          'MRI 是 IVDD 術前影像的金標準',
          'CT 對 Hansen Type I（鈣化椎間盤突出）的偵測也很優異',
          'MRI T2 脊髓高訊號 > 2 椎體長度提示預後較差',
          '脊椎 X 光僅為篩檢工具，不足以做手術規劃',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '影像結果判讀與手術決策',
        prompt: 'MRI 結果：\n• T12-T13 椎間盤突出（Hansen Type I），嚴重腹側脊髓壓迫\n• 脊髓 T2 高訊號延伸約 1.5 椎體長度（水腫）\n• T11-T12 另有輕度椎間盤膨出但無明顯壓迫\n• 無其他節段異常\n\n是否建議手術？',
        available_actions: [
          '強烈建議緊急手術（hemilaminectomy）。Grade IV + 急性惡化 < 24 hr + MRI 確認嚴重壓迫 → 手術預後佳（恢復率 > 90%）。T2 高訊號 < 2 椎體為可逆水腫。越早減壓預後越好。',
          '先保守治療 4-6 週（嚴格籠休 + 止痛），觀察是否改善',
          '深痛覺還在，不急著手術，等幾天看看',
          '需等 MRI 訊號完全消退才能手術',
        ],
        correct_actions: [
          '強烈建議緊急手術（hemilaminectomy）。Grade IV + 急性惡化 < 24 hr + MRI 確認嚴重壓迫 → 手術預後佳（恢復率 > 90%）。T2 高訊號 < 2 椎體為可逆水腫。越早減壓預後越好。',
        ],
        feedback_correct: '正確！Grade IV IVDD 手術 vs 保守治療的證據明確：手術恢復率 > 90%，保守治療恢復率約 50%。急性惡化的 Grade III-V 是手術強適應症。T2 高訊號 < 2 椎體提示水腫而非壞死軟化，預後良好。Hemilaminectomy 是胸腰椎 IVDD 的標準術式。深痛覺存在但延遲手術有惡化至 Grade V 的風險——Grade V 恢復率顯著下降。',
        feedback_incorrect: 'Grade IV IVDD（癱瘓、有深痛覺）是手術的強適應症。保守治療在 Grade III-V 的恢復率遠低於手術。延遲手術增加惡化至 Grade V（深痛覺消失）的風險，一旦惡化至 Grade V > 48 小時，恢復率驟降至 < 50%。',
        key_learning_points: [
          'Grade III-V IVDD 手術恢復率 > 保守治療',
          'Grade IV 手術恢復率 > 90%',
          'Grade V < 48 hr 深痛覺消失：手術恢復率 50-60%',
          'Grade V > 48 hr 深痛覺消失：手術恢復率 < 5-10%',
          'T2 高訊號長度與預後相關：< 2 椎體較佳',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術執行：Hemilaminectomy',
        prompt: '飼主同意手術。描述 hemilaminectomy 的關鍵步驟與注意事項。',
        available_actions: [
          '正確手術流程：背側入路 → 確認正確節段（counting from T13 or lumbar） → 單側移除椎弓板（hemilaminectomy）→ 移除突出的椎間盤物質 → 檢視脊髓減壓充分 → 關閉（肌肉覆蓋缺損區域）。術中注意不傷及脊髓和脊髓血管。',
          '正中背側入路 → 雙側全椎弓板切除（dorsal laminectomy）',
          '腹側入路直接進入椎間盤',
          '不需移除椎間盤物質，僅打開骨頭減壓即可',
        ],
        correct_actions: [
          '正確手術流程：背側入路 → 確認正確節段（counting from T13 or lumbar） → 單側移除椎弓板（hemilaminectomy）→ 移除突出的椎間盤物質 → 檢視脊髓減壓充分 → 關閉（肌肉覆蓋缺損區域）。術中注意不傷及脊髓和脊髓血管。',
        ],
        feedback_correct: '正確！Hemilaminectomy 是胸腰椎 IVDD 的標準術式。單側入路（通常從壓迫較嚴重側）提供足夠的視野移除突出的椎間盤物質，同時保留對側椎弓板維持脊椎穩定性。正確定位手術節段至關重要——術中可用肋骨計數或術前放置標記。移除所有突出的椎間盤物質確保充分減壓。Dorsal laminectomy 破壞更多結構，增加脊椎不穩定風險。',
        feedback_incorrect: 'Hemilaminectomy（單側椎弓板切除）是胸腰椎 IVDD 金標準術式。相比 dorsal laminectomy 保留更多結構穩定性。腹側入路不適用於胸腰椎。必須移除突出的椎間盤物質才能充分減壓。',
        key_learning_points: [
          'Hemilaminectomy 是胸腰椎 IVDD 的金標準術式',
          '單側入路保留對側椎弓板，維持脊椎穩定性',
          '術中正確定位手術節段極為重要',
          '必須移除所有突出的椎間盤物質確保充分減壓',
        ],
      },
      {
        step_number: 6,
        step_type: 'treatment_plan',
        title: '術後復健與長期管理',
        prompt: '手術順利完成，術後 24 小時犬已可感受深痛刺激，精神改善，膀胱需手動擠壓排尿。術後管理與復健計畫？選擇最適當的 2 項。',
        available_actions: [
          '術後急性期：疼痛管理（multimodal analgesia）+ 膀胱管理（每日 3-4 次手擠排尿至自主排尿恢復）+ 嚴格籠休 4-6 週 + 被動關節活動範圍運動（PROM）開始',
          '復健計畫：第 1-2 週 PROM + 水中跑步機（若可）→ 第 3-4 週漸進站立和輔助行走訓練 → 第 5-8 週逐步增加負重和距離。長期：體重管理（減輕脊椎負擔）+ 環境調整（斜坡取代階梯、限制跳躍）+ 告知再發風險',
          '術後立即自由活動促進恢復',
          '不需復健，等自行恢復即可',
          '終身限制所有活動',
          '術後僅需止痛藥 3 天即可',
        ],
        correct_actions: [
          '術後急性期：疼痛管理（multimodal analgesia）+ 膀胱管理（每日 3-4 次手擠排尿至自主排尿恢復）+ 嚴格籠休 4-6 週 + 被動關節活動範圍運動（PROM）開始',
          '復健計畫：第 1-2 週 PROM + 水中跑步機（若可）→ 第 3-4 週漸進站立和輔助行走訓練 → 第 5-8 週逐步增加負重和距離。長期：體重管理（減輕脊椎負擔）+ 環境調整（斜坡取代階梯、限制跳躍）+ 告知再發風險',
        ],
        feedback_correct: '正確！IVDD 術後管理兩大支柱：(1) 急性期——疼痛管理、膀胱護理（UMN 膀胱恢復常需 1-3 週）和嚴格籠休；(2) 復健——從 PROM 開始漸進至主動運動。水中跑步機（underwater treadmill）利用浮力減輕負重，是最有效的 IVDD 復健工具。長期體重管理極為重要——肥胖增加椎間盤負荷和再發風險。臘腸犬 IVDD 再發率約 15-25%。',
        feedback_incorrect: '術後自由活動可能導致手術部位損傷。IVDD 術後復健是恢復的關鍵——被動到主動運動的漸進過渡。膀胱管理是術後最重要的護理之一。體重管理和環境調整是降低再發的長期策略。',
        key_learning_points: [
          'UMN 膀胱恢復通常需 1-3 週，期間需手動擠壓排尿',
          '水中跑步機是 IVDD 復健最有效的工具',
          '臘腸犬 IVDD 術後再發率約 15-25%',
          '體重管理和環境調整是降低再發的關鍵',
          'Grade IV 術後大多數犬在 2-6 週內恢復行走能力',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '臘腸犬（好發品種）+ 急性後肢癱瘓 + UMN 徵象 + 背痛 T12-L2 → 胸腰椎 IVDD。神經學分級 Grade IV（癱瘓、有深痛覺）→ MRI 確認 T12-T13 嚴重壓迫 → 緊急 hemilaminectomy → 術後疼痛管理 + 膀胱護理 + 漸進復健 → 長期體重和環境管理。',
      key_decisions: [
        '神經學分級（Modified Frankel）決定手術 vs 保守治療',
        '深層痛覺為最重要的預後指標',
        'Grade IV 強烈建議手術——恢復率 > 90%',
        'MRI 評估壓迫位置和脊髓訊號',
        '術後復健計畫是恢復的關鍵',
      ],
      common_pitfalls: [
        '未做深層痛覺測試導致錯誤分級',
        '將 Grade IV 以保守治療拖延導致惡化至 Grade V',
        '術中定位錯誤節段',
        '術後忽略膀胱管理導致膀胱過度擴張',
        '忽略術後復健',
      ],
    },
    related_nodes_for_review: ['NEURO-L3-002', 'SURG-L3-005', 'NEURO-L5-002', 'NEURO-L4-001'],
    tags: ['IVDD', '臘腸犬', 'hemilaminectomy', '脊髓壓迫', '復健', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-003',
    title: '犬反覆排尿困難膀胱結石',
    specialty: 'SURG',
    cross_specialties: ['IM', 'CPATH'],
    required_nodes: ['SURG-L3-004', 'SURG-L4-002', 'SURG-L5-003'],
    difficulty: 3,
    species: '犬',
    scenario: {
      signalment: '5 歲已絕育雄性迷你雪納瑞，體重 7 kg',
      chief_complaint: '反覆血尿和頻尿三週，昨日開始排尿時嗚叫',
      brief_history: '三週前開始出現血尿和頻尿，曾至他院使用抗生素治療一週無改善。昨日開始排尿困難且排尿時嗚叫。食慾稍減，精神略差。過去兩年有兩次泌尿道感染病史。飲食為乾飼料為主。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '選擇最重要的 3 項。',
        available_actions: [
          '詢問飲食內容（品牌、是否含處方飼料）',
          '詢問過去泌尿道感染時是否做過尿液培養',
          '詢問是否有結石病史或影像紀錄',
          '詢問每日飲水量',
          '詢問疫苗施打紀錄',
          '詢問居家環境（室內/室外）',
        ],
        correct_actions: [
          '詢問飲食內容（品牌、是否含處方飼料）',
          '詢問過去泌尿道感染時是否做過尿液培養',
          '詢問是否有結石病史或影像紀錄',
        ],
        feedback_correct: '正確！飲食直接影響尿液 pH 和結石形成風險。過去感染未培養可能錯過感染相關結石（如 struvite）。結石病史幫助判斷是否為復發性問題。迷你雪納瑞為草酸鈣結石好發品種。',
        feedback_incorrect: '泌尿道結石病史重點：飲食（結石風險）、過去感染與培養結果（感染性結石鑑別）、先前影像紀錄（復發評估）。品種易感性也很重要。',
        key_learning_points: [
          '迷你雪納瑞為草酸鈣和磷酸鈣結石好發品種',
          '反覆 UTI 需考慮感染性結石（struvite）',
          '飲食影響尿液 pH 和結石類型',
          '未做尿液培養的 UTI 可能漏診潛在結石',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '選擇最關鍵的檢查發現。',
        available_actions: [
          '膀胱觸診：膨脹、觸診時疼痛，可觸及硬質異物感',
          '腹部觸診：無其他異常',
          '尿道觸診：陰莖骨處無明顯阻塞',
          '體溫 38.8°C（正常）',
          '心肺聽診正常',
          '口腔黏膜粉紅、CRT < 2 秒',
        ],
        correct_actions: [
          '膀胱觸診：膨脹、觸診時疼痛，可觸及硬質異物感',
          '尿道觸診：陰莖骨處無明顯阻塞',
        ],
        feedback_correct: '正確！膀胱觸診可直接感覺到膀胱結石（cystic calculi）。陰莖骨處是雄犬尿道結石最常嵌頓的位置，觸診排除尿道阻塞很重要。膀胱膨脹和疼痛提示排尿困難。',
        feedback_incorrect: '泌尿道結石理學檢查重點：膀胱觸診（大小、疼痛、異物感）和尿道觸診（陰莖骨處阻塞評估）。雄犬陰莖骨處為尿道最窄處。',
        key_learning_points: [
          '膀胱結石常可經腹部觸診感覺到',
          '雄犬尿道最窄處為 os penis（陰莖骨）位置',
          '排尿困難需區分膀胱結石 vs 尿道結石 vs 尿道阻塞',
          '完全尿道阻塞為急診狀況',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '影像與實驗室檢查',
        prompt: '選擇最適當的 3 項。',
        available_actions: [
          '腹部 X 光（含全段尿道）',
          '腹部超音波',
          '尿液分析 + 尿液培養與藥敏',
          '完整血檢（CBC + 生化）',
          '靜脈腎盂造影（IVP）',
          '膀胱鏡檢查',
        ],
        correct_actions: [
          '腹部 X 光（含全段尿道）',
          '尿液分析 + 尿液培養與藥敏',
          '完整血檢（CBC + 生化）',
        ],
        feedback_correct: '正確！腹部 X 光可偵測 radio-opaque 結石（草酸鈣、struvite 大多可見）。全段尿道 X 光排除尿道結石。尿液分析（pH、結晶、感染）+ 培養幫助判斷結石類型。血檢評估腎功能和術前狀態。',
        feedback_incorrect: '膀胱結石診斷流程：X 光（偵測 radio-opaque 結石）→ 尿液分析+培養（結石類型線索）→ 血檢（腎功能+術前）。超音波可輔助偵測 radiolucent 結石。',
        key_learning_points: [
          '草酸鈣和 struvite 結石多為 radio-opaque',
          'Urate 和 cystine 結石為 radiolucent（X 光不可見）',
          'X 光需含全段尿道（排除尿道結石）',
          '尿液培養在泌尿道結石病例中必做（排除感染性結石）',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '檢查結果判讀',
        prompt: '結果：\n- X 光：膀胱內多顆 radio-opaque 結石（最大 1.2 cm），尿道無結石\n- 尿液：pH 6.0，大量 RBC，少量 WBC，草酸鈣結晶，尿比重 1.035\n- 尿液培養：無細菌生長\n- 血檢：BUN/Creatinine 正常，其餘正常\n\n最可能的結石類型和處理方案？',
        available_actions: [
          '草酸鈣結石 → 手術取出（cystotomy）為首選，因草酸鈣無法溶解',
          'Struvite 結石 → 可嘗試飲食溶解治療',
          '混合型結石 → 先藥物治療觀察',
          '結石太小 → 先觀察等待自行排出',
        ],
        correct_actions: [
          '草酸鈣結石 → 手術取出（cystotomy）為首選，因草酸鈣無法溶解',
        ],
        feedback_correct: '正確！酸性尿液（pH 6.0）+ 草酸鈣結晶 + 無感染 + 迷你雪納瑞品種 = 高度懷疑草酸鈣結石。草酸鈣結石無法經飲食或藥物溶解，手術為唯一治療選擇。Struvite 才可嘗試飲食溶解。',
        feedback_incorrect: '草酸鈣結石特徵：酸性尿 + 草酸鈣結晶 + 無感染 + 好發品種。草酸鈣無法溶解（不像 struvite），手術為必要治療。',
        key_learning_points: [
          '草酸鈣結石無法溶解，手術為唯一治療選擇',
          'Struvite 結石可嘗試飲食溶解（需確認無解剖阻塞）',
          '草酸鈣好發條件：酸性尿、高鈣、特定品種',
          '結石類型最終需送結石分析確認',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術計畫',
        prompt: '選擇最適當的手術方案。',
        available_actions: [
          'Cystotomy（膀胱切開取石）+ 術中 normograde urohydropropulsion 沖洗尿道 + 結石送分析',
          '僅 cystotomy 取出可見結石',
          '體外震波碎石術（ESWL）',
          '經尿道取石（voiding urohydropropulsion）',
        ],
        correct_actions: [
          'Cystotomy（膀胱切開取石）+ 術中 normograde urohydropropulsion 沖洗尿道 + 結石送分析',
        ],
        feedback_correct: '正確！Cystotomy 為膀胱結石標準治療。術中 normograde urohydropropulsion 沖洗尿道確保無殘留結石（術後 X 光確認）。結石送分析確認類型以指導長期預防。術後 X 光確認完全清除非常重要。',
        feedback_incorrect: 'Cystotomy 術中重點：完全取出所有結石 + 尿道沖洗排除殘留 + 術後 X 光確認 + 結石送分析。僅取出可見結石不夠，微小結石常殘留。',
        key_learning_points: [
          'Cystotomy 為膀胱結石的標準手術治療',
          '術中 normograde urohydropropulsion 排除尿道殘留結石',
          '術後 X 光必做（確認無殘留結石）',
          '結石樣本送實驗室分析以確認類型',
        ],
      },
      {
        step_number: 6,
        step_type: 'treatment_plan',
        title: '術後管理與長期預防',
        prompt: '結石分析確認為 100% 草酸鈣。選擇最適當的長期預防方案。',
        available_actions: [
          '處方飼料（低草酸、適度蛋白質限制）+ 增加飲水量 + 定期尿液監測 + 每 3-6 個月腹部 X 光/超音波',
          '酸化尿液飲食',
          '長期抗生素預防',
          '不需特殊預防，僅追蹤',
        ],
        correct_actions: [
          '處方飼料（低草酸、適度蛋白質限制）+ 增加飲水量 + 定期尿液監測 + 每 3-6 個月腹部 X 光/超音波',
        ],
        feedback_correct: '正確！草酸鈣結石復發率高（約 50% 在 3 年內復發）。處方飼料降低尿液鈣和草酸濃度。增加飲水稀釋尿液。定期影像追蹤早期偵測新結石。酸化尿液反而增加草酸鈣風險（酸化適用於 struvite 預防）。',
        feedback_incorrect: '草酸鈣預防：飲食管理（低草酸）+ 增加飲水 + 定期追蹤。注意：酸化尿液增加草酸鈣風險！鹼化尿液才有幫助。復發率高需長期監測。',
        key_learning_points: [
          '草酸鈣結石 3 年內復發率約 50%',
          '預防策略：處方飼料 + 增加飲水量 + 定期影像追蹤',
          '酸化尿液增加草酸鈣風險（與 struvite 預防相反）',
          '檸檬酸鉀（potassium citrate）可鹼化尿液降低草酸鈣風險',
        ],
      },
      {
        step_number: 7,
        step_type: 'case_summary',
        title: '病例總結',
        prompt: '總結此病例的關鍵決策點。',
        available_actions: [
          '迷你雪納瑞反覆 UTI + 血尿 → 膀胱結石鑑別 → X 光確診 → 草酸鈣結石手術取出（cystotomy）→ 長期飲食管理預防復發',
          '抗生素治療反覆 UTI 即可',
          '小結石觀察等待排出',
          '僅改變飲食不需手術',
        ],
        correct_actions: [
          '迷你雪納瑞反覆 UTI + 血尿 → 膀胱結石鑑別 → X 光確診 → 草酸鈣結石手術取出（cystotomy）→ 長期飲食管理預防復發',
        ],
        feedback_correct: '正確！本病例為典型的好發品種草酸鈣膀胱結石。關鍵決策：(1) 反覆 UTI 需影像排除結石，(2) 草酸鈣無法溶解→手術必要，(3) 術中完全清除+術後 X 光確認，(4) 長期飲食管理+定期追蹤預防復發。',
        feedback_incorrect: '草酸鈣結石管理流程：診斷（影像+尿液分析）→ 手術取出 → 結石分析確認 → 長期飲食+水分管理 → 定期影像追蹤。',
        key_learning_points: [
          '反覆 UTI 需影像檢查排除潛在結石',
          '結石類型決定治療方式（溶解 vs 手術）',
          '術中完全清除和術後 X 光確認至關重要',
          '長期預防策略依結石類型而不同',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '好發品種反覆泌尿問題 + 酸性尿草酸鈣結晶 → 草酸鈣結石。無法溶解 → cystotomy。術中徹底沖洗 + 術後 X 光。結石分析確認 → 長期飲食管理和影像追蹤。',
      key_decisions: [
        '辨識好發品種和結石類型',
        '草酸鈣無法溶解 → 手術為必要治療',
        '術中 urohydropropulsion + 術後 X 光確認完全清除',
        '長期飲食管理和定期影像追蹤預防復發',
      ],
      common_pitfalls: [
        '反覆 UTI 未做影像檢查延誤結石診斷',
        '嘗試飲食溶解草酸鈣結石（只有 struvite 可溶解）',
        '術後未 X 光確認殘留結石',
        '酸化尿液預防草酸鈣（應鹼化）',
      ],
    },
    related_nodes_for_review: ['SURG-L3-004', 'SURG-L4-002', 'SURG-L5-003', 'IM-L3-008'],
    tags: ['膀胱結石', '草酸鈣', 'cystotomy', '泌尿外科', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-004',
    title: '小型犬膝蓋骨脫位手術矯正',
    specialty: 'SURG',
    cross_specialties: ['IM'],
    required_nodes: ['SURG-L3-005', 'SURG-L4-003', 'SURG-L5-004'],
    difficulty: 3,
    species: '犬',
    scenario: {
      signalment: '2 歲已絕育雌性吉娃娃，體重 2.8 kg',
      chief_complaint: '間歇性右後肢跛行六個月，近一個月加重',
      brief_history: '六個月前開始偶爾右後肢抬腿行走，數步後恢復正常。近一個月跛行頻率增加，有時後肢呈彎曲姿勢行走。無外傷史。精神食慾正常。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '選擇最重要的 3 項。',
        available_actions: [
          '詢問跛行是否為間歇性（走幾步就好）還是持續性',
          '詢問是否有外傷史或跳躍後開始',
          '詢問對側後肢是否也有類似問題',
          '詢問飲食品牌',
          '詢問居家地板材質',
          '詢問洗澡頻率',
        ],
        correct_actions: [
          '詢問跛行是否為間歇性（走幾步就好）還是持續性',
          '詢問是否有外傷史或跳躍後開始',
          '詢問對側後肢是否也有類似問題',
        ],
        feedback_correct: '正確！間歇性「跳步」（skipping gait）為 MPL 的典型表現，代表膝蓋骨暫時脫位後自行復位。排除外傷性原因（如韌帶斷裂）很重要。MPL 常為雙側性（20-25% 對側也有），需評估雙側。',
        feedback_incorrect: 'MPL 病史重點：跛行模式（間歇性 skipping 為典型）、外傷排除、雙側評估。小型犬 MPL 多為先天性/發育性而非外傷性。',
        key_learning_points: [
          'MPL（Medial Patellar Luxation）為小型犬最常見的骨科問題之一',
          '間歇性「跳步」（skipping gait）為典型表現',
          '雙側 MPL 發生率約 20-50%',
          '小型犬 MPL 多為發育性而非外傷性',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '骨科檢查',
        prompt: '選擇最關鍵的檢查發現。',
        available_actions: [
          '右膝蓋骨可手動推向內側脫位，放手後自行復位（Grade II MPL）',
          '左膝蓋骨位置正常，無法手動脫位',
          '右膝關節無抽屜徵象（anterior drawer test 陰性）',
          '右膝關節活動範圍正常，無捻髮音',
          '髖關節 Ortolani test 陰性',
          '神經學檢查正常',
        ],
        correct_actions: [
          '右膝蓋骨可手動推向內側脫位，放手後自行復位（Grade II MPL）',
          '右膝關節無抽屜徵象（anterior drawer test 陰性）',
          '左膝蓋骨位置正常，無法手動脫位',
        ],
        feedback_correct: '正確！MPL 分級檢查為最重要的骨科評估。Grade II：可手動脫位，放手或伸展膝關節後自行復位。排除前十字韌帶斷裂（drawer test）很重要，因 MPL 會增加 CCL 斷裂風險。檢查對側排除雙側問題。',
        feedback_incorrect: 'MPL 骨科檢查重點：膝蓋骨分級（Grade I-IV）、前十字韌帶評估（drawer test）、對側膝蓋骨檢查。MPL 與 CCL 疾病常共存。',
        key_learning_points: [
          'MPL 分級：Grade I（可脫位但自行復位）、II（手動脫位放手復位）、III（永久脫位可復位）、IV（永久脫位不可復位）',
          'Grade II 以上通常建議手術矯正',
          'MPL 增加前十字韌帶斷裂（CCL rupture）風險',
          '雙側膝蓋骨必須檢查',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '影像檢查',
        prompt: '選擇最適當的影像評估。',
        available_actions: [
          '雙側膝關節 X 光（正面+側面）評估骨骼排列和關節退化',
          '骨盆腹背位 X 光（排除髖關節問題）',
          'MRI 膝關節',
          'CT 膝關節',
          '關節液分析',
          '不需影像，直接手術',
        ],
        correct_actions: [
          '雙側膝關節 X 光（正面+側面）評估骨骼排列和關節退化',
          '骨盆腹背位 X 光（排除髖關節問題）',
        ],
        feedback_correct: '正確！膝關節 X 光評估：(1) 脛骨近端的扭轉/彎曲、(2) 股骨滑車溝深度、(3) 關節退化程度、(4) 軟組織腫脹。骨盆 X 光排除髖關節問題（Legg-Calvé-Perthes 病也好發於小型犬）。術前 X 光為手術計畫的基礎。',
        feedback_incorrect: 'MPL 影像評估：膝關節 X 光（骨骼排列、退化評估）+ 骨盆 X 光（排除髖關節問題）。X 光為術前計畫的基礎。MRI 通常不需要。',
        key_learning_points: [
          'X 光評估重點：脛骨扭轉、股骨彎曲、滑車溝深度、關節退化',
          '小型犬需排除 Legg-Calvé-Perthes 病（股骨頭缺血性壞死）',
          '術前 X 光幫助制定手術方案',
          '嚴重骨骼畸形可能需要矯正截骨術',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: 'X 光結果與手術決策',
        prompt: '結果：\n- 右膝 X 光：膝蓋骨位於正常位置（檢查時復位），股骨滑車溝淺化，輕度關節退化\n- 左膝 X 光：正常\n- 骨盆 X 光：髖關節正常，無 Legg-Calvé-Perthes 徵象\n- 脛骨無明顯彎曲或扭轉\n\n選擇最適當的手術方案。',
        available_actions: [
          '滑車溝加深術（trochleoplasty）+ 脛骨粗隆移位術（TTT）+ 內側關節囊疊縮術（imbrication）',
          '僅滑車溝加深術',
          '僅外側關節囊縫合',
          '保守治療（體重管理 + NSAIDs）',
        ],
        correct_actions: [
          '滑車溝加深術（trochleoplasty）+ 脛骨粗隆移位術（TTT）+ 內側關節囊疊縮術（imbrication）',
        ],
        feedback_correct: '正確！Grade II MPL 合併臨床症狀建議手術。標準手術為「三合一」組合：(1) Trochleoplasty 加深滑車溝提供膝蓋骨穩定軌道，(2) TTT 將股四頭肌力量重新對準，(3) 內側疊縮術收緊鬆弛的關節囊。單一技術復發率較高。',
        feedback_incorrect: 'MPL 手術原則：多重組合技術效果最佳。單一技術復發率高。Grade II 合併臨床症狀為手術適應症。保守治療僅適用 Grade I 無症狀者。',
        key_learning_points: [
          'MPL 手術通常需多重技術組合',
          'Trochleoplasty（block recession 或 wedge recession）為核心技術',
          'TTT（tibial tuberosity transposition）矯正股四頭肌力量方向',
          '內側關節囊疊縮術（imbrication）收緊鬆弛軟組織',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '術後復健計畫',
        prompt: '選擇最適當的術後管理方案。',
        available_actions: [
          '6-8 週限制活動 + 2 週後開始被動關節活動（PROM）+ 逐步增加負重 + 術後 6 週 X 光追蹤',
          '完全籠內休息 8 週不活動',
          '術後立即自由活動',
          '術後使用外固定夾板 4 週',
        ],
        correct_actions: [
          '6-8 週限制活動 + 2 週後開始被動關節活動（PROM）+ 逐步增加負重 + 術後 6 週 X 光追蹤',
        ],
        feedback_correct: '正確！術後復健重點：限制活動預防植入物鬆脫，但適度被動關節活動預防關節僵硬和肌肉萎縮。逐步增加負重促進骨癒合。X 光追蹤確認 TTT pin 位置和骨癒合。成功率約 90%。',
        feedback_incorrect: 'MPL 術後管理：限制活動（非完全不動）+ 適度復健（PROM）+ 逐步負重 + 影像追蹤。完全不動導致關節僵硬和肌肉萎縮。',
        key_learning_points: [
          'MPL 術後限制活動 6-8 週',
          '早期被動關節活動（PROM）預防僵硬',
          '術後成功率約 90%（復發率 10-15%）',
          '術後追蹤重點：膝蓋骨穩定性、關節活動範圍、X 光',
        ],
      },
      {
        step_number: 6,
        step_type: 'case_summary',
        title: '病例總結',
        prompt: '選擇最適當的長期評估。',
        available_actions: [
          'Grade II MPL 合併症狀 → 三合一手術矯正 → 術後復健 → 長期監測對側膝蓋和關節退化。預後良好（90% 功能恢復）。',
          '術後不需追蹤',
          '手術後完全治癒無復發風險',
          '所有 MPL 都應保守治療',
        ],
        correct_actions: [
          'Grade II MPL 合併症狀 → 三合一手術矯正 → 術後復健 → 長期監測對側膝蓋和關節退化。預後良好（90% 功能恢復）。',
        ],
        feedback_correct: '正確！MPL 手術預後良好但非完美。長期追蹤：對側 MPL 發展（20-50%）、關節退化進程、CCL 疾病風險。體重管理對小型犬尤其重要。發育性 MPL 雖手術矯正，骨骼畸形趨勢仍在。',
        feedback_incorrect: 'MPL 手術雖預後良好，但需長期追蹤對側膝蓋骨、關節退化、CCL 疾病風險。復發率 10-15%。',
        key_learning_points: [
          'MPL 手術治療 Grade II 以上合併臨床症狀',
          '多重技術組合效果最佳',
          '術後復健為成功關鍵',
          '長期追蹤對側膝蓋和關節退化',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '小型犬間歇性後肢跛行（skipping gait）→ Grade II MPL → X 光排除骨骼畸形和髖關節問題 → 三合一手術矯正 → 術後復健 → 長期監測。',
      key_decisions: [
        'Grade II 合併臨床症狀為手術適應症',
        '多重技術組合（trochleoplasty + TTT + imbrication）',
        '排除 CCL 共病和 Legg-Calvé-Perthes',
        '術後復健計畫和長期追蹤',
      ],
      common_pitfalls: [
        '僅做單一技術導致復發率高',
        '未評估對側膝蓋骨',
        '未排除前十字韌帶問題',
        '術後過早自由活動導致植入物失敗',
      ],
    },
    related_nodes_for_review: ['SURG-L3-005', 'SURG-L4-003', 'SURG-L5-004', 'SURG-L2-003'],
    tags: ['膝蓋骨脫位', 'MPL', 'trochleoplasty', 'TTT', '骨科', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-005',
    title: '未絕育母犬子宮蓄膿',
    specialty: 'SURG',
    cross_specialties: ['ECC', 'IM'],
    required_nodes: ['SURG-L3-006', 'SURG-L4-001', 'SURG-L5-002'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '8 歲未絕育雌性拉布拉多，體重 28 kg',
      chief_complaint: '嘔吐、食慾廢絕兩天，多渴多尿一週',
      brief_history: '一週前開始明顯多喝多尿。兩天前開始嘔吐，完全不吃。精神沉鬱。飼主注意到外陰部有少量膿樣分泌物。最後一次發情約 6 週前。從未絕育，從未懷孕。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '選擇最重要的 3 項。',
        available_actions: [
          '詢問最後一次發情的確切時間',
          '詢問外陰部分泌物的顏色、量和持續時間',
          '詢問是否曾使用外源性荷爾蒙（如避孕針）',
          '詢問飲食品牌',
          '詢問排便狀況',
          '詢問居家其他寵物健康狀況',
        ],
        correct_actions: [
          '詢問最後一次發情的確切時間',
          '詢問外陰部分泌物的顏色、量和持續時間',
          '詢問是否曾使用外源性荷爾蒙（如避孕針）',
        ],
        feedback_correct: '正確！子宮蓄膿典型在發情後 4-8 週發生（黃體期，高黃體酮環境）。外陰分泌物區分 open vs closed pyometra（closed 更危險）。外源性荷爾蒙（如 medroxyprogesterone）大幅增加子宮蓄膿風險。',
        feedback_incorrect: '子宮蓄膿病史重點：發情史（發情後 4-8 週好發）、陰道分泌物（open vs closed）、荷爾蒙使用史。未絕育 + 中老年 = 高風險。',
        key_learning_points: [
          '子宮蓄膿好發於發情後 4-8 週（黃體期）',
          'Open pyometra 有陰道分泌物，closed pyometra 無（更危險）',
          '外源性黃體酮使用大幅增加風險',
          '未絕育中老年母犬發生率隨年齡增加',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '選擇最關鍵的檢查發現。',
        available_actions: [
          '體溫 39.8°C（發燒），心率 140 bpm，黏膜略蒼白，CRT 2.5 秒',
          '腹部觸診：中後腹部可觸及管狀膨大結構，輕度疼痛',
          '外陰部：腫脹，少量紅棕色膿樣分泌物',
          '脫水評估：約 6-8% 脫水（皮膚彈性下降，黏膜略乾）',
          '心肺聽診：竇性心搏過速',
          '體重 28 kg（飼主說正常體重 30 kg）',
        ],
        correct_actions: [
          '體溫 39.8°C（發燒），心率 140 bpm，黏膜略蒼白，CRT 2.5 秒',
          '腹部觸診：中後腹部可觸及管狀膨大結構，輕度疼痛',
          '脫水評估：約 6-8% 脫水（皮膚彈性下降，黏膜略乾）',
        ],
        feedback_correct: '正確！發燒 + 心搏過速 + CRT 延長提示 SIRS/敗血症風險。腹部管狀膨大結構為子宮膨脹的直接證據（但觸診需輕柔避免子宮破裂）。脫水評估指導術前液體復甦計畫。',
        feedback_incorrect: '子宮蓄膿理學檢查重點：全身發炎反應評估（SIRS 徵象）、腹部觸診（膨大子宮）、脫水程度。觸診子宮需輕柔！',
        key_learning_points: [
          'SIRS 標準（犬）：體溫 > 39.2 或 < 37.8°C、HR > 120、RR > 20、WBC > 16 或 < 6',
          '子宮觸診需輕柔（避免破裂導致腹膜炎）',
          '脫水程度指導液體治療計畫',
          'Closed pyometra 更容易子宮破裂（壓力更高）',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '診斷與術前評估',
        prompt: '選擇最適當的 3 項。',
        available_actions: [
          '腹部超音波（確認子宮蓄膿 + 排除子宮破裂）',
          '完整血檢（CBC + 生化 + 電解質）',
          '腹部 X 光',
          '陰道細胞學',
          '尿液分析',
          '凝血功能檢查',
        ],
        correct_actions: [
          '腹部超音波（確認子宮蓄膿 + 排除子宮破裂）',
          '完整血檢（CBC + 生化 + 電解質）',
          '凝血功能檢查',
        ],
        feedback_correct: '正確！超音波為子宮蓄膿首選影像（液體填充子宮、子宮壁厚度、排除破裂和腹腔游離液）。血檢評估白血球（常見嗜中性球增多 + left shift）、腎功能（子宮蓄膿常併發腎損傷）、電解質。凝血檢查排除 DIC（敗血症併發症）。',
        feedback_incorrect: '子宮蓄膿診斷：超音波（確診+排除破裂）→ 血檢（感染/腎功能/電解質）→ 凝血（DIC 風險）。子宮蓄膿常併發腎損傷和 DIC。',
        key_learning_points: [
          '超音波為子宮蓄膿首選影像檢查',
          '常見血檢異常：嗜中性球增多（+/- left shift）、高球蛋白血症、BUN/Cre 升高',
          '子宮蓄膿可導致腎小管損傷（E. coli 內毒素抑制 ADH 作用 → 多尿）',
          'DIC 為嚴重併發症，需監測凝血功能',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '檢查結果與穩定化',
        prompt: '結果：\n- 超音波：雙側子宮角明顯膨大（直徑約 4 cm），充滿液體，子宮壁增厚，無游離腹腔液\n- CBC：WBC 38,000（嗜中性球增多，band neutrophils 15%），PCV 35%\n- 生化：BUN 45 mg/dL（↑），Creatinine 2.1 mg/dL（↑），ALP 280 U/L（↑），Glucose 正常\n- 電解質：正常\n- 凝血：PT/aPTT 正常\n\n術前穩定化方案？',
        available_actions: [
          '積極靜脈輸液復甦（crystalloid 20-30 mL/kg/hr × 1-2 hr）+ 廣效抗生素（Ampicillin-Sulbactam IV）+ 止吐劑 + 矯正脫水後盡速手術',
          '先抗生素治療穩定 3-5 天再手術',
          '嘗試內科治療（Aglepristone + 抗生素）',
          '立即手術不需穩定化',
        ],
        correct_actions: [
          '積極靜脈輸液復甦（crystalloid 20-30 mL/kg/hr × 1-2 hr）+ 廣效抗生素（Ampicillin-Sulbactam IV）+ 止吐劑 + 矯正脫水後盡速手術',
        ],
        feedback_correct: '正確！子宮蓄膿為手術急診，但需先穩定化。積極輸液矯正脫水和改善腎灌流。廣效抗生素覆蓋 gram(-) 菌（E. coli 為最常見病原）。矯正脫水後（通常 2-6 小時）盡速 OHE。延遲手術增加敗血症和子宮破裂風險。',
        feedback_incorrect: '子宮蓄膿處理：穩定化（輸液+抗生素）→ 盡速手術。延遲手術風險高。純內科治療僅考慮於繁殖犬且 open pyometra。腎功能異常通常術後可改善。',
        key_learning_points: [
          '子宮蓄膿為手術急診，OHE 為首選治療',
          '術前穩定化：積極輸液 + 廣效抗生素 + 止吐',
          'E. coli 為最常見病原（>70%）',
          '腎功能異常多數在子宮移除後可改善',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術注意事項',
        prompt: '選擇 OHE 手術的最重要注意事項。',
        available_actions: [
          '避免子宮破裂（輕柔處理）、確實結紮子宮頸殘端、腹腔沖洗（若有污染）、術中血壓和體溫監測',
          '使用標準 OHE 技術即可無需特殊注意',
          '先抽吸子宮內容物再切除',
          '術後不需特殊監測',
        ],
        correct_actions: [
          '避免子宮破裂（輕柔處理）、確實結紮子宮頸殘端、腹腔沖洗（若有污染）、術中血壓和體溫監測',
        ],
        feedback_correct: '正確！蓄膿子宮壁脆弱易破裂，操作需極輕柔。子宮頸殘端結紮需確實（感染組織縫合線易鬆脫）。若有子宮內容物溢出需溫熱生理食鹽水沖洗腹腔。麻醉期間需密切監測血壓（敗血症低血壓風險）和體溫。',
        feedback_incorrect: '子宮蓄膿 OHE 特殊注意：子宮脆弱避免破裂、殘端確實結紮、腹腔污染沖洗、麻醉密切監測。與常規 OHE 不同，風險更高。',
        key_learning_points: [
          '蓄膿子宮壁脆弱，術中需極輕柔處理',
          '子宮頸殘端結紮需額外確實（感染組織握持力差）',
          '腹腔污染需溫熱生理食鹽水沖洗',
          '麻醉風險高於常規 OHE（敗血症、低血壓、低體溫）',
        ],
      },
      {
        step_number: 6,
        step_type: 'treatment_plan',
        title: '術後管理',
        prompt: '選擇最適當的術後管理方案。',
        available_actions: [
          '持續 IV 輸液（監測尿量 1-2 mL/kg/hr）+ IV 抗生素 48-72 hr 後轉口服（Amoxicillin-Clavulanate 14 天）+ 每日血檢追蹤腎功能 + 止痛',
          '手術完成即可出院口服抗生素',
          '抗生素使用 3 天即可停藥',
          '不需監測腎功能',
        ],
        correct_actions: [
          '持續 IV 輸液（監測尿量 1-2 mL/kg/hr）+ IV 抗生素 48-72 hr 後轉口服（Amoxicillin-Clavulanate 14 天）+ 每日血檢追蹤腎功能 + 止痛',
        ],
        feedback_correct: '正確！術後需住院監護。持續輸液支持腎灌流（監測尿量確認腎功能恢復）。IV 抗生素 48-72 小時待穩定後轉口服，總療程 10-14 天。腎功能通常在 48-72 小時內改善。止痛管理對術後恢復重要。',
        feedback_incorrect: '子宮蓄膿術後重點：輸液支持（腎灌流）+ 抗生素（IV→口服，共 14 天）+ 腎功能監測 + 止痛。術後需住院觀察至少 48-72 小時。',
        key_learning_points: [
          '術後持續輸液支持腎灌流',
          '尿量監測（目標 1-2 mL/kg/hr）為腎功能恢復指標',
          '抗生素療程 10-14 天',
          '腎功能通常在子宮移除後 48-72 小時改善',
        ],
      },
      {
        step_number: 7,
        step_type: 'case_summary',
        title: '病例總結',
        prompt: '選擇最適當的預後評估。',
        available_actions: [
          'Open pyometra + 及時手術 + 術前穩定化 → 預後良好（存活率 > 95%）。腎損傷通常可逆。術後需完成抗生素療程和腎功能追蹤。',
          '子宮蓄膿一律預後不良',
          '腎損傷不可逆需長期治療',
          '不需術後追蹤',
        ],
        correct_actions: [
          'Open pyometra + 及時手術 + 術前穩定化 → 預後良好（存活率 > 95%）。腎損傷通常可逆。術後需完成抗生素療程和腎功能追蹤。',
        ],
        feedback_correct: '正確！Open pyometra 及時治療存活率 > 95%。Closed pyometra 或合併嚴重敗血症/DIC 預後較差。腎損傷為子宮蓄膿常見併發症但多數可逆（E. coli 內毒素干擾 ADH → 腎小管濃縮功能障礙）。早期預防性 OHE 可完全避免此疾病。',
        feedback_incorrect: 'Open pyometra 及時手術預後良好（> 95% 存活）。預後不良因子：closed pyometra、DIC、敗血性休克、子宮破裂。腎損傷多數可逆。',
        key_learning_points: [
          'Open pyometra 手術存活率 > 95%',
          'Closed pyometra 和嚴重敗血症為預後不良因子',
          '腎損傷多數在子宮移除後可逆',
          '早期 OHE 為最佳預防策略',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '未絕育中老年母犬 + 發情後 6 週 + 全身症狀 + 陰道分泌物 → open pyometra。超音波確診 → 術前穩定化（輸液+抗生素）→ 緊急 OHE → 術後腎功能追蹤。',
      key_decisions: [
        '辨識 SIRS 徵象和敗血症風險',
        '術前穩定化再手術（不是延遲手術）',
        '手術中避免子宮破裂的輕柔操作',
        '術後腎功能監測和輸液支持',
      ],
      common_pitfalls: [
        '未辨識子宮蓄膿（當作一般腸胃炎處理）',
        '延遲手術等待內科穩定太久',
        '術前未評估凝血功能（DIC 風險）',
        '術後過早停止輸液和抗生素',
      ],
    },
    related_nodes_for_review: ['SURG-L3-006', 'SURG-L4-001', 'SURG-L5-002', 'ECC-L3-005', 'IM-L3-009'],
    tags: ['子宮蓄膿', 'pyometra', 'OHE', '敗血症', '急診手術', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-006',
    title: '犬前肢骨折固定選擇',
    specialty: 'SURG',
    cross_specialties: ['ECC'],
    required_nodes: ['SURG-L3-007', 'SURG-L4-003', 'SURG-L5-002'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '3 歲已絕育雄性邊境牧羊犬，體重 20 kg',
      chief_complaint: '今日被車撞後右前肢無法負重',
      brief_history: '今日下午散步時掙脫牽繩跑到馬路上被車撞。右前肢完全不負重，前臂區域腫脹明顯。犬意識清醒但疼痛。飼主立即送醫。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'physical_examination',
        title: '急診評估',
        prompt: '選擇最優先的 3 項評估。',
        available_actions: [
          '初級調查（ABC）：呼吸道、呼吸、循環系統快速評估',
          '胸腔快速超音波（TFAST）排除氣胸和胸腔積液',
          '腹腔快速超音波（AFAST）排除腹腔出血',
          '右前肢詳細檢查',
          '神經學檢查',
          '體溫測量',
        ],
        correct_actions: [
          '初級調查（ABC）：呼吸道、呼吸、循環系統快速評估',
          '胸腔快速超音波（TFAST）排除氣胸和胸腔積液',
          '腹腔快速超音波（AFAST）排除腹腔出血',
        ],
        feedback_correct: '正確！車禍傷患的第一優先永遠是生命徵象穩定，不是骨折處理。初級調查（Airway, Breathing, Circulation）排除威脅生命的問題。TFAST 排除氣胸（車禍常見）。AFAST 排除腹腔出血。骨折為第二優先。',
        feedback_incorrect: '車禍急診處理原則：先穩定生命徵象（ABC）→ 排除胸腹腔危及生命的損傷（TFAST/AFAST）→ 再處理骨折。骨折不會致命但內出血和氣胸會。',
        key_learning_points: [
          '車禍患者第一優先：生命徵象穩定（ABC）',
          'TFAST 排除氣胸和胸腔積液',
          'AFAST 排除腹腔出血',
          '骨折處理為第二優先（穩定後處理）',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '骨折部位評估',
        prompt: '生命徵象穩定後，右前肢檢查發現如下。選擇最重要的 3 項。',
        available_actions: [
          '右前臂中段明顯腫脹、變形，觸診有捻髮音（crepitus）',
          '右前肢遠端（掌部）感覺和血循存在',
          '右前肢可觸及異常活動度（abnormal mobility）',
          '右肩關節和肘關節穩定',
          '左前肢正常',
          '皮膚完整，無開放性傷口',
        ],
        correct_actions: [
          '右前臂中段明顯腫脹、變形，觸診有捻髮音（crepitus）',
          '右前肢遠端（掌部）感覺和血循存在',
          '皮膚完整，無開放性傷口',
        ],
        feedback_correct: '正確！捻髮音和變形為骨折的直接證據。遠端感覺和血循評估排除血管神經損傷（骨折碎片可能損傷橈動脈或神經）。確認皮膚完整排除開放性骨折（open fracture 需緊急處理且預後不同）。',
        feedback_incorrect: '骨折部位評估重點：骨折徵象（腫脹、變形、捻髮音）、遠端神經血管狀態、開放 vs 閉鎖性。Open fracture 為急診手術適應症。',
        key_learning_points: [
          '骨折評估三要素：骨折特徵、遠端神經血管、open vs closed',
          '開放性骨折（皮膚破損+骨折外露）為急診手術適應症',
          '遠端血循和感覺評估排除血管神經損傷',
          '車禍骨折常合併軟組織損傷',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '影像檢查',
        prompt: '選擇最適當的影像檢查。',
        available_actions: [
          '右前臂正面+側面 X 光（含肘關節和腕關節）+ 三視位胸腔 X 光',
          '僅右前臂 X 光',
          'CT 右前肢',
          '右前臂超音波',
          'MRI 右前肢',
          '全身 X 光',
        ],
        correct_actions: [
          '右前臂正面+側面 X 光（含肘關節和腕關節）+ 三視位胸腔 X 光',
        ],
        feedback_correct: '正確！骨折 X 光需兩個正交視角（AP + lateral）且必須包含上下關節。胸腔 X 光為車禍患者必做（排除肺挫傷、氣胸、橫膈膜疝氣等），即使 TFAST 正常也建議做。',
        feedback_incorrect: '骨折影像原則：兩個正交視角 + 含上下關節。車禍患者需加做胸腔 X 光（肺挫傷、氣胸可能延遲出現）。',
        key_learning_points: [
          '骨折 X 光：至少兩個正交視角（AP + lateral）',
          '必須包含骨折上下的關節（排除脫位/關節內骨折）',
          '車禍患者胸腔 X 光必做（肺挫傷可延遲 24-48 hr 出現）',
          '關節內骨折可能需要 CT 進一步評估',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: 'X 光結果與骨折分類',
        prompt: '結果：\n- 右橈尺骨中段橫向骨折（transverse fracture），骨折處有輕度位移，無粉碎\n- 肘關節和腕關節正常\n- 胸腔 X 光：輕度右側肺挫傷，無氣胸\n\n選擇最適當的骨折固定方式。',
        available_actions: [
          '骨板固定（plate and screws）：DCP 或 locking plate',
          '外固定器（external skeletal fixator, ESF）',
          '髓內釘（intramedullary pin）搭配 cerclage wire',
          '石膏外固定（cast/splint）保守治療',
        ],
        correct_actions: [
          '骨板固定（plate and screws）：DCP 或 locking plate',
        ],
        feedback_correct: '正確！中大型犬（20 kg）前臂橈尺骨橫向骨折，骨板固定為金標準。前肢承受彎曲力為主，骨板提供最佳抗彎曲穩定性。Locking plate 在骨質較差或粉碎性骨折時特別有利。外固定或石膏對中大型犬前臂骨折效果不如骨板。',
        feedback_incorrect: '犬橈尺骨骨折固定選擇：骨板固定為中大型犬的金標準。前臂主要承受彎曲力，骨板抗彎曲效果最佳。小型犬/玩具犬橈尺骨骨折反而是骨科挑戰（不癒合率高）。',
        key_learning_points: [
          '中大型犬橈尺骨骨折：骨板固定為金標準',
          '骨板提供最佳抗彎曲穩定性（前臂主要受力方式）',
          'Locking plate vs DCP：骨質差或粉碎時 locking 更佳',
          '小型犬橈尺骨骨折不癒合率高（血供差），為骨科挑戰',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術計畫與時機',
        prompt: '選擇最適當的手術時機和計畫。',
        available_actions: [
          '先止痛+Robert Jones 繃帶暫時固定 → 24-48 hr 後（肺挫傷穩定後）進行骨板固定手術 → 術後 X 光確認復位',
          '立即手術不等待',
          '等 1 週消腫後再手術',
          '保守治療外固定 6-8 週',
        ],
        correct_actions: [
          '先止痛+Robert Jones 繃帶暫時固定 → 24-48 hr 後（肺挫傷穩定後）進行骨板固定手術 → 術後 X 光確認復位',
        ],
        feedback_correct: '正確！有肺挫傷時不宜立即全身麻醉（肺挫傷在 24-48 小時內可能惡化）。Robert Jones 繃帶提供暫時穩定和消腫。待胸腔狀況穩定後再進行骨折固定手術。閉鎖性骨折不需緊急手術（不像開放性骨折需 6-8 小時內手術）。',
        feedback_incorrect: '閉鎖性骨折手術時機：不需立即手術（除非開放性）。合併肺挫傷時需等待胸腔穩定（24-48 hr）。Robert Jones 繃帶提供暫時固定。',
        key_learning_points: [
          '閉鎖性骨折為選擇性手術（非急診手術）',
          '開放性骨折需 6-8 小時內手術清創固定',
          'Robert Jones 繃帶提供暫時固定和消腫',
          '合併胸腔損傷時需等待穩定後再麻醉',
        ],
      },
      {
        step_number: 6,
        step_type: 'treatment_plan',
        title: '術後管理與骨癒合監測',
        prompt: '選擇最適當的術後管理。',
        available_actions: [
          '限制活動 8-12 週 + 每 4-6 週 X 光追蹤骨癒合 + 止痛管理 + 適度復健（術後 2 週開始 PROM）',
          '術後自由活動促進骨癒合',
          '術後外固定繃帶 8 週',
          '僅在有問題時才 X 光追蹤',
        ],
        correct_actions: [
          '限制活動 8-12 週 + 每 4-6 週 X 光追蹤骨癒合 + 止痛管理 + 適度復健（術後 2 週開始 PROM）',
        ],
        feedback_correct: '正確！骨折癒合平均需要 8-12 週。X 光追蹤監測骨痂形成和癒合進度。限制活動避免植入物失敗。適度被動活動預防關節僵硬和肌肉萎縮。骨板固定後通常不需額外外固定。',
        feedback_incorrect: '骨折術後管理：限制活動（8-12 週）+ 定期 X 光追蹤 + 止痛 + 適度復健。過度活動導致植入物失敗，完全不動導致關節僵硬。',
        key_learning_points: [
          '骨折癒合平均 8-12 週',
          '每 4-6 週 X 光追蹤骨痂形成',
          '骨板固定後通常不需額外外固定',
          '術後復健預防關節僵硬和肌肉萎縮',
        ],
      },
      {
        step_number: 7,
        step_type: 'case_summary',
        title: '病例總結',
        prompt: '選擇最適當的總結。',
        available_actions: [
          '車禍多重創傷 → ABC 優先 → 排除胸腹腔危及生命損傷 → 閉鎖性橈尺骨骨折骨板固定 → 術後復健和影像追蹤。預後良好（中大型犬橈尺骨骨板固定癒合率 > 95%）。',
          '骨折為車禍最優先處理項目',
          '所有骨折都需立即手術',
          '保守治療效果與手術相同',
        ],
        correct_actions: [
          '車禍多重創傷 → ABC 優先 → 排除胸腹腔危及生命損傷 → 閉鎖性橈尺骨骨折骨板固定 → 術後復健和影像追蹤。預後良好（中大型犬橈尺骨骨板固定癒合率 > 95%）。',
        ],
        feedback_correct: '正確！關鍵決策鏈：生命優先 → 系統性損傷排除 → 骨折分類（open/closed、位置、型態）→ 固定方式選擇 → 手術時機（考量合併損傷）→ 術後管理。中大型犬橈尺骨骨板固定癒合率高，預後良好。',
        feedback_incorrect: '車禍骨折處理流程：ABC → 系統性損傷排除 → 骨折分類和固定 → 術後追蹤。生命徵象永遠第一優先。',
        key_learning_points: [
          '車禍急診：ABC 和生命徵象第一優先',
          '骨折分類決定固定方式和手術時機',
          '中大型犬橈尺骨骨折骨板固定為金標準',
          '合併胸腔損傷影響麻醉時機',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '車禍 → ABC 優先 + TFAST/AFAST → 發現肺挫傷 → 暫時 Robert Jones 繃帶 → 穩定後骨板固定橈尺骨橫向骨折 → 術後復健和影像追蹤。',
      key_decisions: [
        '車禍急診先穩定生命徵象再處理骨折',
        '閉鎖性骨折配合胸腔穩定延遲手術',
        '中大型犬橈尺骨選擇骨板固定',
        '術後限制活動和定期影像追蹤',
      ],
      common_pitfalls: [
        '忽略胸腹腔損傷直接處理骨折',
        '合併肺挫傷時過早全身麻醉',
        '中大型犬橈尺骨骨折使用外固定效果不佳',
        '術後過早自由活動導致植入物失敗',
      ],
    },
    related_nodes_for_review: ['SURG-L3-007', 'SURG-L4-003', 'SURG-L5-002', 'ECC-L3-001', 'ECC-L4-002'],
    tags: ['骨折', '骨板固定', '車禍', '急診骨科', 'Robert Jones', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-007',
    title: '犬會陰疝氣手術',
    specialty: 'SURG',
    cross_specialties: ['IM', 'CPATH'],
    required_nodes: ['SURG-L3-008', 'SURG-L4-002', 'SURG-L5-001'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '9 歲未絕育雄性柯乃犬，體重 12 kg',
      chief_complaint: '會陰部腫脹三週，排便困難一週',
      brief_history: '三週前飼主發現肛門旁右側有軟性腫脹，初期不影響生活。一週前開始排便困難，需用力才能排出少量軟便。偶有排尿改變（尿流較細）。精神食慾尚可但近日稍減。從未絕育。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '選擇最重要的 3 項。',
        available_actions: [
          '詢問腫脹是否在排便用力時增大（Valsalva 相關）',
          '詢問是否有排尿異常（膀胱後翻可能）',
          '詢問是否有前列腺相關症狀（裡急後重、血尿）',
          '詢問飲食品牌',
          '詢問活動量',
          '詢問過去手術史',
        ],
        correct_actions: [
          '詢問腫脹是否在排便用力時增大（Valsalva 相關）',
          '詢問是否有排尿異常（膀胱後翻可能）',
          '詢問是否有前列腺相關症狀（裡急後重、血尿）',
        ],
        feedback_correct: '正確！用力時增大為疝氣典型表現（腹壓增加使臟器疝出）。排尿異常提示膀胱可能後翻（retroflexion）進入疝氣囊，為嚴重併發症。前列腺肥大為未絕育老年犬會陰疝氣的重要促發因子（裡急後重增加腹壓）。',
        feedback_incorrect: '會陰疝氣病史重點：腫脹與腹壓關係、排尿功能評估（膀胱後翻）、前列腺狀態。未絕育雄犬幾乎專屬此病。',
        key_learning_points: [
          '會陰疝氣幾乎僅發生於未絕育老年雄犬',
          '前列腺肥大（BPH）為主要促發因子',
          '膀胱後翻（retroflexion）為危急併發症，可導致尿路阻塞',
          '雄性荷爾蒙導致骨盆膈肌肉萎縮為病因',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '選擇最關鍵的檢查發現。',
        available_actions: [
          '右側會陰部：可復位的軟性腫脹（約 6 cm），站立時更明顯',
          '直腸指診：直腸偏移至右側疝氣囊，前列腺中度對稱增大',
          '膀胱觸診：腹腔內可觸及膀胱（未後翻）',
          '左側會陰部：輕度肌肉薄弱但無明顯疝出',
          '肛門周圍皮膚完整，無潰瘍',
          '體溫正常',
        ],
        correct_actions: [
          '右側會陰部：可復位的軟性腫脹（約 6 cm），站立時更明顯',
          '直腸指診：直腸偏移至右側疝氣囊，前列腺中度對稱增大',
          '膀胱觸診：腹腔內可觸及膀胱（未後翻）',
        ],
        feedback_correct: '正確！可復位腫脹確認為疝氣（不可復位可能為嵌頓/絞扼）。直腸指診為最重要的檢查：評估疝氣內容（直腸偏移）、前列腺大小（促發因子）。膀胱位置確認排除膀胱後翻（若膀胱不在腹腔需緊急處理）。檢查對側排除雙側。',
        feedback_incorrect: '會陰疝氣理學檢查重點：疝氣可復位性、直腸指診（內容物+前列腺）、膀胱位置確認、對側評估。直腸指診為最關鍵檢查。',
        key_learning_points: [
          '直腸指診為會陰疝氣最重要的檢查',
          '疝氣內容物可能包含：直腸、前列腺、膀胱、小腸、腹膜脂肪',
          '膀胱後翻為急診狀況（泌尿阻塞）',
          '評估對側（雙側會陰疝氣常見，約 20-40%）',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '術前評估',
        prompt: '選擇最適當的 3 項。',
        available_actions: [
          '腹部超音波（確認膀胱位置 + 前列腺評估 + 排除其他腹腔問題）',
          '完整血檢（CBC + 生化）',
          '會陰部 X 光（對比劑灌腸確認直腸位置）',
          '尿液分析',
          '前列腺活檢',
          '心電圖',
        ],
        correct_actions: [
          '腹部超音波（確認膀胱位置 + 前列腺評估 + 排除其他腹腔問題）',
          '完整血檢（CBC + 生化）',
          '會陰部 X 光（對比劑灌腸確認直腸位置）',
        ],
        feedback_correct: '正確！超音波確認膀胱位置（腹腔 vs 疝氣囊）、前列腺大小和形態（排除腫瘤，BPH vs neoplasia）。血檢為術前評估。對比劑灌腸 X 光顯示直腸偏移程度和疝氣囊範圍，幫助手術計畫。',
        feedback_incorrect: '會陰疝氣術前評估：超音波（膀胱+前列腺）、血檢（術前）、X 光/對比劑灌腸（疝氣範圍）。前列腺評估很重要，需排除腫瘤。',
        key_learning_points: [
          '超音波確認膀胱位置和前列腺狀態',
          '前列腺需排除腫瘤（尤其有血尿或不對稱增大時）',
          'BPH 為最常見前列腺疾病（絕育後可消退）',
          '對比劑灌腸顯示直腸偏移和結腸擴張程度',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '檢查結果與手術計畫',
        prompt: '結果：\n- 超音波：膀胱在腹腔正常位置，前列腺對稱增大（3.5 cm，BPH 型態），無腫瘤跡象\n- 血檢：正常\n- X 光灌腸：直腸明顯右偏進入會陰疝氣囊\n\n選擇最適當的手術方案。',
        available_actions: [
          '右側會陰疝氣修補術（internal obturator muscle transposition）+ 同時去勢（castration）+ 評估左側是否需預防性修補',
          '僅去勢不修補疝氣',
          '僅修補疝氣不去勢',
          '保守治療（軟便劑 + 灌腸）',
        ],
        correct_actions: [
          '右側會陰疝氣修補術（internal obturator muscle transposition）+ 同時去勢（castration）+ 評估左側是否需預防性修補',
        ],
        feedback_correct: '正確！Internal obturator muscle transposition 為最常用的會陰疝氣修補技術（將內閉孔肌翻轉覆蓋缺損）。同時去勢為必要步驟：移除雄性荷爾蒙使 BPH 消退且降低復發率（未去勢復發率 > 40%，去勢後 < 10%）。對側可能需預防性修補。',
        feedback_incorrect: '會陰疝氣手術原則：修補缺損 + 同時去勢（降低復發率）。不去勢復發率 > 40%。Internal obturator transposition 為標準技術。',
        key_learning_points: [
          'Internal obturator muscle transposition 為標準修補技術',
          '同時去勢為必要（未去勢復發率 > 40%）',
          '去勢使 BPH 消退，移除促發因子',
          '對側預防性修補視情況決定',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術技術要點',
        prompt: '選擇手術的最重要注意事項。',
        available_actions: [
          '辨識並保護陰部神經（pudendal nerve）和內陰部動脈 + 確實縫合內閉孔肌至薦結節韌帶和外肛門括約肌 + 避免直腸損傷',
          '單純直接縫合缺損邊緣即可',
          '使用人工網片（mesh）為首選',
          '不需注意任何特殊結構',
        ],
        correct_actions: [
          '辨識並保護陰部神經（pudendal nerve）和內陰部動脈 + 確實縫合內閉孔肌至薦結節韌帶和外肛門括約肌 + 避免直腸損傷',
        ],
        feedback_correct: '正確！陰部神經損傷導致糞便失禁（最嚴重的手術併發症）。內閉孔肌需確實固定至強壯的薦結節韌帶（sacrotuberous ligament）和外肛門括約肌才能有效關閉缺損。直腸位於手術區域，操作不慎可導致穿孔和感染。',
        feedback_incorrect: '會陰疝氣手術關鍵：保護陰部神經（避免失禁）、確實固定至強壯結構（薦結節韌帶）、避免直腸損傷。手術併發症嚴重需謹慎。',
        key_learning_points: [
          '陰部神經損傷 → 糞便失禁（最嚴重併發症）',
          '內閉孔肌固定至薦結節韌帶和外肛門括約肌',
          '直腸損傷導致嚴重感染',
          '手術併發症發生率約 10-25%（感染、復發、直腸損傷）',
        ],
      },
      {
        step_number: 6,
        step_type: 'treatment_plan',
        title: '術後管理',
        prompt: '選擇最適當的術後管理。',
        available_actions: [
          '軟便劑（lactulose）2-4 週避免排便用力 + 低殘留飲食 + 抗生素 7-10 天 + 限制活動 4 週 + 伊莉莎白項圈防舔',
          '術後正常飲食和活動',
          '術後禁食 3 天',
          '長期使用灌腸',
        ],
        correct_actions: [
          '軟便劑（lactulose）2-4 週避免排便用力 + 低殘留飲食 + 抗生素 7-10 天 + 限制活動 4 週 + 伊莉莎白項圈防舔',
        ],
        feedback_correct: '正確！術後管理目標：減少會陰區域壓力。軟便劑避免排便用力（用力增加會陰壓力，影響修補）。低殘留飲食減少排便量和頻率。抗生素預防會陰區感染（汙染風險高）。限制活動和項圈保護傷口。',
        feedback_incorrect: '會陰疝氣術後重點：軟便劑（減少排便壓力）+ 低殘留飲食 + 抗生素（汙染區手術）+ 限制活動 + 傷口保護。排便壓力管理為術後成功關鍵。',
        key_learning_points: [
          '軟便劑（lactulose）減少排便用力為術後關鍵',
          '會陰區手術感染風險高（接近肛門）',
          '限制活動 4 週保護修補',
          '去勢後 BPH 約 2-4 週開始消退',
        ],
      },
      {
        step_number: 7,
        step_type: 'case_summary',
        title: '病例總結',
        prompt: '選擇最適當的預後和追蹤計畫。',
        available_actions: [
          'Internal obturator transposition + 去勢 → 復發率 < 10%。追蹤重點：排便功能、對側會陰、前列腺消退。預後良好但需長期注意對側發展。',
          '手術後完全治癒不會復發',
          '保守治療效果等同手術',
          '所有會陰疝氣預後不良',
        ],
        correct_actions: [
          'Internal obturator transposition + 去勢 → 復發率 < 10%。追蹤重點：排便功能、對側會陰、前列腺消退。預後良好但需長期注意對側發展。',
        ],
        feedback_correct: '正確！修補+去勢的復發率低（< 10%），預後良好。追蹤重點：排便功能恢復、對側會陰肌肉薄弱監測（雙側發生率高）、前列腺消退確認（超音波追蹤）。去勢後前列腺約 4-8 週明顯縮小。長期排便功能通常可恢復正常。',
        feedback_incorrect: '會陰疝氣修補+去勢預後良好（復發率 < 10%）。追蹤重點：排便功能、對側監測、前列腺消退。',
        key_learning_points: [
          '修補+去勢復發率 < 10%',
          '對側會陰疝氣發展需長期監測',
          '前列腺去勢後 4-8 週明顯消退',
          '排便功能通常可恢復正常',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '未絕育老年雄犬 + 會陰腫脹 + 排便困難 → 會陰疝氣。直腸指診確認直腸偏移和 BPH。超音波排除膀胱後翻和前列腺腫瘤 → Internal obturator transposition + 去勢 → 術後軟便劑管理。',
      key_decisions: [
        '直腸指診確認疝氣內容和前列腺狀態',
        '確認膀胱位置排除後翻',
        '修補術+同時去勢降低復發率',
        '術後軟便劑管理減少排便壓力',
      ],
      common_pitfalls: [
        '未同時去勢導致高復發率',
        '術中損傷陰部神經導致失禁',
        '忽略膀胱後翻的可能性',
        '術後未使用軟便劑導致修補失敗',
      ],
    },
    related_nodes_for_review: ['SURG-L3-008', 'SURG-L4-002', 'SURG-L5-001', 'IM-L3-010'],
    tags: ['會陰疝氣', 'perineal hernia', '去勢', 'BPH', '軟組織外科', 'case_challenge'],
  },

  // ===== CASE-SURG-008: French Bulldog with BOAS =====
  {
    id: 'CASE-SURG-008',
    title: '法國鬥牛犬運動不耐與呼吸困難 — BOAS',
    specialty: 'SURG',
    cross_specialties: ['ECC'],
    required_nodes: ['SURG-L3-010', 'SURG-L1-001'],
    difficulty: 3,
    species: '犬',
    scenario: {
      signalment: '2 歲未絕育雌性法國鬥牛犬，體重 13 kg（BCS 7/9）',
      chief_complaint: '運動不耐、睡眠打鼾嚴重、天氣熱時出現呼吸費力與偶發發紺',
      brief_history: '飼主報告自幼犬時期即有打鼾，近半年症狀加劇：散步 10 分鐘即需停下喘息、睡覺時打鼾聲響到影響飼主睡眠、上週天氣炎熱時出現舌頭發紫約 30 秒自行緩解。進食速度快但偶有逆噴嚏（reverse sneezing）。無嘔吐或腹瀉。疫苗驅蟲定期。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集與風險評估',
        prompt: '這隻法國鬥牛犬因呼吸問題就診。你需要確認哪些關鍵資訊？選擇最重要的 3 項。',
        available_actions: [
          '確認運動不耐的嚴重程度與進展速度（漸進性 vs 急性惡化）',
          '評估打鼾類型：吸氣性 stertor（低沉鼾聲）vs stridor（高音哨聲）',
          '詢問是否有睡眠呼吸暫停（sleep apnea）、睡姿偏好（坐姿睡眠）',
          '詢問飲食品牌與餵食量',
          '詢問居住環境是否有冷氣',
          '詢問最近是否更換洗毛精',
        ],
        correct_actions: [
          '確認運動不耐的嚴重程度與進展速度（漸進性 vs 急性惡化）',
          '評估打鼾類型：吸氣性 stertor（低沉鼾聲）vs stridor（高音哨聲）',
          '詢問是否有睡眠呼吸暫停（sleep apnea）、睡姿偏好（坐姿睡眠）',
        ],
        feedback_correct: '正確！漸進性運動不耐提示慢性上呼吸道阻塞。Stertor（低沉鼾聲）指向鼻咽/軟腭阻塞，stridor（高音哨聲）指向喉部阻塞——有助判斷阻塞層級。睡眠呼吸暫停和坐姿睡眠為嚴重 BOAS 的重要指標（BOAS functional grading）。BCS 7/9 顯示過胖——肥胖加重 BOAS 症狀。',
        feedback_incorrect: 'BOAS 病史重點：運動不耐進程（漸進性加重為典型）、呼吸音類型（stertor vs stridor 區分阻塞層級）、睡眠呼吸品質（嚴重度指標）。肥胖和高溫為惡化因子。',
        key_learning_points: [
          'Stertor（低沉鼾聲）= 鼻咽/軟腭阻塞；Stridor（高音哨聲）= 喉部阻塞',
          '睡眠呼吸暫停為嚴重 BOAS 的警示信號',
          '法國鬥牛犬為 BOAS 最高風險品種之一',
          '肥胖（BCS > 6/9）顯著加重 BOAS 症狀',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查與 BOAS 評估',
        prompt: '進行重點理學檢查。選擇最關鍵的 3 項發現。',
        available_actions: [
          '鼻孔明顯狹窄（stenotic nares, Grade II-III），吸氣時鼻翼幾乎完全塌陷',
          '口腔檢查：軟腭過長延伸超過會厭尖端約 1.5 cm，黏膜輕度水腫',
          '頸部聽診：明顯吸氣性 stertor，安靜時亦可聞及',
          '心臟聽診：第二心音分裂',
          '腹部觸診：無明顯異常',
          '眼球突出',
        ],
        correct_actions: [
          '鼻孔明顯狹窄（stenotic nares, Grade II-III），吸氣時鼻翼幾乎完全塌陷',
          '口腔檢查：軟腭過長延伸超過會厭尖端約 1.5 cm，黏膜輕度水腫',
          '頸部聽診：明顯吸氣性 stertor，安靜時亦可聞及',
        ],
        feedback_correct: '正確！BOAS 理學檢查三大發現：(1) 鼻孔狹窄——分級為 Grade I（輕微）至 Grade III（幾乎完全閉合），此犬 Grade II-III 明確需要手術矯正。(2) 軟腭過長——超過會厭尖端表示需要 staphylectomy。(3) 安靜時可聞及 stertor——表示靜態氣道阻塞已相當嚴重（BOAS Grade III）。下一步建議：鎮靜下喉鏡檢查評估喉室外翻和喉塌陷。',
        feedback_incorrect: 'BOAS 理學檢查核心：鼻孔狹窄程度（Grade I-III）、軟腭長度（是否超過會厭）、靜態呼吸音（stertor/stridor）。安靜時可聞及呼吸音異常表示嚴重阻塞。',
        key_learning_points: [
          'Stenotic nares 分級：Grade I（輕微）→ Grade III（幾乎完全閉合）',
          '軟腭過長標準：超過會厭尖端',
          '安靜時 stertor = 靜態氣道阻塞嚴重',
          '鎮靜下喉鏡檢查評估喉室外翻和喉塌陷等繼發變化',
        ],
      },
      {
        step_number: 3,
        step_type: 'treatment_plan',
        title: '手術計畫與術後管理',
        prompt: '根據評估結果（BOAS Grade III），建議手術矯正。最適當的手術計畫為何？',
        available_actions: [
          'Multilevel surgery：Wedge rhinoplasty + Folded flap palatoplasty + Sacculectomy（如有喉室外翻）。術前減重至 BCS 5/9。術後密切監測上呼吸道水腫 24-48 小時，備好氣管插管/暫時氣管切開。',
          '僅做 Wedge rhinoplasty，軟腭不處理',
          '終身內科治療（類固醇 + 減重），不手術',
          '直接做永久氣管切開術',
        ],
        correct_actions: [
          'Multilevel surgery：Wedge rhinoplasty + Folded flap palatoplasty + Sacculectomy（如有喉室外翻）。術前減重至 BCS 5/9。術後密切監測上呼吸道水腫 24-48 小時，備好氣管插管/暫時氣管切開。',
        ],
        feedback_correct: '正確！BOAS 手術原則為多層級同時矯正：(1) Wedge rhinoplasty 擴大鼻孔——增加鼻腔氣流。(2) Folded flap palatoplasty（較傳統 staphylectomy 術後水腫少、癒合佳）截短軟腭。(3) Sacculectomy 切除外翻的喉室（如有）。術前減重可降低麻醉風險和改善術後效果。術後最關鍵 24-48 小時：上呼吸道水腫可能導致急性阻塞，需備好再插管或緊急氣管切開的設備和人力。約 85-95% 犬術後呼吸功能顯著改善。',
        feedback_incorrect: 'BOAS 手術為多層級矯正（rhinoplasty + palatoplasty ± sacculectomy），單一矯正效果不佳。術前減重和術後氣道監測為成功關鍵。',
        key_learning_points: [
          'Multilevel surgery 為 BOAS 標準治療——同時處理所有阻塞層級',
          'Folded flap palatoplasty 較傳統 staphylectomy 術後水腫少',
          '術後 24-48 小時為最危險期——上呼吸道水腫可能致命',
          '術前減重（BCS ≤ 5/9）顯著改善手術效果',
          '早期手術（< 2 歲）可預防喉塌陷等不可逆繼發變化',
        ],
      },
      {
        step_number: 4,
        step_type: 'treatment_plan',
        title: '術後急性期監測與併發症處理',
        prompt: '手術完成（Wedge rhinoplasty + Folded flap palatoplasty + Sacculectomy）。拔管後犬呼吸平穩但 2 小時後出現漸增的吸氣性 stridor，呼吸費力。SpO₂ 從 98% 降至 91%。最適當的處置？選擇 2 項。',
        available_actions: [
          '判斷為術後上呼吸道水腫導致氣道阻塞：Dexamethasone 0.1-0.2 mg/kg IV（抗水腫）+ 鎮靜（Butorphanol 0.2 mg/kg IV 降低焦慮和呼吸需求）；若無改善需準備再插管',
          '若藥物無法緩解且 SpO₂ 持續下降，需緊急暫時氣管切開（temporary tracheostomy）維持氣道通暢，通常在水腫消退後 24-72 小時可移除',
          '僅給予氧氣等待自行改善',
          '立即再次手術修正軟腭',
          '給予大量 IV 輸液',
          '口服類固醇觀察',
        ],
        correct_actions: [
          '判斷為術後上呼吸道水腫導致氣道阻塞：Dexamethasone 0.1-0.2 mg/kg IV（抗水腫）+ 鎮靜（Butorphanol 0.2 mg/kg IV 降低焦慮和呼吸需求）；若無改善需準備再插管',
          '若藥物無法緩解且 SpO₂ 持續下降，需緊急暫時氣管切開（temporary tracheostomy）維持氣道通暢，通常在水腫消退後 24-72 小時可移除',
        ],
        feedback_correct: '正確！術後上呼吸道水腫是 BOAS 手術最常見且最危險的併發症（發生率 5-10%）。處置升級階梯：(1) Dexamethasone IV 減輕黏膜水腫 + Butorphanol 鎮靜（降低氧氣需求，避免焦慮引起的呼吸惡化）。(2) 若無改善，嘗試再插管（可能困難因水腫造成喉部腫脹）。(3) 插管困難或反覆拔管後阻塞，則緊急暫時氣管切開。暫時氣管切開通常在 24-72 小時水腫消退後可安全移除。術後 ICU 監護至少 24 小時是必要的——不可低估短頭種犬的氣道風險。',
        feedback_incorrect: '僅給氧不能解決機械性氣道阻塞。術後水腫的處理需要「升級階梯」思維：藥物 → 插管 → 氣管切開。及時氣管切開可救命。',
        key_learning_points: [
          '術後上呼吸道水腫是 BOAS 手術最危險併發症（發生率 5-10%）',
          '處置升級階梯：Dexamethasone + 鎮靜 → 再插管 → 暫時氣管切開',
          '暫時氣管切開通常 24-72 小時後可移除',
          'Butorphanol 鎮靜可降低焦慮和氧氣需求，避免呼吸惡性循環',
          '短頭種犬術後需 ICU 監護至少 24 小時',
        ],
      },
      {
        step_number: 5,
        step_type: 'case_summary',
        title: '長期預後與體重管理',
        prompt: '術後水腫在 48 小時後消退，犬呼吸平穩出院。2 週後回診拆線：鼻孔明顯擴大，stertor 顯著減少。4 週後回診飼主報告運動耐受力大幅改善、打鼾明顯減輕。長期管理建議？選擇 2 項。',
        available_actions: [
          '強調體重管理為長期成功的關鍵：目標 BCS 4-5/9，肥胖復發會使呼吸症狀惡化；建議獸醫營養師制定減重計畫（目標 1-2% 體重/週），定期量體重追蹤',
          '說明 BOAS 手術改善率約 85-95%，但需持續監測：避免高溫高濕環境（短頭種散熱差）、避免使用項圈改用胸背帶、飛行限制評估。若未來出現進行性 stridor 需評估喉塌陷進展',
          '手術後完全治癒，不需任何追蹤',
          '需終身服用支氣管擴張劑',
          '建議此犬繁殖以傳承良好基因',
          '可以讓犬進行高強度運動測試肺功能',
        ],
        correct_actions: [
          '強調體重管理為長期成功的關鍵：目標 BCS 4-5/9，肥胖復發會使呼吸症狀惡化；建議獸醫營養師制定減重計畫（目標 1-2% 體重/週），定期量體重追蹤',
          '說明 BOAS 手術改善率約 85-95%，但需持續監測：避免高溫高濕環境（短頭種散熱差）、避免使用項圈改用胸背帶、飛行限制評估。若未來出現進行性 stridor 需評估喉塌陷進展',
        ],
        feedback_correct: '正確！BOAS 手術後的長期管理：(1) 體重管理是最重要的可控因子——肥胖復發會嚴重影響手術效果，BCS 應維持 4-5/9。(2) 環境管理——短頭種犬主要靠喘息散熱，高溫高濕環境是中暑的高風險因子；項圈壓迫氣管應改用胸背帶。(3) 持續監測——雖然手術改善率高（85-95%），但部分犬可能發展喉塌陷（特別是 Grade III 喉塌陷為不可逆變化，預後差）。(4) 繁殖倫理——BOAS 具遺傳性，確診犬隻不建議繁殖。(5) 航空旅行——短頭種犬是機艙氣壓環境的高風險群體。',
        feedback_incorrect: 'BOAS 手術效果良好但非完全治癒——需持續體重管理和環境調整。喉塌陷可能為長期進展的問題。BOAS 具遺傳性，不建議繁殖。',
        key_learning_points: [
          '體重管理（BCS 4-5/9）是 BOAS 術後長期成功的最重要可控因子',
          '短頭種犬避免高溫高濕環境、使用胸背帶取代項圈',
          'BOAS 手術改善率 85-95%，但部分犬可能發展喉塌陷',
          'BOAS 具遺傳性，確診犬不建議繁殖',
          '短頭種犬航空旅行為高風險，需審慎評估',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '2 歲法國鬥牛犬 + 漸進性運動不耐 + 打鼾 + 發紺 → BOAS Grade III。理學檢查確認 stenotic nares + 軟腭過長 + 靜態 stertor。鎮靜下喉鏡排除喉塌陷後 → Multilevel surgery（rhinoplasty + palatoplasty ± sacculectomy）。術後密切氣道監測處理水腫。長期體重管理與環境調整。',
      key_decisions: [
        'BOAS 嚴重度分級指導手術時機與範圍',
        '鎮靜下喉鏡檢查排除喉塌陷（Grade III 喉塌陷預後差）',
        '多層級同時矯正而非分次手術',
        '術後 24-48 小時密切氣道監測和緊急氣管切開準備',
      ],
      common_pitfalls: [
        '僅矯正單一層級（如僅做鼻孔）而忽略軟腭',
        '術後未準備緊急氣管切開導致上呼吸道水腫窒息',
        '軟腭截短過度導致鼻咽逆流',
        '忽略術前減重——肥胖增加麻醉和術後風險',
        '延遲手術導致不可逆喉塌陷',
      ],
    },
    related_nodes_for_review: ['SURG-L3-010', 'SURG-L1-001', 'SURG-L1-003', 'SURG-L5-003'],
    tags: ['BOAS', '法國鬥牛犬', '短頭種', '上呼吸道', 'multilevel surgery', 'case_challenge'],
  },

  // ===== CASE-SURG-009: Yorkshire Terrier with PSS =====
  {
    id: 'CASE-SURG-009',
    title: '約克夏㹴幼犬生長遲緩與餐後神經症狀 — 門體分流',
    specialty: 'SURG',
    cross_specialties: ['IM', 'CPATH'],
    required_nodes: ['SURG-L3-011', 'SURG-L2-001'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '8 月齡未絕育雄性約克夏㹴，體重 1.2 kg（同胎手足約 2.0 kg）',
      chief_complaint: '生長遲緩、餐後呆滯流口水、偶發抽搐',
      brief_history: '飼主報告此犬自斷奶後明顯較同胎手足矮小。進食蛋白質含量較高的食物後常出現異常行為：呆滯、流口水、無目的遊走、撞牆。上週曾有一次短暫全身性抽搐（約 30 秒），之後嗜睡約 2 小時恢復正常。飲水量偏多，排尿頻繁。無嘔吐腹瀉。疫苗驅蟲按時進行。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集與鑑別診斷',
        prompt: '這隻約克夏㹴幼犬有生長遲緩和餐後神經症狀。你需要確認哪些關鍵資訊？選擇最重要的 3 項。',
        available_actions: [
          '確認神經症狀與進食的時間關係（餐後 1-3 小時發作典型）',
          '詢問蛋白質攝取量與症狀嚴重度的相關性',
          '詢問尿液是否有結晶或結石病史（尿酸銨結晶）',
          '詢問疫苗品牌',
          '詢問是否有蟎蟲感染',
          '詢問飼料是否含穀物',
        ],
        correct_actions: [
          '確認神經症狀與進食的時間關係（餐後 1-3 小時發作典型）',
          '詢問蛋白質攝取量與症狀嚴重度的相關性',
          '詢問尿液是否有結晶或結石病史（尿酸銨結晶）',
        ],
        feedback_correct: '正確！PSS 的神經症狀（肝性腦病，HE）典型為餐後 1-3 小時發作——腸道吸收的氨和其他神經毒素繞過肝臟直接進入全身循環影響腦部。高蛋白飲食加重症狀（更多氨產生）。尿酸銨結晶/結石為 PSS 的特徵性泌尿道問題（肝臟無法將尿酸轉換為尿囊素）。約克夏㹴為先天性肝外 PSS 最高風險品種。',
        feedback_incorrect: 'PSS 病史三要素：餐後神經症狀（HE）、蛋白質攝取與症狀的關聯、尿酸銨結晶。約克夏㹴為 PSS 高風險品種。',
        key_learning_points: [
          '餐後 1-3 小時神經症狀為肝性腦病特徵',
          '高蛋白飲食加重 HE 症狀',
          '尿酸銨結晶為 PSS 特徵性尿液發現',
          '約克夏㹴為先天性肝外 PSS 最高風險品種',
        ],
      },
      {
        step_number: 2,
        step_type: 'diagnostic_plan',
        title: '診斷計畫',
        prompt: '你高度懷疑門體分流（PSS）。最適當的診斷步驟？選擇最重要的 3 項。',
        available_actions: [
          '血液生化：BUN、白蛋白、血糖、膽汁酸（空腹 + 餐後）',
          'CBC：評估微小紅血球症（microcytosis）',
          '腹部超音波：尋找異常分流血管、評估肝臟大小（小肝症）和腎臟',
          '腹部 X 光',
          '腦脊液分析',
          '皮膚生檢',
        ],
        correct_actions: [
          '血液生化：BUN、白蛋白、血糖、膽汁酸（空腹 + 餐後）',
          'CBC：評估微小紅血球症（microcytosis）',
          '腹部超音波：尋找異常分流血管、評估肝臟大小（小肝症）和腎臟',
        ],
        feedback_correct: '正確！PSS 診斷三支柱：(1) 血液生化——膽汁酸（最敏感的篩檢指標）空腹 + 餐後均升高（常 > 100 μmol/L）、BUN 偏低（肝臟尿素合成不足）、白蛋白偏低、空腹低血糖。(2) CBC——微小紅血球症（MCV 低）為 PSS 特徵性但非特異性發現，原因未完全釐清，可能與鐵代謝異常有關。(3) 腹部超音波——可直接看到分流血管（有經驗者偵測率 80-95%）、小肝症（microhepatica）、腎盂擴張。CT 血管攝影（CTA）為確認分流解剖的金標準。',
        feedback_incorrect: 'PSS 診斷：膽汁酸（最敏感）、CBC（微小紅血球症）、腹部超音波（直接可視化分流血管和小肝症）。CTA 為解剖定位金標準。',
        key_learning_points: [
          '膽汁酸（空腹 + 餐後）為 PSS 最敏感的篩檢指標',
          '微小紅血球症（MCV 低）為 PSS 特徵性 CBC 發現',
          '腹部超音波可直接看到分流血管和小肝症',
          'CT 血管攝影（CTA）為分流解剖定位金標準',
        ],
      },
      {
        step_number: 3,
        step_type: 'treatment_plan',
        title: '治療計畫',
        prompt: '診斷結果：\n- 膽汁酸：空腹 85 μmol/L，餐後 320 μmol/L（極度升高）\n- BUN 4 mg/dL（低）、Alb 1.7 g/dL（低）、Glucose 55 mg/dL（偏低）\n- MCV 58 fL（低）\n- 超音波：小肝症，可疑肝外分流血管\n- CTA 確認：單一肝外門體分流（porto-caval）\n\n最適當的治療計畫？',
        available_actions: [
          '先內科穩定（低蛋白飲食 + Lactulose + Metronidazole/Ampicillin 控制 HE）2-4 週 → 手術漸進閉合分流（Ameroid constrictor 或 Cellophane banding）。術後追蹤膽汁酸評估閉合效果。',
          '立即完全結紮分流血管',
          '終身內科治療，不手術',
          '肝臟移植',
        ],
        correct_actions: [
          '先內科穩定（低蛋白飲食 + Lactulose + Metronidazole/Ampicillin 控制 HE）2-4 週 → 手術漸進閉合分流（Ameroid constrictor 或 Cellophane banding）。術後追蹤膽汁酸評估閉合效果。',
        ],
        feedback_correct: '正確！PSS 治療策略：(1) 術前內科穩定（2-4 週）——低蛋白飲食減少氨產生、Lactulose 促進腸道排氨、Metronidazole 減少產氨菌。(2) 手術漸進閉合——Ameroid constrictor 或 Cellophane banding 在 2-5 週內緩慢閉合分流，避免急性門脈高壓（portal hypertension）。急性完全結紮可致命（門脈高壓 → 腸道水腫 → 休克）。(3) 術後追蹤——3 個月後重測膽汁酸評估閉合效果。約 85-90% 犬術後臨床改善良好。約克夏㹴預後通常良好。',
        feedback_incorrect: '急性完全結紮可致命（門脈高壓）。標準為先內科穩定 → 手術漸進閉合（ameroid/cellophane）→ 術後追蹤。',
        key_learning_points: [
          '術前內科穩定 2-4 週：低蛋白飲食 + Lactulose + 抗生素',
          '漸進閉合（Ameroid/Cellophane）避免急性門脈高壓',
          '急性完全結紮可致命——絕不可一次完全阻斷',
          '術後 3 個月重測膽汁酸評估閉合效果',
          '約 85-90% 犬術後臨床改善良好',
        ],
      },
      {
        step_number: 4,
        step_type: 'treatment_plan',
        title: '術後急性期監測與併發症處理',
        prompt: 'Ameroid constrictor 手術完成。術後 6 小時犬出現全身性癲癇發作，持續約 1 分鐘後停止。血檢：Glucose 42 mg/dL（低血糖）、NH₃ 正常。最可能的原因與處置？選擇 2 項。',
        available_actions: [
          '術後癲癇最常見原因為低血糖——PSS 犬肝醣儲存不足，手術禁食加劇；立即 50% Dextrose 0.5-1 mL/kg IV diluted bolus → 持續 Dextrose CRI（2.5-5%）維持 Glucose > 80 mg/dL',
          '術後需監測門脈高壓徵兆（腹水、腸道水腫、血便）——分流漸進閉合過程中若門脈壓升高過快可致急性門脈高壓；術後 24-72 小時密切監測腹部膨脹、腸道蠕動音和糞便性狀',
          '給予 Phenobarbital 高劑量控制',
          '立即重新手術移除 Ameroid',
          '觀察等待不需處置',
          '判斷為腦損傷，建議安樂死',
        ],
        correct_actions: [
          '術後癲癇最常見原因為低血糖——PSS 犬肝醣儲存不足，手術禁食加劇；立即 50% Dextrose 0.5-1 mL/kg IV diluted bolus → 持續 Dextrose CRI（2.5-5%）維持 Glucose > 80 mg/dL',
          '術後需監測門脈高壓徵兆（腹水、腸道水腫、血便）——分流漸進閉合過程中若門脈壓升高過快可致急性門脈高壓；術後 24-72 小時密切監測腹部膨脹、腸道蠕動音和糞便性狀',
        ],
        feedback_correct: '正確！PSS 術後癲癇的兩大原因：(1) 低血糖——PSS 犬的肝醣合成和儲存能力差（小肝症），手術禁食使原本就不足的肝醣儲備耗盡。處理：立即補充 Dextrose 並維持 CRI。(2) 術後門脈高壓——Ameroid constrictor 在手術放置後可能因局部反應導致過快閉合，引起急性門脈高壓（腹水、腸道水腫、腸黏膜屏障破壞 → 細菌轉移 → 敗血症）。第三個原因（較少見）為術後持續性 HE——NH₃ 正常排除此可能。術後 72 小時為高風險期，需密集監測 Glucose、電解質、腹部大小和糞便。',
        feedback_incorrect: '術後癲癇需立即鑑別原因：低血糖（最常見）、急性門脈高壓、持續性 HE。NH₃ 正常排除 HE，Glucose 42 mg/dL 確認低血糖為主因。門脈高壓監測是術後 72 小時的重要任務。',
        key_learning_points: [
          'PSS 術後癲癇最常見原因為低血糖（肝醣儲存不足）',
          '處理：50% Dextrose bolus → 持續 CRI 維持 Glucose > 80',
          '術後門脈高壓徵兆：腹水、腸道水腫、血便',
          '術後 72 小時為併發症高風險期',
          '低血糖、門脈高壓、持續性 HE 為術後癲癇三大鑑別',
        ],
      },
      {
        step_number: 5,
        step_type: 'case_summary',
        title: '長期追蹤與預後評估',
        prompt: '術後 3 個月回診。犬體重增至 2.0 kg（追上同胎手足），精神食慾極佳，無任何神經症狀。追蹤膽汁酸：空腹 25 μmol/L（術前 85）、餐後 45 μmol/L（術前 320）。長期管理建議？選擇 2 項。',
        available_actions: [
          '膽汁酸顯著改善（雖未完全正常化）代表分流已大幅閉合；約 50% 犬術後膽汁酸不會完全正常化但臨床症狀消失，此為可接受的結果。繼續每 6 個月追蹤膽汁酸至穩定',
          '可逐步放寬飲食限制（漸進增加蛋白質含量），觀察是否再出現 HE 症狀；若無症狀可過渡至正常優質蛋白飲食。Lactulose 和 Metronidazole 在無 HE 症狀時可逐步停藥',
          '膽汁酸未完全正常化需再次手術',
          '需終身限制蛋白質飲食',
          '需終身服用 Lactulose 和抗生素',
          '完全正常不需追蹤',
        ],
        correct_actions: [
          '膽汁酸顯著改善（雖未完全正常化）代表分流已大幅閉合；約 50% 犬術後膽汁酸不會完全正常化但臨床症狀消失，此為可接受的結果。繼續每 6 個月追蹤膽汁酸至穩定',
          '可逐步放寬飲食限制（漸進增加蛋白質含量），觀察是否再出現 HE 症狀；若無症狀可過渡至正常優質蛋白飲食。Lactulose 和 Metronidazole 在無 HE 症狀時可逐步停藥',
        ],
        feedback_correct: '正確！PSS 術後追蹤的核心指標：(1) 膽汁酸——約 50% 犬術後不會完全正常化（可能存在殘餘分流或微血管分流），但若臨床無症狀則為可接受的結果。膽汁酸從 320 降至 45 μmol/L 代表分流已大幅閉合。(2) 體重追趕是最直觀的成功指標——從 1.2 kg 增至 2.0 kg 表示肝功能恢復後生長恢復正常。(3) 飲食管理——無 HE 症狀時可漸進恢復正常蛋白飲食（每 2 週增加 10-15% 蛋白質），避免突然改變。(4) 藥物漸減——Lactulose 和 Metronidazole 在膽汁酸改善且無 HE 時可逐步停用。約 85-90% 犬術後長期預後良好。約克夏㹴預後通常佳。',
        feedback_incorrect: '約 50% 犬術後膽汁酸不完全正常化但臨床無症狀為可接受結果。體重追趕正常為良好指標。無 HE 症狀時可漸進恢復正常飲食和停藥。不需再次手術。',
        key_learning_points: [
          '約 50% PSS 犬術後膽汁酸不完全正常化但臨床良好——可接受',
          '體重追趕正常為手術成功的直觀指標',
          '無 HE 症狀時可漸進恢復正常蛋白飲食',
          'Lactulose 和抗生素在膽汁酸改善後可逐步停藥',
          '約 85-90% 犬術後長期預後良好',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '8 月齡約克夏㹴 + 生長遲緩 + 餐後神經症狀 → 高度懷疑先天性 PSS。膽汁酸極度升高 + 小肝症 + CTA 確認肝外單一分流 → 先內科穩定控制 HE → 手術漸進閉合（Ameroid constrictor）→ 術後監測低血糖和門脈高壓 → 3 個月追蹤膽汁酸改善 → 漸進恢復正常飲食。',
      key_decisions: [
        '餐後神經症狀 + 約克夏㹴 → 立即想到 PSS',
        '膽汁酸為篩檢首選、CTA 為解剖定位金標準',
        '術前內科穩定控制 HE 降低手術風險',
        '選擇漸進閉合而非急性完全結紮',
      ],
      common_pitfalls: [
        '將餐後神經症狀誤診為原發性癲癇而使用抗癲癇藥',
        '急性完全結紮分流血管導致致命門脈高壓',
        '術前未穩定 HE 即手術增加麻醉風險',
        '忽略尿酸銨結石的評估和處理',
        '術後未追蹤膽汁酸評估分流閉合效果',
      ],
    },
    related_nodes_for_review: ['SURG-L3-011', 'SURG-L2-001', 'SURG-L4-002', 'IM-L3-002', 'CPATH-L3-001'],
    tags: ['PSS', '門體分流', '約克夏', '肝性腦病', 'ameroid constrictor', 'case_challenge'],
  },

  // --- CASE-SURG-010: Pyometra ---
  {
    id: 'CASE-SURG-010',
    title: '未絕育黃金獵犬多飲多尿與陰道分泌物 — 子宮蓄膿',
    specialty: 'SURG',
    cross_specialties: ['IM', 'ECC'],
    required_nodes: ['SURG-L3-009', 'SURG-L4-001', 'SURG-L5-001'],
    difficulty: 3,
    species: '犬',
    scenario: {
      signalment: '8 歲未絕育雌性黃金獵犬，體重 32 kg',
      chief_complaint: '嗜睡、多飲多尿、陰道有膿性分泌物，已持續 3 天',
      brief_history: '飼主報告約 6 週前結束發情期。3 天前開始食慾下降、明顯嗜睡、飲水量增加約 2-3 倍、頻尿。昨天注意到陰道有黃綠色分泌物。過去未曾懷孕，從未絕育。疫苗與驅蟲定期。無其他用藥史。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '這隻 8 歲未絕育黃金獵犬因嗜睡與陰道分泌物就診。你需要進一步確認哪些資訊？請選擇最重要的 3 項。',
        available_actions: [
          '確認最後一次發情的時間（距今幾週）',
          '詢問分泌物的顏色、氣味與量',
          '詢問是否有嘔吐或腹瀉',
          '詢問平時的飲食品牌',
          '詢問是否曾使用外源性黃體素或雌激素',
          '詢問最近是否有出門交配',
        ],
        correct_actions: [
          '確認最後一次發情的時間（距今幾週）',
          '詢問分泌物的顏色、氣味與量',
          '詢問是否曾使用外源性黃體素或雌激素',
        ],
        feedback_correct: '優秀！子宮蓄膿典型在發情後 4-8 週（黃體期 diestrus）發生，確認發情時間有助建立時間線。分泌物特徵區分 open vs closed cervix pyometra。外源性荷爾蒙使用（如亂經針）是 pyometra 的重要危險因子。',
        feedback_incorrect: 'Pyometra 病史的核心：1) 發情時間線（黃體期 progesterone 主導下子宮內膜增生→感染），2) 分泌物特徵（open cervix 可見分泌物，closed 則無），3) 荷爾蒙暴露史（外源性 progesterone/estrogen 增加風險）。',
        key_learning_points: [
          'Pyometra 典型發生於發情後 4-8 週（diestrus phase）',
          'Open cervix pyometra 有陰道分泌物，closed cervix 則無（更危險）',
          '外源性 progesterone/estrogen 是重要危險因子',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '補充病史：約 6 週前結束發情，分泌物為黃綠色膿性、有異味、量中等。無外源性荷爾蒙使用。請選擇最關鍵的理學檢查發現。',
        available_actions: [
          '腹部觸診可感受到子宮增大，呈管狀結構',
          '體溫 39.8°C（微燒），心搏過速（HR 140 bpm）',
          '黏膜偏乾、CRT 2.5 秒，皮膚帳篷試驗延遲（脫水）',
          '體重 32 kg 無明顯變化',
          '後肢反射正常',
          '外陰腫大且有膿性分泌物滲出',
        ],
        correct_actions: [
          '腹部觸診可感受到子宮增大，呈管狀結構',
          '體溫 39.8°C（微燒），心搏過速（HR 140 bpm）',
          '黏膜偏乾、CRT 2.5 秒，皮膚帳篷試驗延遲（脫水）',
        ],
        feedback_correct: '正確！腹部觸診感受到增大的管狀子宮是 pyometra 的重要理學檢查發現（注意：closed cervix 時子宮極度膨脹，觸診需輕柔避免破裂）。發燒與心搏過速提示全身性感染反應（SIRS）。脫水是 PU/PD 合併食慾下降的結果，需積極輸液矯正。',
        feedback_incorrect: 'Pyometra 的關鍵理學檢查：腹部觸診子宮增大（需輕柔！）、SIRS 徵候（發燒/心搏過速）、脫水程度評估。這三項決定手術的緊急程度與術前穩定化的需求。',
        key_learning_points: [
          '觸診增大子宮時需輕柔——closed pyometra 有破裂風險',
          '發燒 + 心搏過速 = 全身性感染反應（SIRS），提示需緊急處置',
          'PU/PD 機制：E. coli 內毒素干擾腎小管 ADH 受體 → 腎原性尿崩',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '診斷計畫',
        prompt: '高度懷疑 open cervix pyometra。你會安排哪些檢查？請選擇最適當的 3 項。',
        available_actions: [
          '腹部超音波（評估子宮大小、內容物、壁厚度）',
          'CBC + 生化（BUN/Cre 評估腎功能）+ 電解質',
          '腹部 X 光（評估子宮大小與位置）',
          '陰道細胞學與培養',
          '腹腔穿刺',
          '凝血功能（PT/aPTT）',
        ],
        correct_actions: [
          '腹部超音波（評估子宮大小、內容物、壁厚度）',
          'CBC + 生化（BUN/Cre 評估腎功能）+ 電解質',
          '凝血功能（PT/aPTT）',
        ],
        feedback_correct: '完美！超音波是確診 pyometra 的首選影像——可直接觀察子宮角膨脹、含液體腔室、壁厚度（壁薄提示破裂風險）。CBC 預期嗜中性球 left shift，BUN/Cre 評估 endotoxin 引起的腎損傷。凝血功能評估 DIC 風險（pyometra 為 DIC 的重要誘因之一）。',
        feedback_incorrect: '超音波優於 X 光——可直接評估子宮內容物與壁完整性。CBC/生化/凝血是術前必要評估：嗜中性球毒性變化評估感染嚴重度、BUN/Cre 評估腎功能、凝血篩檢 DIC。',
        key_learning_points: [
          '超音波是 pyometra 首選影像學檢查',
          'Pyometra 典型 CBC：嗜中性球增多伴 left shift 與毒性變化',
          'E. coli endotoxin 可引起 AKI 與 DIC',
          '術前 BUN/Cre 評估對預後判斷至關重要',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '檢查結果判讀',
        prompt: '檢查結果如下：\n\n- 超音波：雙側子宮角明顯膨脹（直徑約 3-4 cm），內含均質無回音液體，子宮壁厚度正常，無游離腹水\n- CBC：WBC 38,000/µL（嗜中性球 32,000 伴 band 形增多與毒性變化），PCV 38%\n- 生化：BUN 42 mg/dL（↑），Cre 1.8 mg/dL（輕度↑），ALT 正常，Glucose 正常\n- 電解質：正常範圍\n- 凝血：PT/aPTT 正常\n\n如何解讀？',
        available_actions: [
          '確診 open cervix pyometra。顯著嗜中性球增多伴 left shift 反映嚴重子宮感染。BUN/Cre 輕度升高為 endotoxin 相關 pre-renal/renal AKI，需積極輸液矯正後再評估。凝血正常排除目前 DIC。建議輸液穩定後緊急 OVH',
          'BUN/Cre 升高代表慢性腎病，不適合手術',
          '凝血正常可直接手術，不需輸液',
          '建議先用抗生素保守治療 1 週觀察',
        ],
        correct_actions: [
          '確診 open cervix pyometra。顯著嗜中性球增多伴 left shift 反映嚴重子宮感染。BUN/Cre 輕度升高為 endotoxin 相關 pre-renal/renal AKI，需積極輸液矯正後再評估。凝血正常排除目前 DIC。建議輸液穩定後緊急 OVH',
        ],
        feedback_correct: '正確！超音波確認 pyometra。WBC 38,000 伴 band 形與毒性變化反映嚴重感染。BUN/Cre 升高在 pyometra 常見——主要為 pre-renal（脫水）合併 endotoxin 對腎小管的損傷。輸液矯正脫水後 BUN/Cre 通常改善。凝血正常是手術的有利條件。Open cervix pyometra 雖然較 closed 安全，但仍需緊急手術。',
        feedback_incorrect: 'Pyometra 的 BUN/Cre 升高多為可逆性（pre-renal + endotoxin AKI），輸液後通常改善，不應因此延遲手術。保守治療僅在種犬需保留繁殖力時考慮（aglepristone 療法），且僅限 open cervix。',
        key_learning_points: [
          'Pyometra 的氮血症多為可逆性：脫水（pre-renal）+ endotoxin AKI',
          '嗜中性球 left shift 伴毒性變化 = 嚴重感染的血液學指標',
          'Open cervix pyometra 仍需緊急手術——分泌物引流不代表自癒',
          '凝血功能正常是手術安全性的重要確認',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術計畫與術後管理',
        prompt: '經 4 小時積極輸液後，BUN 降至 35，Cre 1.5。血壓穩定。準備手術。手術與術後管理應如何安排？',
        available_actions: [
          '緊急 OVH：正中開腹 → 雙重結紮卵巢動脈與子宮動脈 → 完整移除雙側卵巢與子宮 → 避免子宮內容物溢漏 → 術後繼續靜脈輸液 + IV 抗生素（ampicillin-sulbactam）→ 48 小時後追蹤 BUN/Cre',
          '先嘗試經陰道灌洗引流，手術作為備案',
          'OVH 但僅移除子宮體，保留卵巢以維持荷爾蒙功能',
          '使用 aglepristone（抗黃體素）保守治療',
        ],
        correct_actions: [
          '緊急 OVH：正中開腹 → 雙重結紮卵巢動脈與子宮動脈 → 完整移除雙側卵巢與子宮 → 避免子宮內容物溢漏 → 術後繼續靜脈輸液 + IV 抗生素（ampicillin-sulbactam）→ 48 小時後追蹤 BUN/Cre',
        ],
        feedback_correct: '完美！緊急 OVH 是 pyometra 的標準治療。手術重點：(1) 雙重結紮確保止血安全；(2) 完整移除卵巢——殘留卵巢組織（ovarian remnant）可導致復發；(3) 子宮操作需輕柔，避免內容物溢漏污染腹腔；(4) 術後抗生素建議 ampicillin-sulbactam 或 amoxicillin-clavulanate（覆蓋 E. coli 為主的感染）；(5) 持續輸液支持腎灌流。大多數犬在 OVH 後 48-72 小時明顯改善。',
        feedback_incorrect: 'OVH 是 pyometra 的金標準治療。保留卵巢（僅子宮切除）不可接受——progesterone 持續分泌會導致殘端蓄膿。Aglepristone 保守治療僅限年輕種犬需保留繁殖力的 open cervix 案例，且復發率約 20%。經陰道灌洗不是標準做法。',
        key_learning_points: [
          'OVH（非單獨子宮切除）是 pyometra 的標準手術——必須移除雙側卵巢',
          '術中避免子宮破裂與內容物溢漏是降低腹膜炎風險的關鍵',
          'E. coli 是 pyometra 最常見的致病菌（約 70%），抗生素選擇需覆蓋',
          '術後 BUN/Cre 通常在 48-72 小時內改善',
          'Aglepristone 保守治療：僅限需保留繁殖力的 open cervix 案例，復發率約 20%',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '8 歲未絕育黃金獵犬 + 發情後 6 週 + PU/PD + 陰道膿性分泌物 → 典型 open cervix pyometra。超音波確診、CBC 顯示嚴重感染、BUN/Cre 輕度升高為 endotoxin 相關 AKI。輸液穩定後緊急 OVH，術後抗生素與腎功能追蹤。',
      key_decisions: [
        '發情後 6 週 + PU/PD + 分泌物 = 立即想到 pyometra',
        '超音波確診並評估子宮壁完整性',
        '術前輸液矯正脫水與 pre-renal azotemia',
        '緊急 OVH 而非保守治療',
        '術中避免子宮內容物溢漏',
      ],
      common_pitfalls: [
        '將 PU/PD 歸因於慢性腎病而延遲 pyometra 診斷',
        'Closed cervix pyometra 因無分泌物而被忽略',
        '未評估凝血功能即手術，忽略 DIC 風險',
        '僅切除子宮而保留卵巢導致殘端蓄膿',
        '保守治療延誤導致子宮破裂與敗血性腹膜炎',
      ],
    },
    related_nodes_for_review: ['SURG-L3-009', 'SURG-L4-001', 'SURG-L5-001', 'SURG-L2-001', 'ECC-L3-001'],
    tags: ['pyometra', '子宮蓄膿', '黃金獵犬', 'OVH', 'endotoxemia', 'case_challenge'],
  },

  // --- CASE-SURG-011: GI Foreign Body ---
  {
    id: 'CASE-SURG-011',
    title: '幼犬急性嘔吐與線狀異物 — 腸道異物',
    specialty: 'SURG',
    cross_specialties: ['IM', 'ECC'],
    required_nodes: ['SURG-L3-003', 'SURG-L3-015', 'SURG-L4-001', 'SURG-L5-001'],
    difficulty: 4,
    species: '犬',
    scenario: {
      signalment: '1 歲已絕育雄性拉布拉多，體重 28 kg',
      chief_complaint: '急性反覆嘔吐 24 小時，昨晚開始嘔吐清液，今日完全不吃',
      brief_history: '飼主報告該犬有啃咬玩具與咬布的習慣。2 天前曾看到牠拉扯一條繩索玩具，但未注意到是否吞食。昨晚開始嘔吐約 6 次（先食物後清液），今日完全拒食、嗜睡。排便正常至昨天，今日無排便。無先前類似病史。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集',
        prompt: '這隻 1 歲拉布拉多因急性嘔吐就診。你需要進一步確認哪些資訊？請選擇最重要的 3 項。',
        available_actions: [
          '詳問可能吞食的物品類型、大小與時間',
          '確認嘔吐的型態（進食後 vs 非進食相關、有無膽汁）',
          '詢問是否有能接觸到的異物（垃圾桶、襪子、布料等）',
          '詢問最近是否換過飼料',
          '詢問是否有接觸其他狗',
          '詢問排尿量是否正常',
        ],
        correct_actions: [
          '詳問可能吞食的物品類型、大小與時間',
          '確認嘔吐的型態（進食後 vs 非進食相關、有無膽汁）',
          '詢問是否有能接觸到的異物（垃圾桶、襪子、布料等）',
        ],
        feedback_correct: '正確！拉布拉多（異物吞食高風險品種）+ 繩索玩具暴露史 → 異物吞食高度懷疑。嘔吐型態提供定位線索：近端阻塞（胃/十二指腸）嘔吐頻繁且含膽汁，遠端阻塞（空迴腸）可能更晚出現、含糞味。異物暴露評估有助判斷線狀 vs 非線狀異物。',
        feedback_incorrect: '1 歲拉布拉多急性嘔吐的首要鑑別是異物。病史重點：吞食物品細節（線狀 vs 非線狀影響手術決策）、嘔吐型態（定位）、環境中異物暴露風險。',
        key_learning_points: [
          '拉布拉多、黃金獵犬、鬥牛犬是異物吞食最高風險品種',
          '線狀異物（繩索、線、布條）最危險——可導致多處腸道 plication 與穿孔',
          '嘔吐型態有助定位：近端阻塞嘔吐早且頻繁，遠端較晚',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '病史補充：飼主確認 2 天前玩繩索玩具後，該玩具的一段繩索不見了（約 30 cm）。嘔吐為非進食相關，先食物後清液。請選擇最關鍵的理學檢查發現。',
        available_actions: [
          '腹部觸診有多處腸道團聚感（bunching），觸診時犬表現疼痛',
          '體溫 39.2°C，心搏過速（HR 130 bpm）',
          '脫水約 5-7%（黏膜偏乾、CRT 2 秒）',
          '舌下檢查：舌繫帶處可見一段繩索纏繞',
          '腸音減少',
          '後肢正常行走',
        ],
        correct_actions: [
          '腹部觸診有多處腸道團聚感（bunching），觸診時犬表現疼痛',
          '體溫 39.2°C，心搏過速（HR 130 bpm）',
          '舌下檢查：舌繫帶處可見一段繩索纏繞',
        ],
        feedback_correct: '優秀！腹部 bunching（腸道皺摺團聚）是線狀異物的特徵性觸診發現——線狀物固定於近端錨點後，腸蠕動將腸壁沿線收縮。舌下繫帶纏繞是線狀異物的經典錨點（另一常見錨點是幽門）。微燒與心搏過速提示可能的腸壁損傷或早期腹膜炎。',
        feedback_incorrect: '線狀異物的三大理學檢查發現：(1) 腸道 bunching/plication（特徵性），(2) 舌下檢查——必須檢查舌繫帶是否有線狀物纏繞，(3) 全身感染/炎症徵候（可能腸壁穿孔）。',
        key_learning_points: [
          '腸道 plication/bunching 是線狀異物的特徵性觸診發現',
          '線狀異物必須檢查舌繫帶——常為近端錨點',
          '絕對不可拉扯可見的線狀物——可能切割已 plicated 的腸壁',
          '貓的線狀異物錨點通常在舌繫帶，犬在幽門',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '影像學評估',
        prompt: '高度懷疑線狀異物。你會安排哪些影像學檢查？請選擇最適當的 3 項。',
        available_actions: [
          '腹部 X 光（VD + lateral）：評估腸道氣體分佈、plication pattern、游離腹腔氣體',
          'CBC + 生化 + 電解質（術前評估）',
          '腹部超音波（評估腸道蠕動、壁厚度、游離液體）',
          '鋇劑上消化道造影',
          'CT 掃描',
          '糞便檢查',
        ],
        correct_actions: [
          '腹部 X 光（VD + lateral）：評估腸道氣體分佈、plication pattern、游離腹腔氣體',
          'CBC + 生化 + 電解質（術前評估）',
          '腹部超音波（評估腸道蠕動、壁厚度、游離液體）',
        ],
        feedback_correct: '完美！X 光是第一線影像——線狀異物典型表現：多處小腸節段聚集在腹部中央呈「comma shape」或 plication pattern，不均勻氣體分佈，可能可見游離腹腔氣體（穿孔徵象）。超音波可進一步評估腸壁厚度、蠕動、游離液體（腹膜炎徵候）、偶可見異物本身。術前血檢評估電解質紊亂（嘔吐導致低鉀低氯代謝性鹼中毒）。',
        feedback_incorrect: 'X 光 + 超音波是互補的影像評估。鋇劑造影在懷疑穿孔時禁忌（加重腹膜炎），且線狀異物通常不需要造影就可診斷。術前電解質評估對麻醉安全至關重要。',
        key_learning_points: [
          '線狀異物 X 光特徵：腸道 plication、comma-shaped gas pattern、腹腔中央小腸聚集',
          '游離腹腔氣體 = 穿孔，需緊急手術',
          '嘔吐導致的低鉀低氯代謝性鹼中毒需術前矯正',
          '懷疑穿孔時禁用鋇劑造影',
        ],
      },
      {
        step_number: 4,
        step_type: 'result_interpretation',
        title: '檢查結果判讀',
        prompt: '檢查結果：\n\n- X 光：多處小腸節段聚集在腹部中央偏左，呈 plication pattern。腸管內不均勻氣體分佈，部分腸段擴張。無明確游離腹腔氣體。\n- 超音波：多段小腸壁增厚（4-5 mm），腸道蠕動減弱。少量游離腹腔液體。一段小腸腔內可見線狀高回音結構。\n- CBC：WBC 22,000/µL（嗜中性球增多伴輕度 left shift）\n- 生化：BUN 正常，K+ 3.2 mEq/L（↓），Cl⁻ 96 mEq/L（↓）\n- 凝血：正常\n\n如何解讀？',
        available_actions: [
          '線狀異物合併腸道 plication 確診。超音波顯示腸壁增厚與少量游離液體提示早期腸壁損傷但尚未穿孔。低鉀低氯需術前矯正。建議緊急探查手術',
          '無游離氣體代表沒有穿孔，可以等待自然排出',
          '僅有少量腹水不嚴重，可先保守觀察',
          '建議使用催吐藥試圖排出異物',
        ],
        correct_actions: [
          '線狀異物合併腸道 plication 確診。超音波顯示腸壁增厚與少量游離液體提示早期腸壁損傷但尚未穿孔。低鉀低氯需術前矯正。建議緊急探查手術',
        ],
        feedback_correct: '正確！影像學確認線狀異物伴 plication。腸壁增厚提示炎症/缺血但尚未穿孔（無游離氣體）。少量腹水可能為漿膜面滲出——需視為早期腹膜炎前兆。低鉀（3.2）需術前用 KCl 補充（目標 > 3.5），低氯常伴隨嘔吐。線狀異物絕不可催吐或拉扯——腸道 plication 狀態下催吐可造成多處穿孔。',
        feedback_incorrect: '線狀異物不會自然排出。無游離氣體不代表腸壁完好——腸壁增厚和腹水是早期損傷的徵兆。催吐在任何腸道異物中都是禁忌（尤其線狀異物——加重 plication 切割）。',
        key_learning_points: [
          '線狀異物絕對禁忌催吐與經口拉扯',
          '腸壁增厚 + 少量腹水 = 早期腸壁損傷，需緊急手術',
          '低鉀 < 3.5 mEq/L 需術前矯正——低鉀增加 ileus 與心律不整風險',
          '線狀異物無自然排出的可能——plication 只會隨蠕動加重',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '手術決策與技術',
        prompt: '電解質矯正後進入手術室。打開腹腔後確認線狀異物從幽門延伸至遠端空腸，造成 4 處明顯 plication。2 處腸壁有漿膜面暗紅色變（缺血徵兆）但未穿孔。你的手術策略是什麼？',
        available_actions: [
          '先切斷舌下錨點，再於多處 plication 進行 enterotomy 分段取出線狀物。評估 2 處缺血腸壁——若漿膜面恢復顏色則保留；若缺血無法恢復則進行該段腸道 resection-anastomosis',
          '從幽門做單一 gastrotomy 將整條線一次拉出',
          '僅做一處 enterotomy 即可取出所有線',
          '所有 plication 處全部做 resection-anastomosis',
        ],
        correct_actions: [
          '先切斷舌下錨點，再於多處 plication 進行 enterotomy 分段取出線狀物。評估 2 處缺血腸壁——若漿膜面恢復顏色則保留；若缺血無法恢復則進行該段腸道 resection-anastomosis',
        ],
        feedback_correct: '完美！線狀異物手術的核心原則：(1) 切斷錨點——解除線的張力；(2) 多處 enterotomy——在每個 plication 節點切開腸壁取出線段，不可從單一處拉扯（會切割 mesenteric border 的腸壁）；(3) 腸壁活性評估——漿膜面顏色、蠕動、mesenteric 血流。缺血腸壁以溫生理食鹽水覆蓋 5 分鐘後再評估。無法恢復的節段才做 R&A。保留所有可行的腸段以避免短腸症候群。',
        feedback_incorrect: '從單一處拉出整條線是嚴重錯誤——線已嵌入 plication 的 mesenteric border，拉扯會造成多處穿孔。必須在每個 plication 點做 enterotomy 分段取出。不必要的 R&A 增加短腸風險。',
        key_learning_points: [
          '線狀異物手術：切斷錨點 → 多處 enterotomy → 分段取出 → 評估腸壁活性',
          '絕不可從單一處拉出整條線——mesenteric border 切割風險',
          '腸壁活性評估：溫鹽水覆蓋 5 分鐘後評估顏色、蠕動、出血',
          '僅無法恢復的缺血腸段需 resection-anastomosis',
          '線狀異物比單一異物預後差——約 60-80% 需多處 enterotomy',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '1 歲拉布拉多 + 繩索玩具暴露 + 急性嘔吐 + 腸道 plication 觸診 + 舌下繩索錨點 → 線狀異物。X 光 plication pattern + 超音波腸壁增厚 + 腹水 → 早期腸壁損傷。矯正低鉀後緊急手術。多處 enterotomy 分段取出，評估缺血腸壁。',
      key_decisions: [
        '拉布拉多 + 繩索玩具暴露 → 立即考慮線狀異物',
        '舌下檢查發現錨點是關鍵診斷步驟',
        '影像學確認 plication 並排除穿孔',
        '術前矯正電解質紊亂',
        '多處 enterotomy 而非單一處拉出',
      ],
      common_pitfalls: [
        '忽略舌下檢查而漏掉線狀異物錨點',
        '從單一處拉扯線狀物造成多處穿孔',
        '未矯正低鉀即麻醉導致術中心律不整',
        '催吐嘗試排出線狀異物',
        '不必要的全段腸切除導致短腸症候群',
      ],
    },
    related_nodes_for_review: ['SURG-L3-003', 'SURG-L3-015', 'SURG-L4-001', 'SURG-L5-001', 'SURG-L2-001'],
    tags: ['linear_foreign_body', '線狀異物', '拉布拉多', 'enterotomy', 'plication', 'case_challenge'],
  },

  // --- CASE-SURG-012: Wound Management ---
  {
    id: 'CASE-SURG-012',
    title: '犬咬傷合併組織缺損 — 開放性傷口管理',
    specialty: 'SURG',
    cross_specialties: ['ECC'],
    required_nodes: ['SURG-L1-002', 'SURG-L5-001'],
    difficulty: 3,
    species: '犬',
    scenario: {
      signalment: '4 歲已絕育雄性混種犬，體重 18 kg',
      chief_complaint: '被大型犬攻擊，右側胸腹部與前肢有多處咬傷與撕裂傷',
      brief_history: '約 2 小時前在公園被一隻未繫繩的大型犬攻擊。飼主報告傷口有明顯出血，已用毛巾加壓止血。犬隻目前可行走但右前肢跛行。疫苗與驅蟲定期。無已知基礎疾病。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集與初步分級',
        prompt: '這隻犬因狗咬傷被緊急帶入。你需要進一步確認哪些資訊？請選擇最重要的 3 項。',
        available_actions: [
          '確認受傷時間（距就診多久）與攻擊犬的大小',
          '詢問傷口出血是否已控制、出血量估計',
          '確認狂犬病疫苗接種狀態（雙方犬隻）',
          '詢問犬的飲食品牌',
          '詢問最近是否洗過澡',
          '詢問是否有過敏藥物史',
        ],
        correct_actions: [
          '確認受傷時間（距就診多久）與攻擊犬的大小',
          '詢問傷口出血是否已控制、出血量估計',
          '確認狂犬病疫苗接種狀態（雙方犬隻）',
        ],
        feedback_correct: '正確！受傷時間影響傷口分類——6 小時內為「新鮮傷口」，> 6 小時為「延遲就醫」，> 12 小時視為已污染。攻擊犬大小預估 crush injury 嚴重度。出血評估決定是否需優先止血。狂犬病疫苗狀態在犬咬傷中是法律與公衛要求。',
        feedback_incorrect: '咬傷急診的病史重點：時間（影響傷口分類）、出血（決定優先處置）、狂犬病（法律與公衛）。攻擊犬大小可預估深層組織損傷程度——大型犬咬合力可達 200+ psi。',
        key_learning_points: [
          '傷口時間分類：< 6h 新鮮、6-12h 延遲、> 12h 已污染/感染',
          '大型犬咬合力可造成深層 crush injury——表面傷口可能遠小於深層損傷',
          '犬咬傷的「冰山效應」：可見傷口僅為實際損傷的一小部分',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查與傷口評估',
        prompt: '受傷約 2 小時，出血已用加壓暫時控制。攻擊犬為大型羅威納。請進行理學檢查。選擇最關鍵的發現。',
        available_actions: [
          '右胸壁有 3 處穿刺傷（各約 1-2 cm），右側腹部有一處 8x5 cm 撕裂傷伴皮瓣剝離（avulsion flap），皮下組織外露',
          '右前肢有 2 處穿刺傷，觸診右前肢骨骼完整無捻發音，關節活動度正常',
          '呼吸平穩，雙側胸壁聽診肺音正常，無皮下氣腫',
          '心搏略快（HR 120 bpm），黏膜粉紅、CRT 1.5 秒',
          '體溫 38.7°C',
          '左側完全正常',
        ],
        correct_actions: [
          '右胸壁有 3 處穿刺傷（各約 1-2 cm），右側腹部有一處 8x5 cm 撕裂傷伴皮瓣剝離（avulsion flap），皮下組織外露',
          '右前肢有 2 處穿刺傷，觸診右前肢骨骼完整無捻發音，關節活動度正常',
          '呼吸平穩，雙側胸壁聽診肺音正常，無皮下氣腫',
        ],
        feedback_correct: '正確！胸壁穿刺傷必須排除胸腔穿透（皮下氣腫、肺音減弱、呼吸困難）。大面積 avulsion flap 是狗咬傷最常見的嚴重軟組織損傷型態。前肢穿刺傷需排除骨折與關節穿透。心血管穩定（CRT 正常）表示目前無重大出血。',
        feedback_incorrect: '犬咬傷理學檢查優先級：(1) 胸壁穿透評估——氣胸可致命；(2) 傷口全面盤點——分類每處傷口；(3) 骨骼與關節完整性。心血管評估決定是否需先穩定再處理傷口。',
        key_learning_points: [
          '胸壁咬傷必須排除穿透性胸腔損傷（皮下氣腫、氣胸）',
          'Avulsion flap（皮瓣剝離）是犬咬傷最常見的軟組織損傷型態',
          '冰山效應：穿刺傷口徑小，但深層 crush injury 範圍遠大於表面',
          '骨折與關節穿透的排除對四肢咬傷至關重要',
        ],
      },
      {
        step_number: 3,
        step_type: 'differential_diagnosis',
        title: '傷口分類與初步處理決策',
        prompt: '根據傷口評估，如何分類這些傷口並決定初步處理策略？請選擇最適當的 3 項。',
        available_actions: [
          '分類為 Class III 污染傷口（contaminated）：犬口腔多種細菌（Pasteurella、厭氧菌），需清創但不適合一期閉合',
          '所有傷口需全身麻醉下徹底清創、大量沖洗（lavage ≥ 500 mL/cm 傷口）',
          '開始廣效抗生素（amoxicillin-clavulanate 20 mg/kg BID）覆蓋需氧菌與厭氧菌',
          '所有傷口立即縫合（primary closure）加速癒合',
          '僅用優碘消毒即可',
          '不需要全身抗生素，局部抗生素軟膏就夠',
        ],
        correct_actions: [
          '分類為 Class III 污染傷口（contaminated）：犬口腔多種細菌（Pasteurella、厭氧菌），需清創但不適合一期閉合',
          '所有傷口需全身麻醉下徹底清創、大量沖洗（lavage ≥ 500 mL/cm 傷口）',
          '開始廣效抗生素（amoxicillin-clavulanate 20 mg/kg BID）覆蓋需氧菌與厭氧菌',
        ],
        feedback_correct: '正確！犬咬傷一律視為 Class III 污染傷口（犬口腔含 Pasteurella、Streptococcus、Staphylococcus、厭氧菌等 100+ 菌種）。清創原則：「debridement by layer」——逐層移除壞死與無活性組織，保留所有有血流的活組織。大量沖洗（溫生理食鹽水 + 7 psi 壓力沖洗）是降低細菌量最有效的方法。咬傷不適合一期閉合（感染風險太高），應採開放管理或延遲閉合。',
        feedback_incorrect: '咬傷立即縫合（primary closure）是最常見的錯誤——將細菌封在組織內導致膿瘍或蜂窩性組織炎。犬口腔含多種需氧與厭氧菌，全身抗生素是必要的。優碘不足以處理深層污染。',
        key_learning_points: [
          '犬咬傷 = Class III 污染傷口，禁忌一期閉合',
          '清創原則：保留有血流組織，移除壞死組織（「debridement by layer」）',
          '沖洗壓力 7 psi（18G 針頭 + 35 mL 注射器）為最佳清創沖洗壓力',
          'Amoxicillin-clavulanate 是咬傷首選抗生素（覆蓋 Pasteurella + 厭氧菌）',
        ],
      },
      {
        step_number: 4,
        step_type: 'diagnostic_plan',
        title: '傷口管理計畫',
        prompt: '清創與沖洗完成。腹部 avulsion flap 經清創後仍有約 6x4 cm 的開放傷口，皮下組織健康但皮膚缺損無法直接閉合。穿刺傷已清創開放引流。接下來的傷口管理策略？',
        available_actions: [
          '大面積 avulsion 傷口使用 wet-to-dry bandage 管理。每 12-24 小時換藥，目標 3-5 天後形成健康肉芽組織，再進行延遲一期閉合（delayed primary closure）或二期閉合',
          '穿刺傷放置 Penrose drain 引流 3-5 天，減少死腔膿瘍形成',
          '考慮使用負壓傷口治療（VAC therapy）加速肉芽組織形成',
          '立即用游離皮瓣覆蓋所有傷口',
          '僅用繃帶包紮不換藥等 1 週',
          '使用蜂蜜敷料（honey dressing）做為唯一治療',
        ],
        correct_actions: [
          '大面積 avulsion 傷口使用 wet-to-dry bandage 管理。每 12-24 小時換藥，目標 3-5 天後形成健康肉芽組織，再進行延遲一期閉合（delayed primary closure）或二期閉合',
          '穿刺傷放置 Penrose drain 引流 3-5 天，減少死腔膿瘍形成',
          '考慮使用負壓傷口治療（VAC therapy）加速肉芽組織形成',
        ],
        feedback_correct: '正確！開放傷口管理的核心是「wound bed preparation」：(1) Wet-to-dry bandage 提供持續清創——濕紗布乾燥後移除時帶走壞死組織與滲出液；(2) 穿刺傷的死腔用 Penrose drain 引流，避免膿瘍；(3) VAC therapy 加速肉芽組織生長並減少水腫。延遲一期閉合在 3-5 天後評估——當傷口床為健康粉紅色肉芽組織且無感染徵象時即可閉合。',
        feedback_incorrect: '立即皮瓣在污染傷口上進行將導致感染失敗。「先開放管理→肉芽形成→再閉合」是污染傷口的黃金法則。不換藥等 1 週會導致感染惡化。',
        key_learning_points: [
          'Delayed primary closure（延遲一期閉合）：3-5 天內，肉芽形成前閉合',
          'Second intention healing（二期癒合）：讓傷口自行收縮與上皮化',
          'Wet-to-dry bandage 提供機械性清創效果',
          'VAC therapy 加速肉芽組織生長（-125 mmHg 負壓）',
          'Penrose drain 引流深層穿刺傷的死腔',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '傷口閉合決策',
        prompt: '第 4 天回診：穿刺傷引流量減少，已移除 Penrose drain。腹部大面積傷口換藥後已見健康紅色肉芽組織填充，無感染徵象（無膿性分泌物、無異味、傷口邊緣無紅腫擴展）。體溫正常。如何決定閉合方式？',
        available_actions: [
          '健康肉芽組織 + 無感染 = 可進行延遲一期閉合。評估皮膚鬆弛度——若可直接對合則 walking suture + 皮膚縫合；若張力太大則需推進皮瓣（advancement flap）或亞甲基藍引導的旋轉皮瓣',
          '繼續開放管理讓傷口完全以二期癒合自行閉合',
          '肉芽組織已形成，可停止抗生素與換藥',
          '此時不適合閉合，需等到完全無肉芽才能縫',
        ],
        correct_actions: [
          '健康肉芽組織 + 無感染 = 可進行延遲一期閉合。評估皮膚鬆弛度——若可直接對合則 walking suture + 皮膚縫合；若張力太大則需推進皮瓣（advancement flap）或亞甲基藍引導的旋轉皮瓣',
        ],
        feedback_correct: '正確！第 3-5 天是延遲一期閉合的最佳時機——肉芽組織提供血管基底、細菌量已降低、傷口邊緣尚未過度纖維化。閉合技巧：walking sutures（皮下組織推進縫合）消除死腔並減少皮膚張力。若皮膚缺損太大，使用 subdermal plexus flap（推進或旋轉皮瓣）。術後繼續抗生素至臨床痊癒後 7 天。完全以二期癒合雖可行但耗時數週且留下大面積疤痕。',
        feedback_incorrect: '3-5 天健康肉芽組織形成後即為延遲閉合的最佳時機。完全二期癒合對大面積傷口耗時太長（4-6 週+）。肉芽組織是閉合的理想基底，而非阻礙。抗生素不可在傷口尚未癒合時停止。',
        key_learning_points: [
          '延遲一期閉合（delayed primary closure）在傷後 3-5 天執行最佳',
          'Walking sutures 是減少皮膚張力與死腔的關鍵縫合技巧',
          '大面積缺損可用推進皮瓣或旋轉皮瓣覆蓋',
          '抗生素持續至臨床痊癒後 7-14 天',
          '犬咬傷癒合後整體預後良好——大多數犬功能完全恢復',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '4 歲混種犬大型犬咬傷 → Class III 污染傷口。排除胸腔穿透後，全身麻醉下清創沖洗。穿刺傷開放引流，大面積 avulsion 以 wet-to-dry bandage 管理。3-5 天後健康肉芽形成，延遲一期閉合。抗生素持續至癒合。',
      key_decisions: [
        '優先排除胸腔穿透與骨骼損傷',
        '咬傷 = 污染傷口 → 禁忌一期閉合',
        '徹底清創沖洗是最重要的治療步驟',
        '穿刺傷引流 vs avulsion 開放管理的不同策略',
        '延遲一期閉合的時機判斷（3-5 天健康肉芽）',
      ],
      common_pitfalls: [
        '立即一期閉合咬傷導致嚴重感染',
        '忽略深層 crush injury（冰山效應）',
        '未排除胸腔穿透就專注於皮膚傷口',
        '清創不足——保留了無活性組織',
        '抗生素選擇未覆蓋厭氧菌',
      ],
    },
    related_nodes_for_review: ['SURG-L1-002', 'SURG-L5-001', 'SURG-L2-001', 'ECC-L3-001'],
    tags: ['bite_wound', '咬傷', '傷口管理', 'delayed_closure', 'avulsion', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-013',
    title: '公貓反覆尿道阻塞 — 會陰尿道造口術',
    specialty: 'SURG',
    cross_specialties: ['IM', 'ECC'],
    required_nodes: ['SURG-L3-016', 'SURG-L4-001', 'SURG-L5-001'],
    difficulty: 4,
    species: '貓',
    scenario: {
      signalment: '4 歲已絕育雄性家貓（DSH），體重 5.2 kg',
      chief_complaint: '過去 6 個月內第三次尿道阻塞，本次阻塞約 18 小時，完全無法排尿',
      brief_history: '第一次阻塞（6 個月前）經導尿解除後以處方飼料（泌尿道配方）管理，2 個月後再次阻塞，導尿後加用 prazosin 0.5 mg PO q12h。本次為第三次阻塞，飼主發現貓持續進出貓砂盆但無尿液產出，嘔吐兩次，精神極度沉鬱。已由轉診前醫院建立靜脈輸液。室內飼養，無其他同居貓。過去尿液分析顯示草酸鈣結晶與特發性膀胱炎。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集與手術適應症評估',
        prompt: '這隻公貓因第三次尿道阻塞被轉診至外科。你需要進一步確認哪些資訊以評估是否適合會陰尿道造口術（perineal urethrostomy, PU）？請選擇最重要的 3 項。',
        available_actions: [
          '確認過去三次阻塞的原因（結石 vs 尿道栓子 vs 特發性痙攣）與每次解除方式',
          '確認目前內科治療完整性（處方飼料、prazosin、環境豐富化、鎮痛劑）',
          '確認阻塞持續時間與目前腎功能狀態（BUN/Cre/K+）',
          '詢問飼料品牌與每日餵食量',
          '詢問是否有其他寵物',
          '詢問是否有定期洗澡',
        ],
        correct_actions: [
          '確認過去三次阻塞的原因（結石 vs 尿道栓子 vs 特發性痙攣）與每次解除方式',
          '確認目前內科治療完整性（處方飼料、prazosin、環境豐富化、鎮痛劑）',
          '確認阻塞持續時間與目前腎功能狀態（BUN/Cre/K+）',
        ],
        feedback_correct: '正確！PU 手術的適應症評估需要：(1) 阻塞的病因——特發性 FLUTD 合併反覆阻塞（≥ 3 次或內科無法控制）是最常見的 PU 適應症，純結石阻塞需先評估結石類型是否可溶解；(2) 內科治療是否已最大化——確認處方飼料遵從性、prazosin、環境豐富化、止痛（gabapentin/buprenorphine）均已嘗試後仍反覆阻塞才考慮手術；(3) 急性腎後性阻塞的嚴重度決定術前穩定化的需求。',
        feedback_incorrect: '反覆尿道阻塞轉診外科時，最關鍵的病史是：阻塞原因（影響手術決策）、內科治療完整性（確認非內科不遵從）、以及目前阻塞嚴重度（影響術前穩定化）。飼料品牌等非核心資訊。',
        key_learning_points: [
          'PU 手術適應症：≥ 3 次阻塞、內科治療最大化後仍反覆阻塞、或無法導尿的尿道損傷',
          '貓下泌尿道疾病（FLUTD）約 55-69% 為特發性（FIC），非結石',
          '術前必須評估腎功能——阻塞 > 24 小時可致高血鉀、急性腎損傷',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查與急診穩定化',
        prompt: '病史確認：三次均為特發性 FLUTD 合併黏液蛋白栓子阻塞，內科治療已最大化。本次阻塞約 18 小時。進行理學檢查，請選出最關鍵的 3 項發現。',
        available_actions: [
          '膀胱極度膨大且堅硬（turgid），觸診時貓表現劇烈疼痛',
          '心搏過緩（HR 140 bpm，貓正常 160-220），心律不整',
          '體溫偏低 37.2°C，黏膜蒼白、CRT 3 秒，精神極度沉鬱',
          '體重 5.2 kg，BCS 6/9',
          '口腔檢查齒齦正常',
          '被毛略為粗糙',
        ],
        correct_actions: [
          '膀胱極度膨大且堅硬（turgid），觸診時貓表現劇烈疼痛',
          '心搏過緩（HR 140 bpm，貓正常 160-220），心律不整',
          '體溫偏低 37.2°C，黏膜蒼白、CRT 3 秒，精神極度沉鬱',
        ],
        feedback_correct: '正確！膀胱堅硬膨大確認完全阻塞。心搏過緩 + 心律不整高度懷疑高血鉀（K+ > 7.0 mEq/L 可致心臟傳導異常）——這是尿道阻塞最致命的併發症。低體溫 + 循環不良（蒼白黏膜、CRT 延長）提示代謝性酸中毒與血流動力學不穩定。必須先穩定化再進行任何手術。',
        feedback_incorrect: '尿道阻塞急診最關鍵的理學檢查：膀胱觸診（確認阻塞程度）、心率與心律（高血鉀徵兆）、循環狀態（休克評估）。心搏過緩在貓尿道阻塞中提示致命性高血鉀。',
        key_learning_points: [
          '貓尿道阻塞 > 24 小時死亡率顯著上升——主要致死原因為高血鉀性心律不整',
          '心搏過緩（貓 < 160 bpm）+ 心律不整 = 高度懷疑高血鉀（K+ > 7 mEq/L）',
          '術前穩定化優先於手術——先解除阻塞、矯正電解質、恢復腎功能',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '術前穩定化與檢查',
        prompt: '急診血液檢查結果：K+ 8.2 mEq/L（正常 3.5-5.5）、BUN 145 mg/dL、Cre 9.8 mg/dL、pH 7.18。ECG 顯示高尖 T 波與 P 波消失。你的下一步？選擇最適當的 3 項。',
        available_actions: [
          '立即靜脈注射 calcium gluconate 10%（0.5-1.0 mL/kg 緩慢 IV over 10 min）作為心臟保護',
          '靜脈給予 regular insulin（0.25-0.5 IU/kg IV）+ dextrose（1-2 g/unit insulin IV）驅鉀入細胞',
          '緊急導尿解除阻塞，同時開始生理食鹽水（0.9% NaCl）輸液矯正脫水與沖刷鉀離子',
          '立即安排手術進行 PU',
          '口服降鉀藥物 polystyrene sulfonate',
          '僅觀察等待鉀離子自行下降',
        ],
        correct_actions: [
          '立即靜脈注射 calcium gluconate 10%（0.5-1.0 mL/kg 緩慢 IV over 10 min）作為心臟保護',
          '靜脈給予 regular insulin（0.25-0.5 IU/kg IV）+ dextrose（1-2 g/unit insulin IV）驅鉀入細胞',
          '緊急導尿解除阻塞，同時開始生理食鹽水（0.9% NaCl）輸液矯正脫水與沖刷鉀離子',
        ],
        feedback_correct: '正確！K+ 8.2 mEq/L 合併 ECG 變化是緊急狀態：(1) Calcium gluconate 不降低鉀但在 2-5 分鐘內穩定心肌，效果持續 20-30 分鐘，為爭取時間的第一步；(2) Insulin + dextrose 在 15-30 分鐘內將鉀驅入細胞內，降低血清鉀約 0.5-1.0 mEq/L；(3) 導尿解除阻塞後腎臟恢復排鉀功能，0.9% NaCl 輸液同時稀釋鉀與矯正脫水。PU 手術在穩定化後 24-72 小時安排——不可在高血鉀狀態下全身麻醉。',
        feedback_incorrect: '高血鉀合併 ECG 變化是致命急診——必須三管齊下：calcium gluconate（心臟保護）→ insulin + dextrose（驅鉀入細胞）→ 導尿 + 輸液（排除鉀離子根源）。不可在高血鉀時麻醉手術。',
        key_learning_points: [
          'Calcium gluconate 穩定心肌（不降鉀），是高血鉀急救第一步',
          'Regular insulin + dextrose 是降低血清鉀最有效的藥物組合',
          'PU 手術應在電解質與腎功能穩定後 24-72 小時內安排，非急診手術',
        ],
      },
      {
        step_number: 4,
        step_type: 'treatment_plan',
        title: '會陰尿道造口術——手術規劃',
        prompt: '48 小時後，貓的電解質已正常化（K+ 4.1）、腎功能改善（Cre 2.8）、導尿管通暢。決定進行 PU 手術。關於手術技術，以下最正確的 3 項是？',
        available_actions: [
          '手術目標：將較寬的骨盆尿道（pelvic urethra）開口至會陰部皮膚，繞過狹窄的陰莖尿道（penile urethra）',
          '關鍵解剖：球海綿體肌（bulbospongiosus muscle）分離後暴露骨盆尿道，注意保護尿道背側的直腸',
          '吻合技巧：尿道黏膜與皮膚以 4-0/5-0 可吸收縫線（PDS/Monocryl）做 simple interrupted 吻合，確保無張力、黏膜外翻',
          '保留陰莖——僅切開尿道即可',
          '使用不可吸收縫線（nylon）縫合尿道黏膜',
          '術中不需要導尿管定位',
        ],
        correct_actions: [
          '手術目標：將較寬的骨盆尿道（pelvic urethra）開口至會陰部皮膚，繞過狹窄的陰莖尿道（penile urethra）',
          '關鍵解剖：球海綿體肌（bulbospongiosus muscle）分離後暴露骨盆尿道，注意保護尿道背側的直腸',
          '吻合技巧：尿道黏膜與皮膚以 4-0/5-0 可吸收縫線（PDS/Monocryl）做 simple interrupted 吻合，確保無張力、黏膜外翻',
        ],
        feedback_correct: '正確！PU 手術的核心原則：(1) 陰莖尿道直徑僅 0.7 mm（阻塞好發處），骨盆尿道直徑 3-4 mm——造口於骨盆尿道層級可大幅降低再阻塞率；(2) 球海綿體肌是定位骨盆尿道的解剖標誌，必須完全分離以暴露足夠長度的尿道；(3) 黏膜-皮膚吻合使用可吸收單股縫線，確保黏膜外翻（mucosa everted）以防止縫合處狹窄。導尿管（5 Fr red rubber）術中保留以利定位與沖洗。陰莖需完整切除以避免殘留的陰莖尿道成為藏汙納垢處。',
        feedback_incorrect: 'PU 手術的三個關鍵：(1) 造口位置在骨盆尿道（寬）而非陰莖尿道（窄）；(2) 球海綿體肌分離是手術核心步驟；(3) 黏膜外翻吻合防止狹窄。陰莖須切除，使用可吸收縫線，術中保留導尿管輔助定位。',
        key_learning_points: [
          '陰莖尿道直徑 ~0.7 mm vs 骨盆尿道 ~3-4 mm——PU 造口必須在骨盆尿道層級',
          '球海綿體肌（bulbospongiosus）是 PU 手術的關鍵解剖標誌',
          '黏膜-皮膚吻合使用 4-0/5-0 可吸收單股縫線，黏膜外翻以防狹窄',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '術後併發症監測與長期管理',
        prompt: 'PU 手術順利完成，尿道造口通暢。術後管理中最重要的 3 項是？',
        available_actions: [
          '術後 5-7 天使用伊莉莎白圈防止舔舐，監測造口水腫與出血。術後 24 小時內應見自主排尿。抗生素使用 7-10 天（amoxicillin-clavulanate）',
          '長期監測：PU 術後尿路感染（UTI）發生率約 20-30%，因造口繞過正常的尿道防禦機制。建議每 6 個月尿液培養監測',
          '告知飼主預後：PU 術後再阻塞率約 10-25%（多為造口狹窄所致），但 85-90% 飼主滿意度高。繼續處方飼料與環境豐富化以降低 FIC 發作頻率',
          '術後立即拔除導尿管',
          '不需要伊莉莎白圈',
          '術後不需要追蹤，完全治癒',
        ],
        correct_actions: [
          '術後 5-7 天使用伊莉莎白圈防止舔舐，監測造口水腫與出血。術後 24 小時內應見自主排尿。抗生素使用 7-10 天（amoxicillin-clavulanate）',
          '長期監測：PU 術後尿路感染（UTI）發生率約 20-30%，因造口繞過正常的尿道防禦機制。建議每 6 個月尿液培養監測',
          '告知飼主預後：PU 術後再阻塞率約 10-25%（多為造口狹窄所致），但 85-90% 飼主滿意度高。繼續處方飼料與環境豐富化以降低 FIC 發作頻率',
        ],
        feedback_correct: '正確！PU 術後管理重點：(1) 短期——伊莉莎白圈保護造口 5-7 天（舔舐是造口失敗的常見原因）、監測出血與水腫、確認排尿通暢；(2) 長期——UTI 是 PU 最常見的長期併發症（造口位置較低且缺乏陰莖尿道的自然防禦），需定期尿液培養而非僅尿液分析（亞臨床菌尿常見）；(3) PU 手術降低的是「阻塞」頻率，但不治癒 FIC 本身——持續的內科管理（處方飼料、環境豐富化、減壓）至關重要。',
        feedback_incorrect: '術後關鍵：伊莉莎白圈（舔舐是造口失敗主因）、長期 UTI 監測（PU 破壞尿道防禦）、持續內科管理 FIC。PU 降低阻塞但不治癒 FIC。',
        key_learning_points: [
          'PU 術後最常見併發症：造口狹窄（10-25%）、UTI（20-30%）、出血',
          'PU 繞過尿道自然防禦機制，長期 UTI 風險上升——需定期尿液培養',
          'PU 降低阻塞頻率但不治癒 FIC——內科管理（飲食、環境、止痛）必須持續',
        ],
      },
      {
        step_number: 6,
        step_type: 'case_summary',
        title: '病例總結與關鍵教訓',
        prompt: '回顧此病例，以下哪 3 項是最重要的學習重點？',
        available_actions: [
          'PU 手術是反覆尿道阻塞（≥ 3 次或內科最大化仍失敗）的標準治療。手術核心在於骨盆尿道-皮膚吻合，繞過狹窄的陰莖尿道',
          '高血鉀是尿道阻塞最致命的併發症——急救三步驟：calcium gluconate（心臟保護）→ insulin + dextrose（驅鉀）→ 導尿 + 輸液（排鉀）',
          'PU 術後需長期追蹤：UTI（20-30%）、造口狹窄（10-25%）為最常見併發症。FIC 的內科管理不可停止',
          '所有尿道阻塞的貓都應立即手術',
          'PU 可完全治癒 FLUTD',
          '術後不需要定期回診',
        ],
        correct_actions: [
          'PU 手術是反覆尿道阻塞（≥ 3 次或內科最大化仍失敗）的標準治療。手術核心在於骨盆尿道-皮膚吻合，繞過狹窄的陰莖尿道',
          '高血鉀是尿道阻塞最致命的併發症——急救三步驟：calcium gluconate（心臟保護）→ insulin + dextrose（驅鉀）→ 導尿 + 輸液（排鉀）',
          'PU 術後需長期追蹤：UTI（20-30%）、造口狹窄（10-25%）為最常見併發症。FIC 的內科管理不可停止',
        ],
        feedback_correct: '優秀！此病例整合了 FLUTD 的內科管理失敗後轉向外科介入的完整思路：先穩定致命併發症（高血鉀）→ 恢復腎功能 → 擇期 PU 手術 → 長期追蹤。PU 手術雖然是泌尿外科的基本手術，但正確的患者篩選、術前穩定化、精準的手術技巧與術後長期管理同等重要。',
        feedback_incorrect: '本病例的核心教訓：PU 手術需正確的適應症（反覆阻塞 + 內科最大化失敗）、致命併發症（高血鉀）的急救、以及術後長期併發症的監測。不是所有阻塞都需手術，PU 也不能治癒 FIC。',
        key_learning_points: [
          '貓 PU 手術的完整流程：適應症評估 → 術前穩定化 → 手術 → 長期追蹤',
          '高血鉀急救的藥物學：calcium gluconate（秒級心臟保護）→ insulin/dextrose（分鐘級降鉀）→ 輸液排鉀（小時級）',
          'FLUTD 是終身管理的疾病——PU 僅降低阻塞頻率，飲食與環境管理不可中斷',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '4 歲公貓反覆尿道阻塞（3 次/6 個月），內科治療已最大化（處方飼料 + prazosin + 環境豐富化）仍復發 → 符合 PU 手術適應症。本次阻塞 18 小時合併高血鉀（K+ 8.2）+ ECG 變化 → 先急救穩定化（calcium gluconate → insulin/dextrose → 導尿/輸液）→ 48 小時後電解質正常化、腎功能改善 → 擇期 PU 手術 → 術後長期 UTI 監測與 FIC 內科管理。',
      key_decisions: [
        '確認 PU 手術適應症：反覆阻塞 + 內科最大化失敗',
        '高血鉀急救優先於手術規劃',
        '術前穩定 24-72 小時後再擇期手術',
        '骨盆尿道-皮膚黏膜外翻吻合的手術核心技巧',
        '術後長期 UTI 監測與 FIC 持續管理',
      ],
      common_pitfalls: [
        '在高血鉀狀態下急於手術——全麻風險極高',
        '造口位置過低（陰莖尿道）導致再狹窄',
        '黏膜未外翻吻合導致術後狹窄',
        '術後忽略 FIC 內科管理——僅靠 PU 無法根治 FLUTD',
        '未定期監測 UTI（亞臨床菌尿常見）',
      ],
    },
    related_nodes_for_review: ['SURG-L3-016', 'SURG-L5-001', 'SURG-L5-003', 'IM-L3-010', 'ECC-L3-003'],
    tags: ['PU', '會陰尿道造口術', 'perineal_urethrostomy', 'FLUTD', '尿道阻塞', '貓', '高血鉀', 'case_challenge'],
  },
  {
    id: 'CASE-SURG-014',
    title: '幼貓急性嘔吐 — 線狀腸道異物',
    specialty: 'SURG',
    cross_specialties: ['IM', 'ECC'],
    required_nodes: ['SURG-L3-003', 'SURG-L3-015', 'SURG-L4-001'],
    difficulty: 3,
    species: '貓',
    scenario: {
      signalment: '2 歲已絕育雌性家貓（DSH），體重 3.8 kg',
      chief_complaint: '急性嘔吐 3 天，完全拒食，精神越來越差',
      brief_history: '3 天前開始反覆嘔吐，初期為食物後轉為黃色膽汁。完全拒食但仍少量飲水。飼主注意到縫紉用具的線軸近日被打翻。大便量減少但非完全無排便。定期預防注射與驅蟲。室內飼養，有一隻同居犬。過去健康無特殊。',
    },
    steps: [
      {
        step_number: 1,
        step_type: 'history_collection',
        title: '病史採集與異物風險評估',
        prompt: '2 歲母貓急性嘔吐 3 天、拒食、飼主提及線軸被打翻。你需要進一步確認哪些關鍵資訊？選擇最重要的 3 項。',
        available_actions: [
          '詢問線軸上的線是否有缺少——評估吞入線狀異物（linear foreign body）的可能性',
          '詢問嘔吐的頻率與型態——是否有進行性惡化（提示持續阻塞而非單純胃炎）',
          '詢問貓是否有玩線、緞帶、橡皮筋等習慣（貓的線狀異物行為風險評估）',
          '詢問飼料品牌是否更換',
          '詢問是否有打噴嚏或流鼻水',
          '詢問同居犬是否也有嘔吐',
        ],
        correct_actions: [
          '詢問線軸上的線是否有缺少——評估吞入線狀異物（linear foreign body）的可能性',
          '詢問嘔吐的頻率與型態——是否有進行性惡化（提示持續阻塞而非單純胃炎）',
          '詢問貓是否有玩線、緞帶、橡皮筋等習慣（貓的線狀異物行為風險評估）',
        ],
        feedback_correct: '正確！線狀異物（linear foreign body）在貓的腸道異物中佔約 30-50%，是最危險的異物類型。確認線是否缺少可提高臨床懷疑度。進行性嘔吐（由食物轉為膽汁）提示腸道阻塞而非單純胃炎。貓特別喜歡玩線狀物品（線、緞帶、毛線、橡皮筋、聖誕燈飾線），行為史有助於風險分層。',
        feedback_incorrect: '年輕貓急性嘔吐 + 近期接觸線軸 = 線狀異物高度懷疑。關鍵病史：線是否缺少（直接證據）、嘔吐型態（阻塞 vs 胃炎）、異物行為史（風險評估）。飼料和呼吸道症狀非此情境優先。',
        key_learning_points: [
          '線狀異物佔貓腸道異物 30-50%，是最危險的異物類型',
          '典型吞入物品：縫線、緞帶、毛線、橡皮筋、聖誕燈飾線',
          '進行性嘔吐（食物 → 膽汁）提示腸道阻塞而非單純胃炎',
        ],
      },
      {
        step_number: 2,
        step_type: 'physical_examination',
        title: '理學檢查',
        prompt: '飼主確認線軸上約 1 公尺的線不見了，且此貓有玩線習慣。進行理學檢查。選出最關鍵的 3 項發現。',
        available_actions: [
          '腹部觸診：多處腸段呈「串珠狀」或「手風琴褶狀」聚集（bunching），觸診時貓表現疼痛',
          '口腔檢查：舌根基部發現一段線卡住（sublingual linear foreign body anchor），線從舌頭下方消失進入消化道',
          '脫水約 7%（皮膚帳篷延遲、黏膜略乾），心搏 200 bpm，體溫 39.8°C',
          '腹部 X 光觸診無異常',
          '呼吸正常無異狀',
          '四肢關節活動度正常',
        ],
        correct_actions: [
          '腹部觸診：多處腸段呈「串珠狀」或「手風琴褶狀」聚集（bunching），觸診時貓表現疼痛',
          '口腔檢查：舌根基部發現一段線卡住（sublingual linear foreign body anchor），線從舌頭下方消失進入消化道',
          '脫水約 7%（皮膚帳篷延遲、黏膜略乾），心搏 200 bpm，體溫 39.8°C',
        ],
        feedback_correct: '正確！線狀異物的經典三聯徵：(1) 腸段聚集（plication/bunching）——線的一端固定，腸管沿線收縮聚攏形成手風琴狀；(2) 舌下錨定點（sublingual anchor）——約 50% 的貓線狀異物會卡在舌根基部，口腔檢查是必做步驟；(3) 腹痛 + 發燒提示可能有腸壁穿孔或即將穿孔。脫水需術前矯正。',
        feedback_incorrect: '線狀異物的理學檢查重點：腹部腸段聚集（bunching）、口腔舌下錨定點檢查（50% 可見）、以及全身狀態評估（脫水、發燒、腹痛）。舌下檢查是貓嘔吐時必做的步驟。',
        key_learning_points: [
          '貓嘔吐時必須檢查口腔舌根——約 50% 線狀異物錨定於舌下',
          '腸段聚集（plication/bunching）是線狀異物最具特異性的觸診發現',
          '發燒 + 腹痛提示腸壁可能已穿孔——手術急迫性上升',
        ],
      },
      {
        step_number: 3,
        step_type: 'diagnostic_plan',
        title: '影像學確認',
        prompt: '臨床高度懷疑線狀異物。你的影像檢查計畫？選擇最適當的 3 項。',
        available_actions: [
          '腹部 X 光（lateral + VD）：預期看到小腸聚集（plicated small intestine）、偏心性氣泡（eccentric gas bubbles）、以及逗號形小腸段',
          '腹部超音波確認腸管聚集型態，評估有無腹腔游離液（提示穿孔）',
          '血液檢查（CBC、生化、電解質）評估發炎程度與脫水/電解質失衡，為手術做準備',
          '上消化道鋇劑造影',
          '腹腔穿刺',
          'CT 掃描',
        ],
        correct_actions: [
          '腹部 X 光（lateral + VD）：預期看到小腸聚集（plicated small intestine）、偏心性氣泡（eccentric gas bubbles）、以及逗號形小腸段',
          '腹部超音波確認腸管聚集型態，評估有無腹腔游離液（提示穿孔）',
          '血液檢查（CBC、生化、電解質）評估發炎程度與脫水/電解質失衡，為手術做準備',
        ],
        feedback_correct: '正確！X 光是第一線工具——線狀異物的經典影像為腸管聚集、偏心性氣泡（gas accumulates on anti-mesenteric side）、逗號形腸段。超音波更敏感，可直接看到聚集的腸管與腸系膜側的線狀高回音影，且可評估腹腔游離液（腹膜炎/穿孔徵兆）。術前血液檢查評估感染程度（WBC）、腎功能與電解質，指導輸液治療。鋇劑造影在已有臨床確診時不建議——延遲手術且鋇劑如穿孔會造成化學性腹膜炎。',
        feedback_incorrect: '線狀異物影像學：X 光（腸管聚集 + 偏心氣泡）→ 超音波（確認聚集 + 評估穿孔）→ 血液檢查（術前評估）。鋇劑在已確診時不建議。',
        key_learning_points: [
          '線狀異物 X 光經典三徵：腸段聚集、偏心性氣泡、逗號形腸段',
          '超音波可直接看到線狀異物的高回音影與腸管聚集',
          '腹腔游離液（free fluid）提示腸壁穿孔——手術緊急性從擇期升級為緊急',
        ],
      },
      {
        step_number: 4,
        step_type: 'treatment_plan',
        title: '手術決策與技巧',
        prompt: '影像確認線狀異物合併小腸聚集。血液檢查：WBC 22,000/μL（嗜中性球左移）、BUN/Cre 正常、低氯低鉀。超音波未見游離腹腔液。術前輸液矯正電解質後準備手術。手術操作最重要的 3 項原則是？',
        available_actions: [
          '先檢查舌下錨定點——剪斷舌下的線（不可拉扯），然後開腹探查',
          '可能需要多處腸切開術（multiple enterotomies）——在線狀異物造成的腸系膜側穿孔風險最高處分段取出線，絕對不可從單一切口拉出整條線',
          '每個切開處術中評估腸壁活性——若有壞死或穿孔區域需做腸段切除吻合術（resection and anastomosis），以血流與蠕動評估活性',
          '從舌下拉出整條線即可',
          '僅做一處腸切開拉出所有線段',
          '使用內視鏡取出線狀異物',
        ],
        correct_actions: [
          '先檢查舌下錨定點——剪斷舌下的線（不可拉扯），然後開腹探查',
          '可能需要多處腸切開術（multiple enterotomies）——在線狀異物造成的腸系膜側穿孔風險最高處分段取出線，絕對不可從單一切口拉出整條線',
          '每個切開處術中評估腸壁活性——若有壞死或穿孔區域需做腸段切除吻合術（resection and anastomosis），以血流與蠕動評估活性',
        ],
        feedback_correct: '正確！線狀異物手術的絕對禁忌是「從單一切口拉出整條線」——這會像鋸子一樣切穿腸系膜側腸壁，造成多處穿孔與致命性腹膜炎。正確做法：(1) 舌下剪斷解除錨定；(2) 開腹後全程檢查腸管，在每個聚集處做獨立的 enterotomy，分段移除線段；(3) 通常需要 2-4 處 enterotomy；(4) 每處切開時評估腸壁——腸系膜側是穿孔風險最高處，壞死腸段需切除吻合。貓小腸直徑小，吻合使用 4-0/5-0 可吸收縫線。',
        feedback_incorrect: '線狀異物手術最致命的錯誤：從單一切口拉線——會切穿腸壁。正確：舌下剪斷 → 多處 enterotomy 分段取出 → 評估每段腸壁活性 → 壞死段切除吻合。',
        key_learning_points: [
          '線狀異物絕對禁忌：從單一切口拉出整條線——會造成多處腸壁穿孔',
          '正確做法：多處腸切開術（multiple enterotomies），通常需 2-4 處',
          '腸系膜側（mesenteric border）是線狀異物最容易切穿腸壁的部位',
        ],
      },
      {
        step_number: 5,
        step_type: 'treatment_plan',
        title: '術後管理與預後',
        prompt: '手術完成：舌下剪斷線後，經 3 處 enterotomy 分段取出約 90 cm 縫線。一段約 8 cm 的空腸因腸系膜側穿孔合併局部腹膜炎，執行腸段切除吻合。腹腔沖洗後關腹。術後最重要的管理重點是？',
        available_actions: [
          '術後 12-24 小時禁食後開始少量多餐易消化飲食（a/d 或皇家腸胃道處方）。監測嘔吐——若術後持續嘔吐需排除腸吻合處滲漏',
          '抗生素治療：因已有局部腹膜炎，使用廣效抗生素（ampicillin/sulbactam + metronidazole）覆蓋需氧與厭氧菌，療程 7-14 天',
          '疼痛管理（buprenorphine 0.02 mg/kg IV/buccal q6-8h）+ 監測白蛋白（腹膜炎可致低白蛋白血症）。預後：無穿孔存活率 > 90%，有穿孔約 70-80%',
          '術後可立即恢復正常飲食',
          '不需要抗生素',
          '預後極差，存活率 < 30%',
        ],
        correct_actions: [
          '術後 12-24 小時禁食後開始少量多餐易消化飲食（a/d 或皇家腸胃道處方）。監測嘔吐——若術後持續嘔吐需排除腸吻合處滲漏',
          '抗生素治療：因已有局部腹膜炎，使用廣效抗生素（ampicillin/sulbactam + metronidazole）覆蓋需氧與厭氧菌，療程 7-14 天',
          '疼痛管理（buprenorphine 0.02 mg/kg IV/buccal q6-8h）+ 監測白蛋白（腹膜炎可致低白蛋白血症）。預後：無穿孔存活率 > 90%，有穿孔約 70-80%',
        ],
        feedback_correct: '正確！線狀異物合併穿孔與腹膜炎的術後管理關鍵：(1) 控制飲食——術後 12-24 小時禁食讓腸吻合處初步癒合（漿膜面 fibrin seal 需 3-4 小時，足夠張力需 10-14 天），術後持續嘔吐是滲漏的警訊（漏出率 5-10%）；(2) 廣效抗生素覆蓋腸道菌群（需氧 + 厭氧）；(3) 疼痛管理對貓至關重要——buprenorphine 是貓腹部手術首選鎮痛劑。整體預後：無穿孔 > 90% 存活，有穿孔合併腹膜炎約 70-80%。',
        feedback_incorrect: '線狀異物合併穿孔的術後管理：控制飲食（禁食後漸進恢復）、廣效抗生素（已有腹膜炎）、疼痛管理。整體預後良好（有穿孔 70-80% 存活），不是極差。',
        key_learning_points: [
          '腸吻合術後漿膜面 fibrin seal 需 3-4 小時，足夠張力需 10-14 天',
          '術後持續嘔吐需排除吻合處滲漏（dehiscence/leakage rate 5-10%）',
          '貓線狀異物預後：無穿孔 > 90% 存活率，有穿孔合併腹膜炎 70-80%',
        ],
      },
    ],
    expert_pathway: {
      reasoning: '2 歲母貓急性嘔吐 + 接觸線軸 → 口腔舌下發現線狀異物錨定 → 腹部觸診腸段聚集 → X 光/超音波確認線狀異物 + 腸管 plication → 術前穩定化（輸液、電解質矯正）→ 舌下剪線 + 3 處 enterotomy 分段取出 + 壞死段 R&A → 術後抗生素 + 疼痛管理 + 漸進飲食恢復。',
      key_decisions: [
        '口腔檢查舌下——發現錨定點為確診關鍵',
        '絕不從單一切口拉線——必須多處 enterotomy',
        '術中每段腸壁活性評估——壞死段切除吻合',
        '穿孔合併腹膜炎需廣效抗生素',
        '術後嘔吐需警覺吻合處滲漏',
      ],
      common_pitfalls: [
        '未檢查口腔舌下而漏診線狀異物',
        '從單一切口拉出整條線——導致腸壁多處穿孔（致命錯誤）',
        '鋇劑造影延遲手術且增加穿孔風險',
        '忽略腸系膜側穿孔——漏診需切除的壞死腸段',
        '術後未適當禁食導致吻合處滲漏',
      ],
    },
    related_nodes_for_review: ['SURG-L3-003', 'SURG-L3-015', 'SURG-L4-001', 'SURG-L5-001', 'SURG-L5-003'],
    tags: ['linear_foreign_body', '線狀異物', '腸道異物', 'enterotomy', '腸切除', '貓', 'case_challenge'],
  },
];
