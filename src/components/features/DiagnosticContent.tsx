...
export function DiagnosticContent({ data, className }: DiagnosticContentProps) {
  ...
  function renderTabContent(): React.ReactNode {
    switch (activeTab) {
      case 'indications':
        return (
          <div className="space-y-4">
            <div>
              <h4 className="mb-2 text-base font-semibold text-green-700">適應症</h4>
              <ul className="space-y-1">
                {data.indication.map((item, i) => (
                  <li key={i} className="text-base text-gray-700">&bull; {item}</li>
                ))}
              </ul>
            </div>
            <div>
              <h4 className="mb-2 text-base font-semibold text-red-700">禁忌症</h4>
      ...
      case 'technique':
        return <p className="text-base text-gray-700 whitespace-pre-wrap">{data.technique}</p>;
      case 'normal':
        return (
            ...
            {data.normal_findings.length > 0 ? (
              data.normal_findings.map((finding, i) => (
                <div key={i} className="rounded border border-gray-100 bg-gray-50 p-3">
                  <p className="text-base font-medium text-gray-800">{finding.finding}</p>
                  <p className="mt-0.5 text-sm text-gray-600">{finding.description}</p>
                  <p className="mt-0.5 text-sm text-indigo-600">意義：{finding.significance}</p>
                </div>
              ))
            ) : (
            ...
            {data.abnormal_findings.length > 0 ? (
              data.abnormal_findings.map((finding, i) => (
                <div key={i} className="rounded border border-red-100 bg-red-50 p-3">
                  <p className="text-base font-medium text-red-800">{finding.finding}</p>
                  <p className="mt-0.5 text-sm text-red-600">{finding.description}</p>
                  <p className="mt-0.5 text-sm text-red-700 font-medium">意義：{finding.significance}</p>
                </div>
              ))
            ) : (
              <p className="text-sm text-gray-400">無資料</p>
      ...
      case 'interpretation':
        return (
          <div className="space-y-3">
            <p className="text-base text-gray-700 whitespace-pre-wrap">{data.interpretation_guide}</p>
            {data.sensitivity_specificity && (
              <div className="rounded bg-blue-50 p-3">
                <p className="text-sm font-medium text-blue-700">敏感度/特異度</p>
                <p className="text-base text-blue-800">{data.sensitivity_specificity}</p>
              </div>
            )}
            {data.cost_benefit && (
              <div className="rounded bg-amber-50 p-3">
                <p className="text-sm font-medium text-amber-700">成本效益</p>
                <p className="text-base text-amber-800">{data.cost_benefit}</p>
              </div>
            )}
          </div>
        );
  ...
  return (
    <div className={cn('space-y-3', className)}>
      {/* Tabs */}
      <div role="tablist" aria-label="診斷方法分頁" className="flex flex-wrap gap-1 border-b border-gray-200 pb-1">
        {TABS.map((tab) => (
          <button
            key={tab.key}
            type="button"
